...
//! matches the `adnet-room-…` convention.
//!
//! Run with:
//! ```bash
//! cargo run -p adnet-workspace --example workspace_app
//! ```

use adnet_workspace::{Workspace, split_name_ext, workspace_room_topic};
use std::fs;
use std::io::Write;
use std::path::Path;
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    // 4. Topic name follows the `adnet-room-…` convention.
    let topic = workspace_room_topic();
    println!("gossip topic: {topic}");
    assert!(topic.starts_with("adnet-room-"));
