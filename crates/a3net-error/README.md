...
> A3Net 统一的错误报告模型(`AdnetErrorReport` + `ErrorKind` + `Severity`),在 RPC、FFI、CLI、HTTP gateway 等边界处提供稳定的、可机器解析的错误形态。

## 概览 (Overview)

A3Net 中每个 crate 各自定义 `thiserror::Error` 枚举,适合内部使用;但在 **边界**(RPC、FFI、CLI 退出码、HTTP gateway)上需要一个稳定、可被运维分组计数与追踪的统一错误模型。`a3net-error` 就是 A3Net 的"边界错误契约":
...
- **`emit()`** — 在 `tracing` 子系统中输出,并在开启 `metrics` feature 时自增 `a3net_error_total` 计数器。
...
- **Optional features**:
  ...
  - `metrics` — 启用 `a3net_error_total` 计数器,注册到 `a3net-observability::Registry`。

## 安装 (Installation)

`a3net-error` 是一个工作空间内的 lib crate,被 A3Net 所有面向边界的 crate 依赖。

```toml
# crates/<your-crate>/Cargo.toml
[dependencies]
a3net-error = { workspace = true }
a3net-observability = { workspace = true }   # 若要使用 metrics feature
```

在代码中:

```rust
use a3net_error::{AdnetErrorReport, ErrorKind, Severity, IntoReport};
```
...
```rust
use a3net_error::{AdnetErrorReport, IntoReport, ErrorKind, Severity};
...
```rust
use a3net_error::{ErrorKind, AdnetErrorReport};

let report: AdnetErrorReport = serde_json::from_str(r#"{
  "code":"BLB-001","kind":"NotFound","severity":"Warn",
  "message":"blob not found","crate_name":"a3net-blobstore","details":{}
}"#).unwrap();
...
```rust
use a3net_error::{AdnetErrorReport, IntoReport, Severity};

let inner = std::io::Error::new(std::io::ErrorKind::UnexpectedEof, "short read");
let report = inner.into_report("a3net-demo");
// report.cause 会被填为 "short read",更深的链也会被折叠进 cause 字段
```

## 应用案例 (Use Cases / Examples)

- **`a3net-rpc` / `a3net-ffi`** — RPC handler 在边界上把所有内部错误举升为 `AdnetErrorReport`,JSON-RPC 客户端在拿到错误码后可由 `code` 直接跳到 UX 提示,无需解析 `message` 字符串。
- **CLI 退出码** — `a3net-cli` 把 `AdnetErrorReport` 映射为 `process::ExitCode`,`code` 决定退出码分组,`Severity::Fatal` 直接 `ExitCode::from(70)`(EX_SOFTWARE)。
- **HTTP gateway** — `a3net-gateway` 走 `ErrorKind::http_status()` 把错误转换为 4xx / 5xx,并把 `correlation` 透传回响应头(`X-Adnet-Correlation`)。
- **告警平台** — 开启 `metrics` feature 后,告警平台按 `a3net_error_total{code,kind,severity}` 分组,`Severity::Fatal` 直接触发页面。
