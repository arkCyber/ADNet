...
//! and we lift it through the boundary in the same way the A3Net RPC
//! layer does. Demonstrates the cause chain, code convention, and the
//! helper that maps `ErrorKind` to HTTP status codes.
//!
//! Run with:
//! ```bash
//! cargo run -p a3net-error --example a3net_error_app
//! ```
...
fn main() {
    ...
    let report = outer.into_report("a3net-demo");
    ...
    let client_err = ServiceError::BadTicket("not a valid a3net-blob://ticket".into());
    let client_report = client_err.into_report("a3net-demo");
    println!(
