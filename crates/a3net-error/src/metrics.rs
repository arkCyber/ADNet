...
//! of `a3net-error` is cycle-free: `a3net-observability`
//! can depend on `a3net-error` without forming a cycle.
//!
//! When the feature is enabled, [`counter_inc_from_report`]
//! bumps the `a3net_error_total` Prometheus counter with
...
/// `a3net_error_total` counter.
///
/// `a3net-observability::registry::GLOBAL.register_counter`
    ...
    ERROR_TOTAL
        .get_or_init(|| {
            GLOBAL.register_counter(
                "a3net_error_total",
                "Total A3Net errors emitted via AdnetErrorReport::emit",
            )
        })
        .clone()
}

/// Bump the `a3net_error_total` counter. Idempotent — the
    ...
    match LabelSet::new(pairs) {
        ...
        Err(e) => {
            ...
            tracing::warn!(
                error = %e,
                code = %report.code,
                "a3net_error_total: failed to build LabelSet; counter not incremented",
            );
        }
    }
}
...
/// `a3net_error_total` counter for the supplied labels, or
