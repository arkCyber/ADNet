...
#[derive(Parser)]
#[command(name = "a3net-model-catalog")]
#[command(about = "A3Net AI Model Distribution Network - CLI", long_about = None)]
struct Cli {
...
async fn main() -> Result<()> {
    ...
    match cli.command {
        Commands::Serve { host, port } => {
            ...
            let config = a3net_model_catalog::server::ServerConfig::new(&host, port)
                .with_catalog_path(&cli.catalog)
                .with_download_dir(&cli.download_dir);

            a3net_model_catalog::server::start(config).await?;
        }
            ...
            let filtered: Vec<_> = reps
                ...
                .filter(|r| {
                    if blocked {
                        matches!(
                            r.trust_flag,
                            a3net_model_catalog::TrustFlag::Blocked
                        ) || r.trust_tier() == a3net_model_catalog::TrustTier::Blocked
                    } else if trusted {
                        matches!(
                            r.trust_flag,
                            a3net_model_catalog::TrustFlag::Trusted
                        ) || r.trust_tier() == a3net_model_catalog::TrustTier::Trusted
                    } else {
                        true
                    }
                })
        ...
        Commands::ProviderFlag { node_id, flag } => {
            let parsed = match flag.as_str() {
                "trusted" => a3net_model_catalog::TrustFlag::Trusted,
                "blocked" => a3net_model_catalog::TrustFlag::Blocked,
                "neutral" => a3net_model_catalog::TrustFlag::Neutral,
                other => anyhow::bail!(
            ...
            let mut rep = catalog
                ...
                .await?
                .unwrap_or_else(|| {
                    a3net_model_catalog::ProviderReputation {
                        ...
                        trust_flag: a3net_model_catalog::TrustFlag::Neutral,
                    }
                });
        ...
        Commands::ReportProvider { node_id, reason, detail } => {
            let parsed = match reason.as_str() {
                "spam" => a3net_model_catalog::ReportReason::Spam,
                "harassment" => a3net_model_catalog::ReportReason::Harassment,
                "impersonation" => a3net_model_catalog::ReportReason::Impersonation,
                "phishing" => a3net_model_catalog::ReportReason::Phishing,
                "misleading_metadata" => {
                    a3net_model_catalog::ReportReason::MisleadingMetadata
                }
                "license_violation" => {
                    a3net_model_catalog::ReportReason::LicenseViolation
                }
                "malicious_content" => {
                    a3net_model_catalog::ReportReason::MaliciousContent
                }
                "other" => a3net_model_catalog::ReportReason::Other,
                other => anyhow::bail!(
            ...
            let tracker = a3net_model_catalog::ProviderReputationTracker::new();
...
fn print_stats(stats: &CatalogStats) {
    println!("{}", "═".repeat(50));
    println!("  A3Net Model Catalog Statistics");
    println!("{}", "═".repeat(50));
    println!();
...
mod tests {
    use super::*;
    use a3net_model_catalog::Quantization;
    use clap::Parser;
    ...
    fn cli_rejects_unknown_subcommand() {
        let r = Cli::try_parse_from(["a3net-model-catalog", "bogus"]);
        assert!(r.is_err());
    }

    #[test]
    fn cli_uses_default_catalog_path() {
        let cli = Cli::try_parse_from(["a3net-model-catalog", "stats"]).expect("parse");
