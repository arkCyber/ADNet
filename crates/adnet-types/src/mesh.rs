...
//! `adnet_types::NodeId`). Keeping the byte length and shape
...
pub const MESH_INVITE_SCHEME: &str = "adnet-invite://";
/// Canonical URL scheme for network identifiers.
pub const MESH_NETWORK_SCHEME: &str = "adnet-mesh://";
...
impl MeshNetworkId {
    ...
    /// Render the canonical URL form (`adnet-mesh://<hex>`).
...
/// `adnet-invite://<network_id_hex>:<code>:<unix_expires_at>`
///
...
mod tests {
    ...
    fn mesh_network_id_parse_url_rejects_other_scheme() {
        assert!(MeshNetworkId::parse_url("adnet-peer://abcdef").is_err());
    ...
    fn invite_code_legacy_url_is_rejected() {
        ...
        let nid = MeshNetworkId::from_bytes(&[0u8; 32]).unwrap();
        let legacy = format!("adnet-invite://{}:abcd1234", nid.as_hex());
    ...
    fn invite_code_parse_url_rejects_wrong_shape() {
        assert!(InviteCode::parse_url("adnet-invite://").is_err());
        assert!(InviteCode::parse_url("adnet-invite://nocolon").is_err());
