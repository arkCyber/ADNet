...
//!
//! We carry this in `adnet-types` as a pure 20-byte value so the protocol
...
//! re-export `adnet_identity::Address`** — that would force every consumer
//! of `adnet-types` to compile `secp256k1`, `sha3`, `aes-gcm`, and
//! `x25519-dalek`, which contradicts `adnet-types`' "no crypto deps"
//! invariant.
//!
//! Conversion to `adnet_identity::Address` (and back) is provided by the
//! `adnet-identity` crate. The two types are intentionally distinct so
