...
//! [`adnet-gossip`] topics, [`adnet-ipc`] JSON-RPC, or HTTP webhook
...
/// `adnet-types` implements this so the IPC layer can run a uniform
...
/// in the local adnet-blob store; `thumbnail_hash` (when present) is a
