...
> QR-code generator, scanner, and chatmail-compatible payload parser for A3Net.
> Encodes `mailto:`, `MATMSG:`, `VCARD`, `DCACCOUNT`, `DCLOGIN`, `DCBACKUP`,
> `OPENPGP4FPR:`, `a3net-pairing://`, and ad-hoc text — and renders them as
> Delta-Chat-compatible SVG via `qrcodegen`.

## 概览 (Overview)

`a3net-qr` is the on-the-wire QR layer for A3Net. It sits in two places:
...
2. **A3Net-specific payloads.** Adds `a3net-peer://`, `a3net-blob://`,
   `a3net-signed-peer://`, `a3net-token://`, and `a3net-pairing://` URLs
   so A3Net nodes can share peer tickets, blob tickets, signed peer
...
- A3Net-native payload variants: `a3net-peer://`, `a3net-addr://`,
  `a3net-blob://`, `a3net-signed-peer://`, `a3net-token://`,
  `a3net-pairing://`.
...
|--------------|---------|------------------------------------------------------------|
| `a3net-types`| ✅      | enables `a3net-peer://`, `a3net-blob://`, `a3net-signed-peer://` |
| `a3net-token`| ✅      | enables `a3net-token://` (relay-payment pledges)            |
| `pairing`    | ✅      | enables `a3net-pairing://` envelopes                       |
| `mail`       | ❌      | adds `DCLOGIN → a3net_mail::Account` conversion            |
...
```rust
use a3net_qr::{check_qr, QrPayload};
...
let svg = generator::create_qr_svg("https://a3net.example/invite/abc").unwrap();
...
```rust
use a3net_qr::{check_qr, scan::encode_qr};
...
```rust
use a3net_qr::generator::{create_qr_svg_with_style, QrStyle, QrErrorCorrectionLevel};
...
3. **Render and parse `a3net-pairing://` invitations** — combine
   `a3net-pairing::SignedInvitation` with `a3net-pairing::wire::PairingInvitation::to_url`
   and `a3net_qr::generator::create_qr_svg`. The resulting SVG travels
   ...
   `a3net-invite` crate's `complete_invite_workflow` example.
