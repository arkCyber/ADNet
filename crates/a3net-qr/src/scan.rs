...
//!
//! This mirrors `chatmail@core::qr::check_qr` but with three A3Net
//! additions:
//!  1. We classify `a3net-…` URLs first (peer / blob / signed / token),
...
pub fn check_qr_with_limits(raw: &str, limits: &ParseLimits) -> Result<QrPayload> {
    ...
    // A3Net-native URIs get first crack at the input — their prefixes
    // are unique and never collide with chatmail schemes.
    #[cfg(feature = "a3net-types")]
    if let Some(payload) = a3net::try_parse_a3net_ticket(trimmed)? {
        return Ok(payload);
    }
    #[cfg(feature = "pairing")]
    if let Some(payload) = a3net::try_parse_a3net_pairing(trimmed)? {
        return Ok(payload);
    }
    #[cfg(feature = "a3net-token")]
    if let Some(payload) = a3net::try_parse_a3net_token(trimmed)? {
        return Ok(payload);
    }
...
pub fn encode_qr(payload: &QrPayload) -> Result<String> {
    match payload {
        ...
        #[cfg(feature = "a3net-types")]
        QrPayload::AdnetPeer { ticket } => Ok(a3net_types::PeerTicket::encode(
            &ticket.node_id,
            &ticket.endpoint,
        )),
        #[cfg(feature = "a3net-types")]
        QrPayload::AdnetAddr { ticket } => Ok(a3net_types::NodeAddrTicket::encode(&ticket.0)),
        #[cfg(feature = "a3net-types")]
        ...
        QrPayload::AdnetPairing { invitation } => match invitation {
            a3net_pairing::wire::PairingInvitation::Url(encoded) => {
                ...
                if !encoded.starts_with("a3net-pairing://") {
                    Ok(format!("a3net-pairing://{encoded}"))
                } else {
                    Ok(encoded.clone())
                }
            }
            a3net_pairing::wire::PairingInvitation::Json(inv) => {
                a3net_pairing::wire::PairingInvitation::to_url(inv).map_err(|e| {
