...
//! [`a3net_types::group_chat`] record — callers no longer have to
...
impl Default for GroupChatIpcConfig {
    fn default() -> Self {
        Self {
            socket_path: std::env::temp_dir().join("a3net_group_chat.sock"),
...
// `a3net_types::group_chat`, so the gate can call them via the
...
pub use a3net_types::group_chat::attachment_from_hash as group_attachment_from_hash;
/// Re-export of [`a3net_types::group_chat::next_group_message_id`] and
/// [`a3net_types::group_chat::attachment_from_hash`] for callers that
/// want to build messages on this side of the wire without going
/// through `a3net-types` again.
pub use a3net_types::group_chat::next_group_message_id as message_id_for;
pub use a3net_types::{AttachmentKind, MessageType, attachment_from_hash_str};
...
pub fn attachment_with_hash_strict(
    ...
    file_name: impl Into<String>,
    file_size: u64,
) -> a3net_types::Result<MessageAttachment> {
...
/// `a3net_types::group_chat::MessageReceipt` separately.
...
mod tests {
    ...
    async fn strict_rejects_oversize_content() {
        let (_dir, svc, _sock) = new_svc_with_policy(ValidationPolicy::Strict);
        let id = svc.create_group(sample_group("alice")).unwrap();
        let big = "x".repeat(a3net_types::MAX_CONTENT_LEN + 1);
        let bad = GroupMessage {
            message_id: "m-bad".into(),
            group_id: id.clone(),
            sender_id: "alice".into(),
    ...
    async fn strict_rejects_sequence_overflow() {
        ...
        let bad = GroupMessage {
            ...
            is_edited: false,
            edited_at: None,
            sequence: a3net_types::MAX_SEQUENCE,
            integrity_hash: None,
        };
    ...
    async fn strict_rejects_member_temporal_inversion() {
        ...
        let bad = GroupMember {
            agent_id: "bob".into(),
            agent_name: "Bob".into(),
            role: a3net_types::MemberRole::Member,
            joined_at: 200,
            last_seen: 100,
            is_online: false,
            nickname: None,
