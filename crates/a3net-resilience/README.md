...
> A3Net 通用弹性层 — `RetryPolicy` / `CircuitBreaker` / `CancellationScope` / `ResourceLimiter` / `ResilientHttpClient`,可被 mesh、ipc、relay 等多个 crate 共用,不依赖 A3Net 业务类型。

## 概览 (Overview)

`a3net-resilience` 是 A3Net 中所有"对外调用"都会用到的弹性组件,设计目标是 **transport-agnostic** — 不绑定任何 A3Net 业务类型,可以被任意子模块直接复用:
...
- **零 P2P 依赖** — 仅依赖 `tokio`、`reqwest`、`tracing`、`parking_lot`,无 A3Net 类型,无循环依赖风险。

## Cancellation (P1-3)

P1-3 落地后,`a3net-node::Node` 自带一个 `Arc<CancellationScope>`,在 `Node::shutdown()` 时:
...
```rust
use a3net_resilience::{CancellationScope, CancellationToken};
...
- Cancellation across process boundaries — 这属于 iroh-style 的 wire cancel,见 `a3net-transport` 后续规划。

## ResourceLimiter (P1-5)

P1-5 落地后,`a3net-node::Node` 自带三个 limiter:
...
```rust
use a3net_resilience::{AcquireError, ResourceLimiter};
use std::time::Duration;
...
```rust
use a3net_resilience::{retry_with_backoff, RetryPolicy};
...
```rust
use std::sync::Arc;
use a3net_resilience::{CircuitBreaker, CircuitBreakerConfig, CircuitState};
...
- **`a3net-mesh`** — 所有"对远端节点"的调用都通过 `ResilientHttpClient`,把瞬时网络抖动吸收在 mesh 层,不污染业务。
- **`a3net-ipc`** — Unix socket 调用容易瞬时失败(对端重启 / 升级),`retry_with_backoff` 让客户端自动重连。
- **`a3net-relay`** — relay 拉取上游 CDN / IPFS 网关时,熔断器在多个 gateway 同时宕机时快速失败,避免长尾延迟。
- **DHT 查询** — `a3net-dht` 用 `retry_with_backoff` 包裹对 bootstrap 节点的 ping,网络抖动不再触发报警。
- **混沌工程** — `a3net-chaos` 的故障场景直接复用同一套 `RetryPolicy` / `CircuitBreaker` 配置来对比 baseline / chaos 行为。
