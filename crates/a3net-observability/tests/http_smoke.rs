...
use a3net_observability::http::{MetricsServer, MetricsServerConfig, serve};
...
async fn metrics_endpoint_returns_prometheus_text() {
    ...
    let counter = registry.register_counter("a3net_smoke_total", "smoke");
    counter.inc_by(11);
    ...
    assert!(
        body.contains("# HELP a3net_smoke_total smoke"),
        "missing HELP line: {body}"
    );
    assert!(body.contains("a3net_smoke_total 11"));
    assert!(body.contains(r#"a3net_smoke_total{topic="lobby"} 1"#));
...
/// through `/metrics` and an operator running `a3net metrics`
...
async fn default_registry_serves_global() {
    ...
    let counter = a3net_observability::registry::GLOBAL
