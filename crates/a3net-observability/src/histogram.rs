...
//! pre-defined buckets.
//!
//! A3Net's histogram is intentionally **simple**:
//!
...
//! at construction or when memory is at a premium. A3Net's
...
pub struct Histogram {
    ...
    /// than storing it per-inner because every A3Net histogram
...
mod tests {
    ...
    fn render_prometheus_includes_help_type_buckets_count_sum() {
        let h = Histogram::new("a3net_test_seconds", "test histogram");
        h.observe(0.05);
        h.observe(5.0);
        let out = h.render_prometheus();
        assert!(out.contains("# HELP a3net_test_seconds test histogram"));
        assert!(out.contains("# TYPE a3net_test_seconds histogram"));
        // Bucket lines (cumulative).
        assert!(out.contains(r#"a3net_test_seconds_bucket{le="0.1"} 1"#));
        assert!(out.contains(r#"a3net_test_seconds_bucket{le="5"} 2"#));
        assert!(out.contains(r#"a3net_test_seconds_bucket{le="+Inf"} 2"#));
        assert!(out.contains("a3net_test_seconds_count 2"));
        assert!(out.contains("a3net_test_seconds_sum 5.05"));
    }
