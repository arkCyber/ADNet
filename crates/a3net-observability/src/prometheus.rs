...
//! OpenMetrics). All A3Net metrics are exposed in 0.0.4
...
mod tests {
    ...
    fn render_counter_emits_help_type_and_value() {
        let reg = Registry::default();
        let c = reg.register_counter("a3net_test_total", "test help");
        c.inc_by(7);
        let out = PrometheusExporter::new(&reg).render();
        let text = out.text();
        assert!(text.contains("# HELP a3net_test_total test help"));
        assert!(text.contains("# TYPE a3net_test_total counter"));
        assert!(text.contains("a3net_test_total 7"));
    }
