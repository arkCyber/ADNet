...
> A3Net mesh VPN 的 Magic DNS：把 `<hostname>.<network>`（如 `alice.gaming.ray`）映射成 mesh 成员的确定性虚拟 IP。 / Magic DNS — name resolution for the A3Net mesh VPN.

## 概览 (Overview)

`a3net-magicdns` 把 mesh DNS 名字解析这件事做成了三层：

1. **纯数据 [`Resolver`]** —— 输入 `(network, hostname)`，输出 [`VirtualIp`]。无网络、
   无锁竞争，热路径就是 `HashMap` 查询。
2. **TUN 拦截 [`TunDnsForwarder`]** —— 在 `a3net-tun` 数据通路里截 UDP/53 的 DNS 包，
   命中 `.ray` / `.a3net` 后构造响应注入回 TUN。
...
`.ray` 与 `.a3net` 是默认 TLD；`.mesh`、`.vpn` 等可附加在 `ResolverConfig::extra_tlds` 中。
...
```rust
use a3net_magicdns::{Resolver, ResolverConfig, MagicName, MagicQuery, TunDnsForwarder, DnsServer};
use a3net_types::{MeshMembership, MeshNetworkId};
```
...
```rust,no_run
use a3net_magicdns::Resolver;
# use a3net_magicdns::ResolverConfig;
# let resolver = Resolver::new(ResolverConfig::default());
...
```rust,no_run
use a3net_magicdns::TunDnsForwarder;
...
- **`a3net-cli`** 用 `DnsServer` 让 OS resolver 能直接 `dig alice.gaming.ray`。
- **`a3net-tun`** 在 packet loop 中调用 `TunDnsForwarder::maybe_handle`，命中后注入响应。
- **`a3net-mesh-coordinator`** 收到 `MeshMembership` 后调用 `Resolver::apply_roster`，
...
handshake. (See [`a3net-types::VirtualIp`] for the derivation.)
...
```rust,no_run
use a3net_magicdns::{Resolver, ResolverConfig, MagicName};
use a3net_types::{MeshMembership, MeshNetworkId};
...
loop {
    ...
    let parsed = a3net_tun::packet::parse_packet(&pkt)?;
...
| `extra_tlds`    | `[]`                     | Additional mesh TLDs (e.g. `mesh`, `vpn`). `.ray` and `.a3net` are always valid. |
...
let server = a3net_magicdns::DnsServer::new(resolver, config);
...
- `.ray` / `.a3net` (and any configured `extra_tlds`) — answered locally via the resolver. A / AAAA records with the deterministic mesh virtual IP.
...
- `ResolverConfig` builder, TLD matching (case-insensitive, ray/a3net/extra), serde roundtrip

End-to-end smoke test:

```bash
cargo test -p a3net-tun --test end_to_end
```
...
`a3net-magicdns` and the rest of the stack.

## Demo

```bash
cargo run -p a3net-magicdns --example magicdns_demo
```
...
### Why two built-in TLDs (`.ray` and `.a3net`)?

The legacy wire format uses `.ray`. Operators who want a more
mnemonic TLD can add `.a3net` to their config; it is built-in
