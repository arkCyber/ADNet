...
//! `adnet_blobstore::BlobStore` in addition to the in-memory cache — the
...
impl Default for BlobsIpcConfig {
    fn default() -> Self {
        ...
        socket_path.push("adnet_blobs.sock");
        Self {
            socket_path,
            data_dir: None,
...
/// Implement the shared `adnet_types::Validate` trait so the IPC gate
/// can be used uniformly.
impl adnet_types::Validate for HashedBlob {
    fn validate(&self) -> adnet_types::Result<()> {
...
pub fn validate_blob_identity(b: &HashedBlob) -> adnet_types::Result<()> {
    if b.hash.len() != adnet_types::ContentHash::HEX_LEN {
        return Err(adnet_types::AdnetError::Validation(format!(
            "blob hash: expected {} hex chars, got {}",
            adnet_types::ContentHash::HEX_LEN,
            b.hash.len()
        )));
    }
    if !b.hash.chars().all(|c| c.is_ascii_hexdigit()) {
        return Err(adnet_types::AdnetError::Validation(
        ...
        if let Ok(Some(store)) = self.disk_store().await {
            ...
            if let Ok(read) = std::fs::read_dir(store.data_dir()) {
                for entry in read.flatten() {
                    ...
                    if adnet_types::ContentHash::from_hex(name).is_ok()
