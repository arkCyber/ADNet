...
> ADNet 的本地进程间 JSON-RPC 通道 — `JsonRpcServer` + `json_rpc_call`,在 Unix domain socket 上跑标准 JSON-RPC 2.0。

## 概览 (Overview)

`adnet-ipc` 把 ADNet 节点暴露为可在同一台机器上跨进程调用的 JSON-RPC 服务:
...
- **类型化封装** — 在裸 JSON-RPC 之上,`blobs_service` / `gossip_service` / `group_chat_service` 提供了直接面向 ADNet 业务的服务包装。
...
- **类型化 IPC service** — `BlobsIpcService` / `GossipIpcService` / `GroupChatIpcService` 直接面向 `adnet-types` 暴露业务方法,无需手写 RPC 方法名。
...
use adnet_ipc::{JsonRpcServer, RpcHandler};
...
impl RpcHandler for Adder {
    async fn handle(&self, method: &str, params: Value) -> Result<Value, adnet_ipc::RpcHandlerError> {
        match method {
            "add" => {
            ...
            "echo" => Ok(params),
            other => Err(adnet_ipc::RpcHandlerError::method_not_found(other)),
        }
    }
}
...
#[tokio::main]
async fn main() {
    let sock = PathBuf::from("/tmp/adnet-adder.sock");
...
```rust
use adnet_ipc::{json_rpc_call, JsonRpcError};
use serde_json::json;
...
```rust
use adnet_ipc::{JsonRpcServer, Notification};
use serde_json::json;
...
```rust
use adnet_ipc::{Validate, ValidationPolicy, ValidationOutcome};
...
- **CLI ↔ Node IPC** — `adnet-cli` 通过 `json_rpc_call` 与 `adnet-node` 守护进程通信,调用 RPC 方法名以 `/api/v0` 开头,与 HTTP gateway 完全对齐。
- **TUI 前端** — `adnet-tui` 在用户切换标签页时订阅 `Notification`,实时显示 gossip / bitswap 状态。
- **CI / 集成测试** — `adnet-integration-tests` 在同一进程里启动一个 `JsonRpcServer` 再用客户端连过去,跑端到端场景。
- **多语言客户端** — JSON-RPC 协议天然跨语言,Python / Go 客户端只需打开 Unix socket 即可使用 ADNet 节点。
