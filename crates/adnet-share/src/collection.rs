...
//! ## Wire format
//!
//! ADNet's `Collection` is wire-compatible with
...
//! `adnet_types::ContentHash` (also 32-byte BLAKE3) instead of
//! `iroh_blobs::Hash`.
//!
...
//! send us a `Collection` and we'll load it back into ADNet-native
...
///
/// sendme does not enforce an explicit cap; ADNet picks 16,384 to bound
