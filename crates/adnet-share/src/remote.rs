...
//! (the `network_acceptance` example in `adnet-node`); unit tests
...
pub async fn discover_endpoints(
    node_id: &adnet_types::NodeId,
) -> ShareResult<(adnet_types::NodeAddr, Endpoint)> {
    use iroh::Endpoint;
    ...
    let mut addr = adnet_types::NodeAddr::new(node_id.clone());
    ...
    for socket in endpoint.addr().ip_addrs() {
        if !socket.ip().is_loopback() {
            let ep = adnet_types::node::Endpoint::from_socket_addr(*socket);
            addr.direct = Some(ep);
            break;
        }
    }
    ...
    for relay_url in endpoint.addr().relay_urls() {
        addr.relay = Some(adnet_types::RelayUrl::new(relay_url.to_string()));
        break;
    }
