//! Persistent resume state for `adnet share receive`.
//!
...
//!    that survives across `adnet` invocations and across machines
...
//! 3. A list/clean API (`adnet share resume ls` / `clean`) so the
...
pub struct ResumeState {
    ...
    /// `adnet share resume continue {hash_short}` to resume without
    /// re-typing the URL.
    pub ticket: String,
...
mod tests {
    ...
    fn ticket() -> ShareTicket {
        ...
        let addr = adnet_types::NodeAddr::new(id.clone())
