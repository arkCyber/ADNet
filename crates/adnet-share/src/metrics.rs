...
//! drives, and `adnet-cli::share` is free to ignore it).
//!
//! ## Metric names
//!
//! All names are prefixed with `adnet_share_receive_*` so the
...
//! follows `DEFAULT_BUCKETS` from `adnet-observability` (1 ms
...
use adnet_observability::registry::{
...
pub struct ShareMetrics {
    /// `adnet_share_receive_bytes_total` — cumulative
    ...
    /// `adnet_share_receive_bytes_done` — cumulative
    ...
    /// `adnet_share_receive_files_total` — cumulative
    /// manifest entry count across every receive.
    pub receive_files_total: Arc<Counter>,
    /// `adnet_share_receive_files_done` — cumulative
    /// number of files successfully written.
    pub receive_files_done: Arc<Counter>,
    /// `adnet_share_receive_errors_total` — every error
    ...
    /// `adnet_share_receive_seconds` — wall-clock duration
    ...
    pub fn register(registry: &Registry) -> Self {
        Self {
            receive_bytes_total: registry.register_counter(
                "adnet_share_receive_bytes_total",
            ...
            receive_bytes_done: registry.register_counter(
                "adnet_share_receive_bytes_done",
...
/// function so callers can `use adnet_share::share_metrics;`
...
mod tests {
    ...
    fn register_creates_all_metrics() {
        ...
        assert!(names.contains(&"adnet_share_receive_bytes_total".to_string()));
        assert!(names.contains(&"adnet_share_receive_bytes_done".to_string()));
        assert!(names.contains(&"adnet_share_receive_files_total".to_string()));
        assert!(names.contains(&"adnet_share_receive_files_done".to_string()));
        assert!(names.contains(&"adnet_share_receive_errors_total".to_string()));
        assert!(names.contains(&"adnet_share_receive_seconds".to_string()));
