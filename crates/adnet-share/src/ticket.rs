...
//! blob that an `iroh-blobs` peer (or a pure-ADNet peer) can use to
//! pull bytes.
//!
//! ## Wire format
//!
//! ```text
//! adnet-share://<node-id-hex>@<addr-display>/<manifest-hash-hex>[/name1=hash1,name2=hash2,…]
//! ```
//!
//! - `<node-id-hex>` — 64 lowercase hex chars (32-byte BLAKE3 digest),
//!   the same shape as `adnet-blob://` tickets in `adnet-types`.
//! - `<addr-display>` — `direct=<host>:<port>` and/or
//!   `relay=<relay-url>`, matching the `NodeAddr::display_with_id`
//!   convention used elsewhere in ADNet.
...
/// literal. We pick `adnet-share://` (and NOT `adnet-blob://`) so a
/// receiver knows up-front whether it's pulling a single blob or a
/// manifest, even before parsing.
pub const SHARE_TICKET_PREFIX: &str = "adnet-share://";
...
/// `adnet_blob_ticket`-style URL budgets used elsewhere in ADNet.
...
mod tests {
    ...
    fn parse_rejects_wrong_prefix() {
        assert!(ShareTicket::parse("adnet-blob://x/y").is_err());
