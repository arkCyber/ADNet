...
use a3net_media::persist::{AliasMap, MediaStore};
...
fn ingest_clip() -> a3net_media::ingest::IngestReport {
...
fn alias_unknown_rejected() {
    ...
    assert!(matches!(err, a3net_media::error::MediaError::InvalidConfig(_)));
}
...
fn verify_complete_fails_if_segment_missing() {
    ...
    assert!(matches!(err, a3net_media::error::MediaError::Quarantined { .. }));
}
...
fn tampered_segment_rejected_at_persist_time() {
    ...
    assert!(matches!(
        err,
        a3net_media::error::MediaError::ManifestHashMismatch { .. }
    ));
}
...
fn sr_10_path_traversal_rejected() {
    ...
    for b in bad {
        ...
        assert!(
            matches!(err, a3net_media::error::MediaError::InvalidConfig(_)),
...
fn collect_segments_for_test(
    report: &a3net_media::ingest::IngestReport,
