...
//!
//! ```sh
//! cargo test -p a3net-media --features aerospace --test aerospace_compliance
//! ```
//!
//! Every test maps to a Safety Requirement (SR-1 .. SR-9) in
//! `crates/a3net-media/SAFETY_CASE.md`.

#![cfg(feature = "aerospace")]

use a3net_media::codec::{AudioCodec, SampleFormat, VideoCodec};
use a3net_media::config::{MediaConfig, VariantSpec};
use a3net_media::error::MediaError;
use a3net_media::ingest::MediaIngester;
use a3net_media::integrity::{decode_lp, LP_AUDIO, LP_VIDEO, SegmentDigest};
use a3net_media::segment::{SegmentKind, Segmenter};
use a3net_media::transcode::{Frame, PureTranscoder, TranscodeInput, Transcoder};
use a3net_media::verify::{verify_dag, verify_manifest, VerifyStatus};
use a3net_media::*;
...
fn ingest_one() -> a3net_media::ingest::IngestReport {
...
fn sr_2_manifest_round_trip() {
    ...
    let back: a3net_media::manifest::MediaManifest = bincode::deserialize(&bytes).unwrap();
...
fn sr_9_oversized_payload_rejected() {
    ...
    let huge = 48_000u64 * 2 * 2 * (a3net_media::config::MAX_MEDIA_BYTES / 96_000 + 4);
...
//
// Persisting a MediaDag to `a3net_blobstore::BlobStore` MUST
...
fn open_store() -> (tempfile::TempDir, a3net_media::persist::MediaStore) {
    let dir = tempfile::tempdir().unwrap();
    let bs = BlobStore::new(dir.path()).unwrap();
    let store = a3net_media::persist::MediaStore::open(bs).unwrap();
    (dir, store)
}
...
fn sr_10_load_segment_rejects_invalid_hex() {
    ...
    for bad in ["", "abc", &"A".repeat(64), &"g".repeat(64)] {
        let res = store.load_segment_with_kind(bad, a3net_media::integrity::LP_VIDEO);
        assert!(matches!(res, Err(MediaError::InvalidConfig(_))), "input {bad:?}");
    }
}
...
fn sr_10_load_segment_rejects_wrong_kind() {
    ...
    let res = store.load_segment_with_kind(
        &first.digest.as_hex(),
        a3net_media::integrity::LP_AUDIO,
    );
    assert!(
...
fn sr_10_load_segment_rejects_tampered_index() {
    ...
    let mut parsed: std::collections::BTreeMap<
        String,
        a3net_media::persist::SegmentIndexEntry,
...
fn sr_10_load_by_alias_rejects_tampered_root() {
    use a3net_media::persist::AliasMap;
    let (_dir, store) = open_store();
    let report = ingest_one();
    let (v, a) = collect_segments(&report);
    store
...
fn sr_10_persist_rejects_oversized_segment() {
    ...
    v[0] = vec![0u8; a3net_media::persist::MAX_SEGMENT_BYTES + 1];
...
fn sr_10_verify_complete_rejects_wrong_lp_kind_in_index() {
    ...
    parsed.insert(
        ...
        a3net_media::persist::SegmentIndexEntry {
            ...
            kind: a3net_media::integrity::LP_AUDIO, // wrong!
        },
    );
    std::fs::write(
        &index_path,
