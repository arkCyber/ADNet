...
//! Run with:
//! ```bash
//! cargo run -p a3net-cli --example config_validator
//! ```

use a3net_cli::config::{self, AppConfig};
use anyhow::Result;
use std::fs;

fn main() -> Result<()> {
    println!("=== A3Net Config Validator Demo ===\n");
...
fn create_modified_config() -> Result<AppConfig> {
    Ok(AppConfig {
        ...
        log: a3net_cli::config::LogConfig {
            level: "info".to_string(),
            format: a3net_cli::config::LogFormat::Json,
        },
        default_room: None,
        repl: a3net_cli::config::ReplConfig::default(),
        mesh: None,
        relay: None,
...
fn temp_path(name: &str) -> Result<std::path::PathBuf> {
    ...
    path.push(format!("a3net-validator-demo-{}", name));
    Ok(path)
}
