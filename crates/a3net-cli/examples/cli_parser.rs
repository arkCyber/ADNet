...
use a3net_cli::{Cli, Cmd, ConfigCmd, IrohOpt};
use anyhow::Result;
use clap::Parser;

fn main() -> Result<()> {
    println!("=== A3Net CLI Parser Demo ===\n");

    // 1. Basic init command
    println!("1. Parsing 'a3net init':");
    ...
    // 2. Echo with room
    println!("\n2. Parsing 'a3net echo --room lobby':");
    let cli = Cli::try_parse_from(["a3net", "echo", "--room", "my-room"])?;
    ...
    println!("\n3. Parsing 'a3net announce' with all options:");
    ...
    // ConfigCmd::Path
    let cli = Cli::try_parse_from(["a3net", "config", "path"])?;
    ...
    // ConfigCmd::Show
    let cli = Cli::try_parse_from(["a3net", "config", "show"])?;
    match &cli.cmd {
        Cmd::Config { sub } => match sub {
    ...
    // ConfigCmd::Validate
    let cli = Cli::try_parse_from(["a3net", "config", "validate"])?;
    match &cli.cmd {
        Cmd::Config { sub } => match sub {
    ...
    // ConfigCmd::Edit
    let cli = Cli::try_parse_from(["a3net", "config", "edit"])?;
    match &cli.cmd {
        Cmd::Config { sub } => match sub {
    ...
    let cli = Cli::try_parse_from(["a3net", "config", "reset", "--yes"])?;
    match &cli.cmd {
        Cmd::Config { sub } => match sub {
    ...
    // ConfigCmd::Set
    let cli = Cli::try_parse_from(["a3net", "config", "set", "log.level", "debug"])?;
    match &cli.cmd {
        Cmd::Config { sub } => match sub {
    ...
    let cli = Cli::try_parse_from(["a3net", "diagnostics", "--json"])?;
    ...
    for (input, expected) in cases {
        let result = a3net_cli::cli::parse_iroh_opt(input)
