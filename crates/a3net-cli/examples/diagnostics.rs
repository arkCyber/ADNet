...
//! Run with:
//! ```bash
//! cargo run -p a3net-cli --example diagnostics
//! ```

use a3net_cli::diagnostics::{self, DiagnosticsSnapshot};
use anyhow::Result;
use std::fs;

fn main() -> Result<()> {
    println!("=== A3Net Diagnostics Demo ===\n");
    ...
    println!("\n4. Error handling:");
    let missing_dir = std::env::temp_dir().join("nonexistent-a3net-dir-999");
    ...
    println!("   Short ID:    a3net-{}", snap.node_id_short);
...
fn print_snapshot(snap: &DiagnosticsSnapshot) {
    ...
    println!("   Short:       a3net-{}", snap.node_id_short);
    println!("   Public Key: {}", snap.public_key);
    println!("   Data Dir:   {}", snap.data_dir);
    match &snap.mesh_url {
...
fn temp_dir() -> Result<std::path::PathBuf> {
    ...
    path.push(format!("a3net-diag-demo-{}", std::process::id()));
