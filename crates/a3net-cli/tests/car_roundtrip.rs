//! End-to-end integration tests for `a3net ipfs car …`.
//!
//! Drives the public `a3net_cli::ipfs_exec::run_car_command`
...
use a3net_cli::ipfs::{CarCmd, IpfsCmd};
...
fn run_sync(cmd: &IpfsCmd, data_dir: &Path) -> Result<()> {
    ...
    rt.block_on(async {
        ...
        a3net_cli::ipfs_exec::run_ipfs_command(cmd, &store, Some(data_dir)).await
    })
}
...
fn write_car_one_block(
    ...
    use a3net_blobstore::car::{write_car, CarBlock, CarHeader};
...
/// `a3net ipfs car import <file>` ingests every block in the
...
/// `a3net ipfs car export <cid> --out <file>` writes a CAR file
...
fn car_export_writes_a_valid_archive() -> Result<()> {
    ...
    let (header, blocks) = a3net_blobstore::car::read_car(&mut cursor)?;
...
fn car_import_handles_archive_with_no_roots() -> Result<()> {
    ...
    {
        use a3net_blobstore::car::{write_car, CarHeader};
