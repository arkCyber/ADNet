//! `a3net status` / `a3net storage …` subcommands.
//!
//! Backed by `a3net_blobstore::StorageTopology` and the
//! `a3net_observability::dashboard` JSON surface. Every
...
//! Two main surfaces:
//!
//! - `a3net status [--json]` — operator-friendly view of
//!   the running node (storage + replication + alerts).
//! - `a3net storage <sub>` — explicit scope management:
...
/// edit (`a3net storage quota --set 5GiB`) survives a config-file
...
pub fn run_status(data_dir: &Path, json: bool) -> Result<()> {
    ...
    if json {
        ...
        println!("A3Net status — data_dir: {}", data_dir.display());
        println!(
...
fn run_info(topo: &StorageTopology) -> Result<()> {
    ...
    println!("A3Net storage topology @ {}", topo.data_dir.display());
    println!(
...
/// `a3net storage quota` — print the effective policy, or apply a
...
fn run_quota(
    ...
    if changed {
        ...
        if !json {
            println!(
                "a3net: persisted new quota policy to {}/topology.json",
                data_dir.display()
            );
        }
    }
...
fn run_reset(
    ...
    if matches!(scope, BlobStoreScope::Shared) && !i_know && !dry_run {
        anyhow::bail!(
            "refusing to wipe the sealed shared scope without \
             ...
             by the replication protocol. Re-import via `a3net share \
    ...
    match scope {
        BlobStoreScope::Private => {
            ...
            if !yes {
                eprintln!(
                    "a3net: about to delete {} blobs ({} bytes) from {scope}",
                    hashes.len(),
                    total
                );
        ...
        BlobStoreScope::Shared => {
            ...
            if !yes {
                eprintln!(
                    "a3net: about to wipe {} blobs from the SEALED shared scope.",
                    hashes.len()
                );
...
pub fn build_dashboard(
    ...
    factor: u8,
    sweep: u64,
) -> a3net_observability::dashboard::Dashboard {
    use a3net_observability::dashboard::{
...
pub fn run_encrypt_init(
    ...
    use a3net_blobstore::{EncryptionError, KeyStore};
...
mod tests {
    ...
    fn encrypt_init_with_passphrase_persists_kdf_metadata() {
        ...
        assert!(
            matches!(err, a3net_blobstore::EncryptionError::Kdf(_)),
            "got {:?}",
            err
        );
    ...
    /// `a3net storage quota --set 50GiB` round-trips: the topology
    ...
    /// `a3net storage quota --set junk` is rejected with a
