...
//! - `a3net mdns status` — show mDNS discovery status
//! - `a3net mdns peers` — list discovered peers
//! - `a3net mdns health` — run health check
...
/// Run `a3net mdns <sub>` (top-level dispatcher).
...
pub fn run_mdns_status(data_dir: &Path, json: bool) -> Result<()> {
    ...
    let status = MdnsStatus {
        ...
        service_name: "_a3net._udp.local".to_string(),
