//! Shared "human-friendly" projection of [`a3net_node::RoomFeed`].
//!
//! Both the one-shot `a3net feed` subcommand and the interactive
//! `a3net run` REPL need the same flattened JSON, so we keep the
...
pub fn feed_for_humans(feed: &a3net_node::RoomFeed) -> HumanFeed<'_> {
