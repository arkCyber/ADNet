...
//!
//! The `a3net profile` CLI subcommand surfaces this to operators
...
/// Top-level dispatcher for `a3net profile <sub>`. Offline — does not
...
fn load_node_id(data_dir: &Path) -> Result<a3net_types::NodeId> {
    ...
    if hex.len() == 64 && hex.chars().all(|c| c.is_ascii_hexdigit()) {
        return a3net_types::NodeId::from_hex(hex)
...
mod tests {
    ...
    fn profile_snapshot_missing_identity_errors() {
        let dir = std::env::temp_dir().join(format!("a3net-profile-no-id-{}", std::process::id()));
        fs::create_dir_all(&dir).unwrap();
