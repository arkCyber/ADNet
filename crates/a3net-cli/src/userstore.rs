//! `a3net user …` — offline user-profile subcommands.
//!
...
use a3net_userstore::{SqliteUserStore, SqliteUserStoreConfig, UserProfile, UserStore};
...
pub async fn run(cmd: &super::cli::UserCmd, data_dir: &Path) -> Result<()> {
    ...
    match cmd {
        ...
        super::cli::UserCmd::Delete { user_id, yes } => {
            let id = validate_cli_user_id(user_id)?;
            if !yes {
                eprintln!("a3net: about to delete user {id} (cascade clears keys, devices, digit)");
