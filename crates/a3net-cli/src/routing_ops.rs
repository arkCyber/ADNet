//! `a3net` P2P / routing / swarm / dht top-level commands.
//!
//! Equivalent of the Kubo `routing`, `swarm`, and `dht` command
//! families, exposed as first-class top-level commands:
//!
//! - `a3net routing findprovs <cid>` — look up providers via DHT
//! - `a3net routing findpeer <peer-id>` — closest known peers
//!   for a node id (returns routing-table entries)
//! - `a3net dht findpeer <peer-id>` — single-peer multiaddr lookup
//!   (falls back to known_peers + DHT routing-table search)
//! - `a3net dht query <key>` — generic DHT value lookup
//! - `a3net swarm peers` / `connect` / `disconnect` — connection
//!   management (works when the iroh feature is compiled in;
//!   reports a clear error otherwise)
//!
//! Naming convention (audit V6): `a3net routing ...` /
//! `a3net dht ...` / `a3net swarm ...`, mirroring the Kubo UX
//! without the `ipfs` prefix. The old `a3net dht` sub-tree from
...
/// Run `a3net routing <sub>` against a running node.
#[cfg(feature = "dht")]
    ...
    match cmd {
        ...
        RoutingCmd::FindPeer { peer_id, json } => {
            ...
            if let Some(dht) = dht {
                ...
                println!("DHT not initialized. Use `a3net init --dht` to enable.");
            }
            Ok(())
        }
        RoutingCmd::Get { key, json: _ } => {
            ...
            let dht = node.dht().await;
            if let Some(dht) = dht {
                let dht_key = a3net_dht::DhtKey::from_bytes(key.as_bytes().to_vec());
                ...
                bail!("DHT not initialized. Use `a3net init --dht` to enable.");
            }
            Ok(())
        }
        RoutingCmd::Put { key, value, json: _ } => {
            ...
            if let Some(dht) = dht {
                ...
                let now = SystemTime::now()
                    ...
                    .unwrap_or_default()
                    .as_secs();
                let dht_value = a3net_dht::record::DhtValue {
...
/// Run `a3net routing <sub>` against a running node (no DHT).
#[cfg(not(feature = "dht"))]
pub async fn run_routing(
    cmd: &RoutingCmd,
    ...
    match cmd {
        ...
        RoutingCmd::FindPeer { peer_id, json } => {
            ...
            if *json {
                println!("{}", serde_json::to_string_pretty(&Vec::<(u128, a3net_types::NodeId)>::new())?);
            } else {
        ...
        RoutingCmd::Get { key, json: _ } => {
            bail!(
                "a3net routing get requires DHT support. Rebuild with `--features dht`."
            );
        }
        RoutingCmd::Put { key, value, json: _ } => {
            bail!(
                "a3net routing put requires DHT support. Rebuild with `--features dht`."
            );
        }
    }
}

/// Run extra `a3net dht <sub>` commands (findpeer / query / put / get).
///
/// These are additive — the canonical `a3net dht {provide,find,peers,stats,ipns}`
...
/// Run extra `a3net dht <sub>` commands (no DHT feature).
#[cfg(not(feature = "dht"))]
pub async fn run_dht_extra(
    cmd: &DhtExtraCmd,
    _node: &a3net_node::Node,
    ...
    match cmd {
        ...
        DhtExtraCmd::Put { key, value, json: _ } => {
            bail!(
                "a3net dht put requires DHT support. Rebuild with `--features dht`."
            );
        }
        DhtExtraCmd::Get { key, json: _ } => {
            bail!(
                "a3net dht get requires DHT support. Rebuild with `--features dht`."
            );
        }
    }
}
...
/// Run `a3net swarm <sub>`.
#[cfg(feature = "iroh")]
pub async fn run_swarm(cmd: &SwarmCmd, _data_dir: &Path, node: &a3net_node::Node) -> Result<()> {
...
/// Run `a3net swarm <sub>` (no iroh feature).
#[cfg(not(feature = "iroh"))]
pub async fn run_swarm(cmd: &SwarmCmd, _data_dir: &Path, _node: &a3net_node::Node) -> Result<()> {
    ...
    match cmd {
        SwarmCmd::Peers { json } => {
            if *json {
                ...
                bail!(
                    "a3net swarm peers requires iroh transport. \
        ...
        SwarmCmd::Connect { addr } => {
            ...
            bail!(
                "a3net swarm connect requires iroh transport. \
        ...
        SwarmCmd::Disconnect { peer_id } => {
            ...
            bail!(
                "a3net swarm disconnect requires iroh transport. \
                 Rebuild with `--features iroh` to enable connection management."
            );
        }
        SwarmCmd::Addrs { json } => {
            if *json {
                ...
                bail!(
                    "a3net swarm addrs requires iroh transport. \
                     Rebuild with `--features iroh` or use `a3net id` for local identity."
                );
            }
            Ok(())
        }
...
fn parse_node_id(s: &str) -> Result<a3net_types::NodeId> {
    // Accept both `a3net-<short>` and the full BLAKE3 hex form.
    let cleaned = s.trim_start_matches("a3net-").trim_start_matches("/p2p/");
    a3net_types::NodeId::from_hex(cleaned)
        .map_err(|_| anyhow!("invalid PeerID: {s} (expected a3net-<hex> or full hex)"))
}

fn distance(a: &a3net_types::NodeId, b: &a3net_types::NodeId) -> u128 {
...
mod tests {
    ...
    fn distance_is_symmetric_and_zero_for_self() {
        let a = a3net_types::NodeId::from_bytes(&[0u8; 32]).unwrap();
        let b = a3net_types::NodeId::from_bytes(&[1u8; 32]).unwrap();
