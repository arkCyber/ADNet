//! `a3net` application configuration.
//!
//! The CLI reads a JSON5 (`.json` or `.json5`) file from a
//! platform-default location — `~/.config/a3net/config.json` on Linux,
//! `~/Library/Application Support/a3net/config.json` on macOS — and
...
#[cfg(feature = "qr")]
use a3net_qr::generator::QrErrorCorrectionLevel as QrCodeEcc;
...
pub struct AppConfig {
    ...
    /// the same place by default — `./.a3net-data` in the current
    /// working directory.
    ...
    /// Default room/lobby used when the user runs `a3net feed` or
    /// `a3net echo` without `--room`. `None` means "require --room".
    pub default_room: Option<String>,

    /// REPL behaviour (only meaningful under `a3net run`).
    pub repl: ReplConfig,
    ...
    /// `iroh` cargo feature on `a3net-node` must be enabled at build
    ...
    /// `a3net storage` subcommands and the `adbnet-blobstore` topology
...
impl Default for AppConfig {
    fn default() -> Self {
        Self {
            data_dir: PathBuf::from("./.a3net-data"),
...
/// `a3net-blobstore` topology. Every field is optional so a
...
pub struct LogConfig {
    ...
    /// as the `RUST_LOG` env var (e.g. `"info"`, `"a3net_mesh=debug"`).
    pub level: String,
...
impl Default for ReplConfig {
    fn default() -> Self {
        Self {
            prompt: "a3net> ".to_string(),
            history_file: None,
        }
    }
}

/// Mirrors [`a3net_ipc::validation::ValidationPolicy`] but is
/// duplicated here so the CLI doesn't have to depend on `a3net-ipc`.
...
/// cargo feature on `a3net-node` is **fail-soft**: the CLI logs a
/// warning and proceeds without the iroh runtime.
///
/// [`IrohIdentity`]: a3net_transport::iroh::IrohIdentity
/// [`IrohRuntime`]: a3net_node::IrohRuntime
#[derive(Debug, Clone, Serialize, Deserialize)]
#[serde(rename_all = "camelCase", default)]
pub struct IrohConfig {
    ...
    /// `a3net_transport::iroh::IrohIdentity::load_or_create` uses.
    ...
    /// [`a3net_transport::iroh::discovery::DiscoveryConfig`]
    /// that operators can set via the config file. Each
    /// sub-field is independent and `None` falls back to the
    /// A3Net / iroh defaults. Operators that want to surface a
...
/// type lives in `a3net-transport` and the CLI must not pull
...
///   publishes, so A3Net endpoints can be discovered by
...
///   via mDNS on the local LAN and discovers other A3Net nodes
...
pub struct DiscoveryConfigToml {
    ...
    ///
    /// The `alias = "user_data"` lets `a3net config set
    ...
    /// discovers other A3Net nodes on the same LAN without a
...
pub fn default_config_path() -> Option<PathBuf> {
    if let Some(base) = dirs::config_dir() {
        return Some(base.join("a3net").join(CONFIG_FILE_NAME));
    }
    if let Some(home) = dirs::home_dir() {
        return Some(home.join(".config").join("a3net").join(CONFIG_FILE_NAME));
    }
...
pub const DEFAULT_CONFIG_TEMPLATE: &str = r#"{
  // A3Net CLI configuration. Edit any field below and re-run the
  ...
  "log": {
    ...
    //   "info"            — global info
    //   "a3net_mesh=debug" — debug for the mesh crate, info elsewhere
    "level": "info",
  ...
  // Default room when --room is omitted on `a3net feed` /
  // `a3net echo`. Comment out to require explicit --room.
  // "defaultRoom": "lobby",

  // REPL settings (only used by `a3net run`).
  "repl": {
  ...
  // the full IrohRuntime (blobs/gossip/docs + a3net/frame/1 ALPN)
  // into the node. Default enabled = false — set to true to opt in.
  // Requires `cargo build -p a3net-node --features iroh` at compile
  ...
  // with `cargo build -p a3net-cli --features qr`; the block is
  ...
  // Storage budget. The CLI hands this to `a3net-blobstore` whenever
...
// ---------------------------------------------------------------------------
// Top-level dispatcher for `a3net config <sub>`
// ---------------------------------------------------------------------------
...
pub fn run_config(sub: &crate::cli::ConfigCmd, data_dir: &std::path::Path) -> anyhow::Result<()> {
    ...
    match sub {
        ...
        CliConfigCmd::Reset { yes } => {
            if !yes {
                eprintln!("a3net: about to reset config to defaults");
...
// ---------------------------------------------------------------------------
//
// These helpers are invoked by the `a3net config <sub>` dispatcher in
...
pub fn load_for_cli(cli_override: Option<&Path>) -> Result<LoadedConfig> {
    ...
    {
        println!("a3net: wrote default config template to {}", path.display());
    }
    Ok(loaded)
}
...
/// `a3net config set log.levl debug` gets a clear error instead of
/// a silently-created bogus field. Each top-level field may be
/// addressed under either its camelCase or its snake_case form, so
/// `a3net config set data_dir /tmp/x` and `a3net config set dataDir
/// /tmp/x` both work.
const KNOWN_DOTTED_KEYS: &[&str] = &[
    ...
    // snake_case so `a3net config set storage.total_bytes 50GiB`
...
pub fn edit(path: &Path) -> Result<()> {
    ...
    tmp.push(format!(".a3net-config-edit-{}.json5", std::process::id()));
    {
    ...
    if edited == original {
        println!("a3net: no changes detected");
        let _ = fs::remove_file(&tmp);
        return Ok(());
    }
    ...
    match parse_file(&tmp) {
        ...
        Err(e) => {
            ...
            // and re-run `a3net config edit`.
            return Err(anyhow!(
                "edited config did not parse: {e}\n\nthe temp file is still at {}\nfix it (or move it back into place) and re-run `a3net config edit`",
                tmp.display()
            ));
        }
    }
    ...
    let _ = fs::remove_file(&tmp);
    println!("a3net: updated {}", path.display());
    Ok(())
}
...
mod tests {
    ...
    fn validate_passes_on_default_template() {
        ...
        reset(&path).unwrap();
        let cfg = validate(&path).unwrap();
        assert_eq!(cfg.data_dir, PathBuf::from("./.a3net-data"));
    }
    ...
    fn show_effective_roundtrip() {
        ...
        assert!(json.contains(".a3net-data"));
    ...
    fn parses_minimal_config() {
        ...
        assert_eq!(cfg.data_dir, PathBuf::from("/tmp/a3net"));
        assert_eq!(cfg.log.level, "debug");
        assert_eq!(cfg.log.format, LogFormat::Json);
        // Defaults still hold for unspecified fields.
        assert_eq!(cfg.repl.prompt, "a3net> ");
    }
    ...
    fn iroh_discovery_user_data_round_trip() {
        let raw = r#"{
            "iroh": {
                "enabled": false,
                "discovery": { "userData": "a3net/role=worker" }
            }
        }"#;
        let cfg: AppConfig = json5::from_str(raw).unwrap();
    ...
    fn set_value_accepts_discovery_user_data_dotted_key() {
        ...
        let dir = tempdir();
        let path = dir.0.join("config.json");
        set_value(&path, "iroh.discovery.userData", "\"a3net/role=worker\"").unwrap();
        let cfg = validate(&path).unwrap();
        let iroh = cfg.iroh.expect("iroh block");
        assert_eq!(
            iroh.discovery.user_data.as_deref(),
    ...
    /// Without this, an operator staring at `a3net config show`
    ...
    fn qr_ec_level_resolves() {
        use a3net_qr::generator::QrErrorCorrectionLevel;
        assert_eq!(
    ...
    fn apply_dotted_set_deeply_nested() {
        ...
        apply_dotted_set(
            &mut root,
            "iroh.discovery.userData",
            serde_json::json!("a3net/role=worker"),
        )
        .unwrap();
        assert_eq!(root["iroh"]["discovery"]["userData"], "a3net/role=worker");
    }
