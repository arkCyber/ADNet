//! Interactive configuration wizard for A3Net.
//!
//! Provides a step-by-step interactive setup experience using
//! the `a3net_tui` crate for rich terminal output.
//!
//! ## Usage
//!
//! ```bash
...
pub fn run_wizard(config_path: &PathBuf) -> Result<()> {
    ...
    //
    // Previously the wizard only printed a hint ("use `a3net storage
...
The shared scope gets the complementary value. 0.5 = 50/50.",
    ...
    println!("Run `a3net config show` to view your configuration.");
    println!("Run `a3net run` to start A3Net with these settings.");
...
fn print_header() {
    ...
    println!(
        "{}",
        Color::Cyan
            .paint("║               A3Net Configuration Wizard                              ║")
    );
    println!(
        "{}",
        Color::Cyan
    ...
    println!(
        "{}",
        Color::Dim.paint("This wizard will help you set up A3Net configuration step by step.")
    );
    println!(
        "{}",
...
fn print_review(state: &WizardState) {
    ...
    panel = panel.add_field(
        "Data Directory",
        state.data_dir.as_deref().unwrap_or("./.a3net-data"),
    );
...
mod tests {
    ...
    fn test_generate_config_defaults() {
        let state = WizardState::default();
        let config = generate_config(&state);
        assert_eq!(config.data_dir, PathBuf::from("./.a3net-data"));
