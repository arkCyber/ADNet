//! `a3net run` — interactive `/cmd` REPL sitting on top of a running [`Node`].
//!
...
pub async fn run(data_dir: PathBuf, node: Node) -> Result<()> {
    println!("a3net REPL — type `/help` for the command list, `/quit` to exit");
    println!(
        "node: {}    data: {}",
        node.node_id().short(),
        data_dir.display()
    ...
    loop {
        ...
        match cmd {
            ...
            "/echo" => {
                ...
                let hash = a3net_types::ContentHash::from_bytes(format!("echo:{room}").as_bytes());
            ...
            "/transport" => match node.transport_handle() {
                Some(t) => {
                    ...
                    let extra = if let Some(quic) = t
                        .as_any()
                        .and_then(|a| a.downcast_ref::<a3net_transport::QuicTransport>())
                    {
                        format!(
                            "\n  bind         = {}\n  cert_fp      = {}\n  incoming_q   = {}",
                            quic.bind_addr(),
...
mod tests {
    ...
    fn feed_for_humans_sorts_recent_first() {
        ...
        let h1 = a3net_types::ContentHash::from_bytes(b"a");
        let h2 = a3net_types::ContentHash::from_bytes(b"b");
        let room = a3net_types::RoomId::new("lobby");
        let older = a3net_types::RoomAsset {
            ...
            source_url: None,
            room_id: room.clone(),
            announcer_node_id: a3net_types::NodeId::random(),
            announced_at: chrono::Utc::now() - chrono::Duration::seconds(60),
        };
        let newer = a3net_types::RoomAsset {
