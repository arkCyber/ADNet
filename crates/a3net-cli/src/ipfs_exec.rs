...
/// real wire-level path used by `a3net ipfs name publish` and
/// `a3net ipfs name resolve`.
#[cfg(feature = "ipfs")]
async fn run_name_command(
    ...
    let data_dir = data_dir
        .map(|p| p.to_path_buf())
        .unwrap_or_else(|| std::path::PathBuf::from("./a3net-data"));
    ...
    let dht_store = a3net_dht::store::new_in_memory_store();
    ...
    let secret: Arc<dyn a3net_namespace::SecretKey> = Arc::new(secret);
    ...
    match cmd {
        NameCmd::Publish { path, lifetime } => {
            ...
            let name =
                a3net_namespace::public_key_to_ipns_name(&secret.public_key_bytes());
...
fn load_or_create_ipns_key(
    path: &Path,
) -> Result<a3net_namespace::Ed25519SecretKey, anyhow::Error> {
...
async fn run_gateway_command(
    ...
    let data_dir = data_dir
        .map(|p| p.to_path_buf())
        .unwrap_or_else(|| PathBuf::from("./a3net-data"));
    match cmd {
        GatewayCmd::Serve { bind, cors: _, writable: _ } => {
            ...
            let dht = Arc::new(a3net_gateway::DhtService::new(
                "gateway".to_string(),
                vec![],
            ));
            ...
            println!("Starting A3Net IPFS Gateway on http://{addr}");
...
async fn run_car_command(
    ...
    use a3net_blobstore::car::{read_car, write_car, CarBlock, CarHeader};
...
async fn run_dns_command(domain: String, json: bool) -> Result<(), anyhow::Error> {
    use a3net_namespace::{DnsLinkPath, DnsLinkResolver};
