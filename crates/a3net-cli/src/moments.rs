//! `a3net moments …` — Social-feed (朋友圈) CLI surface.
//!
//! Mirrors the original Tauri command layer from
//! `Exodus@src-backup/src-tauri/src/microservice/social_feed_commands.rs`.
//! Subcommands:
//!
//! - `a3net moments post <file|->` — publish a post. `-` reads the
//!   post body from stdin.
//! - `a3net moments timeline <viewer>` — print the visible
//!   timeline for a viewer, oldest-first to newest-last.
//! - `a3net moments comment <post_id> <file|->` — add a comment.
//! - `a3net moments react <target_id> <reaction_type>` — add /
//!   remove a reaction.
//! - `a3net moments follow <follower_id> <following_id>` — follow.
//! - `a3net moments unfollow <follower_id> <following_id>` —
//!   unfollow.
//!
//! All subcommands are offline: they only touch
//! `<data_dir>/a3net_social_feed.db` (SQLite) and never spin up
