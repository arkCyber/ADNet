...
//! QR codes) and ADNet-native payloads (peer tickets, blob tickets,
...
use adnet_types::{BlobTicket, NodeAddrTicket, PeerTicket, SignedPeerTicket};

#[cfg(feature = "pairing")]
use adnet_pairing::wire::PairingInvitation;

/// Conversion from a `DCLOGIN:` payload into an `adnet_mail` config.
///
...
/// `DcLoginOptions` because `adnet_mail::Account` may carry fields
...
///   because `adnet-mail` does not embed a provider-db that knows
///   which port to dial.
...
pub struct MailAccountFromQr {
    inner: adnet_mail::login_param::Account,
}

#[cfg(feature = "mail")]
impl MailAccountFromQr {
    /// Borrow the produced [`adnet_mail::Account`].
    pub fn account(&self) -> &adnet_mail::login_param::Account {
        &self.inner
    }
    /// Consume the bridge and hand back the inner `Account`.
    pub fn into_account(self) -> adnet_mail::login_param::Account {
        self.inner
    }
}
...
#[cfg(feature = "mail")]
impl QrPayload {
    /// Convert a `DCLOGIN:` payload into an `adnet_mail::Account`.
    ///
...
fn build_account_from_dclogin(
...
) -> crate::error::Result<adnet_mail::login_param::Account> {
    use adnet_mail::login_param::CertificateChecks;
    ...
    let imap = adnet_mail::login_param::ImapLoginParam {
        server: imap_host,
    ...
    let smtp = adnet_mail::login_param::SmtpLoginParam {
        server: smtp_host,
    ...
    adnet_mail::login_param::Account::new(address.to_string(), imap, smtp)
        .map(|mut acct| {
...
#[cfg(feature = "mail")]
fn map_security(s: Option<DcLoginSecurity>) -> adnet_mail::login_param::SocketSecurity {
    use adnet_mail::login_param::SocketSecurity;
    match s {
...
pub enum QrPayload {
    ...
    // ────────────────────────────────────────────────────────────────────
    // ADNet-native payloads. These are encoded as `adnet-peer://`,
    // `adnet-blob://`, `adnet-signed-peer://`, and `adnet-token://` URLs.
    // ────────────────────────────────────────────────────────────────────
    /// `adnet-peer://…` — share a single node address.
    #[cfg(feature = "adnet-types")]
    #[serde(rename = "adnet_peer")]
    AdnetPeer { ticket: PeerTicket },

    /// `adnet-addr://…` — share a NodeAddr as a printable ticket.
    #[cfg(feature = "adnet-types")]
    #[serde(rename = "adnet_addr")]
    AdnetAddr { ticket: NodeAddrTicket },

    /// `adnet-blob://…` — share a blob (whole or range-restricted).
    #[cfg(feature = "adnet-types")]
    #[serde(rename = "adnet_blob")]
    AdnetBlob { ticket: BlobTicket },

    /// `adnet-signed-peer://…` — peer ticket + signature from a wallet.
    #[cfg(feature = "adnet-types")]
    #[serde(rename = "adnet_signed_peer")]
    AdnetSignedPeer { ticket: SignedPeerTicket },

    /// `adnet-token://…` — relay-payment pledge.
    #[cfg(feature = "adnet-token")]
    #[serde(rename = "adnet_token")]
    AdnetToken { pledge: Pledge },

    /// `adnet-pairing://…` — pairing invitation. Carries the
    /// invitation envelope produced by `adnet-pairing::SignedInvitation`
    ...
    /// [`PairingInvitation::decode`]; see `adnet-pairing` for
    /// verification.
    #[cfg(feature = "pairing")]
    #[serde(rename = "adnet_pairing")]
...
/// values into [`CertificateChecks`] (the same enum `adnet-mail`
/// already exposes).
///
...
pub enum DcCertificateChecks {
    ...
    /// even though `adnet-mail` defaults to strict).
    Automatic,
...
impl QrPayload {
    ...
    pub fn tag(&self) -> &'static str {
        match self {
            ...
            QrPayload::Text { .. } => "text",
            #[cfg(feature = "adnet-types")]
            QrPayload::AdnetPeer { .. } => "adnet_peer",
            #[cfg(feature = "adnet-types")]
            QrPayload::AdnetAddr { .. } => "adnet_addr",
            #[cfg(feature = "adnet-types")]
            QrPayload::AdnetBlob { .. } => "adnet_blob",
            #[cfg(feature = "adnet-types")]
            QrPayload::AdnetSignedPeer { .. } => "adnet_signed_peer",
            #[cfg(feature = "adnet-token")]
            QrPayload::AdnetToken { .. } => "adnet_token",
            #[cfg(feature = "pairing")]
            QrPayload::AdnetPairing { .. } => "adnet_pairing",
        }
    }
    ...
    fn serialize_default<S: serde::Serializer>(
        ...
        match self {
            ...
            QrPayload::AdnetPeer { ticket } => {
                ...
                s.serialize_field("kind", "adnet_peer")?;
            ...
            QrPayload::AdnetAddr { ticket } => {
                ...
                s.serialize_field("kind", "adnet_addr")?;
                s.serialize_field("ticket", ticket)?;
                s.end()
            }
            #[cfg(feature = "adnet-types")]
            QrPayload::AdnetBlob { ticket } => {
                ...
                s.serialize_field("kind", "adnet_blob")?;
                s.serialize_field("ticket", ticket)?;
                s.end()
            }
            #[cfg(feature = "adnet-types")]
            QrPayload::AdnetSignedPeer { ticket } => {
                ...
                s.serialize_field("kind", "adnet_signed_peer")?;
                s.serialize_field("ticket", ticket)?;
                s.end()
            }
            #[cfg(feature = "adnet-token")]
            QrPayload::AdnetToken { pledge } => {
                ...
                s.serialize_field("kind", "adnet_token")?;
            ...
            QrPayload::AdnetPairing { invitation } => {
                ...
                s.serialize_field("kind", "adnet_pairing")?;
...
mod tests {
    ...
    fn dclogin_to_account_happy_path() {
        ...
        assert!(matches!(
            ...
            adnet_mail::login_param::CertificateChecks::Strict
        ));
    }
    ...
    fn dclogin_to_account_accept_invalid_certs_kept() {
        ...
        assert!(matches!(
            acct.certificate_checks,
            adnet_mail::login_param::CertificateChecks::AcceptInvalid
        ));
    }
