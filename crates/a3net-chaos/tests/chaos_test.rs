...
async fn test_fault_types() {
    ...
    assert_eq!(latency.default_severity(), a3net_chaos::Severity::Low);
    
    let partition = FaultType::NetworkFault(NetworkFaultType::Partition);
    assert_eq!(partition.default_severity(), a3net_chaos::Severity::Critical);
    
    // Node faults
    let crash = FaultType::NodeFault(NodeFaultType::Crash);
    assert_eq!(crash.default_severity(), a3net_chaos::Severity::Critical);
    
    // Data faults
    let corruption = FaultType::DataFault(DataFaultType::Corruption);
    assert_eq!(corruption.default_severity(), a3net_chaos::Severity::High);
}
