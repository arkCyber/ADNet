...
A comprehensive fault injection framework for testing the resilience of A3Net distributed systems.

## Overview

This framework provides tools for injecting various faults into A3Net to test system resilience, measure degradation under stress, and validate recovery mechanisms.
...
```rust
use a3net_chaos::{ChaosEngine, Scenario, Hypothesis};
...
cargo run -p a3net-chaos --example chaos_cli -- list

# Run a chaos profile
cargo run -p a3net-chaos --example chaos_cli -- run network-resilience

# Generate a sample scenario
cargo run -p a3net-chaos --example chaos_cli -- generate network-partition -o scenario.json

# Validate a scenario file
cargo run -p a3net-chaos --example chaos_cli -- validate scenario.json
```
...
```rust
use a3net_chaos::{ChaosEngine, ChaosEventEmitter, ChaosEvent};
...
```toml
[dependencies]
a3net-chaos = { version = "0.1", features = ["full"] }

# Or specific features
a3net-chaos = { version = "0.1", features = ["network", "node"] }
```
...
- `full`: All features
- `simulator`: Integration with a3net-simulator
