...
`a3net-rpc` 提供与 IPFS HTTP API 兼容的命令面(command surface),让 A3Net 节点对外部世界呈现"长得像 IPFS 节点"的行为:
...
- **HTTP 兼容** — `a3net-gateway` 直接把命令函数挂到 `/api/v0/<command>` 路径上,符合 IPFS 客户端的预期。
...
[dependencies]
a3net-rpc = { workspace = true }
a3net-blobstore = { workspace = true }   # 命令实现依赖
```
...
use a3net_rpc::{block_put, block_get, version};
...
```rust
use a3net_rpc::commands::{dag_put, dag_get, block_put, block_rm};
...
- **`a3net-gateway`** — 把所有命令函数挂到 `/api/v0/<command>` HTTP handler 上;现有 IPFS SDK(`kubo-client`、IPFS Companion)可直接对 A3Net 节点进行读写。
- **CI 测试** — 集成测试使用 `RpcClient` 在内存 `BlobStore` 上跑端到端场景,无需启动 HTTP server。
- **CLI** — `a3net-cli` 的 `put` / `get` / `pin` 子命令都通过 RPC 调用,逻辑与 gateway 一致,避免两套实现漂移。
- **多语言 SDK** — 因为命令集与 IPFS HTTP API 兼容,任何已存在的 IPFS 客户端 SDK(JS / Python / Go)都可直接对接 A3Net 节点。
- **DHT 索引** — `dht/findprovs` 与 `name/resolve` 让外部世界像查询普通 IPFS 节点一样查询 A3Net 节点。
