//! `adnet-ffi` — C-ABI surface for ADNet.
//!
//! Mobile embedders (iOS / Android) and WASM modules consume
//! ADNet through a **C-compatible ABI** rather than the Rust
...
//!   responsible for calling [`adnet_ffi_free`] to release the
...
//!
//! ```bash
//! # Library only — the embedder links adnet_ffi.{a,lib}.
//! cargo build -p adnet-ffi --release
//!
//! # C-headers via cbindgen (separate tool, not run here).
//! cbindgen --crate adnet-ffi --output adnet_ffi.h
//! ```
//!
//! # Stability
//!
...
//!
//! [`adnet_ffi_free`]: crate::adnet_ffi_free
...
use adnet_types::{ContentHash, NodeCapability};
#[allow(unused_imports)]
use adnet_userstore::{SqliteUserStore, SqliteUserStoreConfig, UserStore};
#[allow(unused_imports)]
use adnet_roster::RosterStore;
...
/// all `adnet_news_*` FFI symbols are skipped.
#[cfg(feature = "news")]
pub mod news_ffi;
...
/// caller asked for the iroh runtime but `adnet-ffi` was
...
/// [`adnet_ffi_free`] once the bytes have been consumed.
#[repr(C)]
...
#[unsafe(no_mangle)]
pub unsafe extern "C" fn adnet_ffi_free(buf: AdnetFfiBuffer) {
    if !buf.ptr.is_null() {
...
pub struct AdnetFfiHandle {
    ...
    // `adnet_ffi_node_lock` / `adnet_ffi_node_unlock` calls
    ...
    pub node: Arc<std::sync::Mutex<Option<adnet_node::Node>>>,
...
/// mismatched library.
#[unsafe(no_mangle)]
pub unsafe extern "C" fn adnet_ffi_version() -> u32 {
    ADNET_FFI_VERSION
}
...
#[unsafe(no_mangle)]
pub unsafe extern "C" fn adnet_ffi_node_create(
    ...
    let node_result = handle.runtime.block_on(async {
        let cfg = adnet_node::NodeConfig::load_or_create(&data_dir)
            .map_err(|e| AdnetFfiError::Node(e.to_string()))?;
        adnet_node::Node::builder(cfg)
            .build()
            .await
            .map_err(Into::into)
    });
...
/// ADNet id is `adnet-abc…`").
#[unsafe(no_mangle)]
pub unsafe extern "C" fn adnet_ffi_node_id(
...
/// node state required.
#[unsafe(no_mangle)]
pub unsafe extern "C" fn adnet_ffi_hash_bytes(
...
/// `OK`).
/// How long `adnet_ffi_node_destroy` waits for the tokio
...
#[unsafe(no_mangle)]
pub unsafe extern "C" fn adnet_ffi_node_destroy(handle: *mut AdnetFfiHandle) -> AdnetFfiStatus {
    if handle.is_null() {
        return ADNET_FFI_OK;
    }
...
#[unsafe(no_mangle)]
pub unsafe extern "C" fn adnet_ffi_node_profile(
    handle: *mut AdnetFfiHandle,
    out: *mut AdnetFfiBuffer,
) -> AdnetFfiStatus {
    let h = match unsafe { handle.as_ref() } {
    ...
    let result = FfiResult::ok(ProfileInfo {
        node_id: profile.node_id.as_hex().to_string(),
        node_id_short: profile.node_id.short().to_string(),
...
#[unsafe(no_mangle)]
pub unsafe extern "C" fn adnet_ffi_node_role(
    handle: *mut AdnetFfiHandle,
    out: *mut AdnetFfiBuffer,
) -> AdnetFfiStatus {
    let h = match unsafe { handle.as_ref() } {
    ...
    let role = node.profile().role.label().to_string();
    #[derive(Serialize)]
    struct RoleInfo {
        role: String,
    }
...
// default-build `cargo build -p adnet-ffi` produces a
// library that *only* exposes the always-available FFI
// surface above (`adnet_ffi_version`, `adnet_ffi_node_create`,
// etc.).
...
#[cfg(feature = "iroh")]
#[unsafe(no_mangle)]
pub unsafe extern "C" fn adnet_ffi_node_addr(
    handle: *mut AdnetFfiHandle,
    out: *mut AdnetFfiBuffer,
) -> AdnetFfiStatus {
    let h = match unsafe { handle.as_ref() } {
    ...
    // peer can dial with `adnet_ffi_dial`.
...
#[cfg(feature = "iroh")]
#[unsafe(no_mangle)]
pub unsafe extern "C" fn adnet_ffi_dial(
    handle: *mut AdnetFfiHandle,
    ...
    let node_id = match adnet_types::NodeId::from_hex(&node_id_str) {
        Ok(n) => n,
...
/// `adnet_ffi_dial` / `adnet_ffi_send_frame`. The lock is
...
//
// The functions below expose the `adnet-roster` and `adnet-userstore`
...
// `UserProfile` structs from `adnet-roster` / `adnet-userstore`
...
fn open_roster(
    data_dir_ptr: *const c_char,
    data_dir_len: usize,
) -> Result<adnet_roster::SqliteRosterStore, AdnetFfiError> {
    let data_dir = bytes_to_nonempty_string(data_dir_ptr, data_dir_len, "data_dir")?;
    adnet_roster::SqliteRosterStore::open(
        adnet_roster::SqliteRosterStoreConfig::under_app_data(std::path::Path::new(&data_dir)),
    )
    .map_err(|e| AdnetFfiError::Node(e.to_string()))
}
...
fn open_userstore(
    data_dir_ptr: *const c_char,
    data_dir_len: usize,
) -> Result<adnet_userstore::SqliteUserStore, AdnetFfiError> {
    let data_dir = bytes_to_nonempty_string(data_dir_ptr, data_dir_len, "data_dir")?;
    adnet_userstore::SqliteUserStore::open(
        adnet_userstore::SqliteUserStoreConfig::under_app_data(std::path::Path::new(&data_dir)),
    )
    .map_err(|e| AdnetFfiError::Node(e.to_string()))
}
...
#[unsafe(no_mangle)]
pub unsafe extern "C" fn adnet_ffi_roster_add_contact(
    data_dir_ptr: *const c_char,
    data_dir_len: usize,
    ...
    let result = (|| -> Result<(), AdnetFfiError> {
        ...
        let contact: adnet_roster::Contact = serde_json::from_str(&payload)
...
#[unsafe(no_mangle)]
pub unsafe extern "C" fn adnet_ffi_roster_list_contacts(
    data_dir_ptr: *const c_char,
    data_dir_len: usize,
    out: *mut AdnetFfiBuffer,
) -> AdnetFfiStatus {
    let result = (|| -> Result<Vec<adnet_roster::Contact>, AdnetFfiError> {
        let store = open_roster(data_dir_ptr, data_dir_len)?;
...
#[unsafe(no_mangle)]
pub unsafe extern "C" fn adnet_ffi_roster_list_groups(
    data_dir_ptr: *const c_char,
    data_dir_len: usize,
    out: *mut AdnetFfiBuffer,
) -> AdnetFfiStatus {
    let result = (|| -> Result<Vec<adnet_roster::ContactGroup>, AdnetFfiError> {
        let store = open_roster(data_dir_ptr, data_dir_len)?;
...
/// `adnet_ffi_roster_list_contacts`.
#[unsafe(no_mangle)]
pub unsafe extern "C" fn adnet_ffi_roster_search_contacts(
    data_dir_ptr: *const c_char,
    data_dir_len: usize,
...
#[unsafe(no_mangle)]
pub unsafe extern "C" fn adnet_ffi_roster_delete_contact(
    data_dir_ptr: *const c_char,
    data_dir_len: usize,
...
#[unsafe(no_mangle)]
pub unsafe extern "C" fn adnet_ffi_user_upsert_profile(
    data_dir_ptr: *const c_char,
    data_dir_len: usize,
    payload_ptr: *const c_char,
    payload_len: usize,
    ...
    let result = (|| -> Result<(), AdnetFfiError> {
        ...
        let payload = bytes_to_string(payload_ptr, payload_len)?;
        let profile: adnet_userstore::UserProfile = serde_json::from_str(&payload)
            .map_err(|e| AdnetFfiError::Json(e.to_string()))?;
...
#[unsafe(no_mangle)]
pub unsafe extern "C" fn adnet_ffi_user_list_profiles(
    data_dir_ptr: *const c_char,
    data_dir_len: usize,
    out: *mut AdnetFfiBuffer,
) -> AdnetFfiStatus {
    let result = (|| -> Result<Vec<adnet_userstore::UserProfile>, AdnetFfiError> {
        let store = open_userstore(data_dir_ptr, data_dir_len)?;
...
#[unsafe(no_mangle)]
pub unsafe extern "C" fn adnet_ffi_user_get_profile(
    data_dir_ptr: *const c_char,
    data_dir_len: usize,
    ...
    let result = (|| -> Result<Option<adnet_userstore::UserProfile>, AdnetFfiError> {
...
#[unsafe(no_mangle)]
pub unsafe extern "C" fn adnet_ffi_user_ensure_digit(
    data_dir_ptr: *const c_char,
    data_dir_len: usize,
    user_id_ptr: *const c_char,
    user_id_len: usize,
...
/// `adnet_ffi_dial` consumes the same shape.
#[derive(Debug, Serialize, Deserialize)]
...
/// Returned by `adnet_ffi_blob_put_bytes`.
#[derive(Debug, Serialize, Deserialize)]
...
/// Returned by `adnet_ffi_blob_fetch_ticket`.
#[derive(Debug, Serialize, Deserialize)]
...
/// Returned by `adnet_ffi_ipns_publish`.
#[derive(Debug, Serialize, Deserialize)]
pub struct IpnsPublishInfo {
    pub name: String,
}

/// Returned by `adnet_ffi_ipns_resolve`.
#[derive(Debug, Serialize, Deserialize)]
...
/// Returned by `adnet_ffi_node_metrics`.
#[derive(Debug, Serialize, Deserialize)]
...
mod tests {
    ...
    /// in a `tempfile` crate to keep `adnet-ffi`'s dev-deps
    ...
    fn result_ok_serializes_value() {
        let r: FfiResult<NodeIdInfo> = FfiResult::ok(NodeIdInfo {
            node_id: "adnet-abc".into(),
        });
        let json = serde_json::to_string(&r).unwrap();
        assert!(json.contains("\"ok\":true"));
        assert!(json.contains("\"node_id\":\"adnet-abc\""));
    ...
    fn destroy_null_handle_is_noop() {
        ...
        // no-op for `adnet_ffi_node_destroy`.
        let status = unsafe { adnet_ffi_node_destroy(std::ptr::null_mut()) };
    ...
    // `adnet-ffi` crate does NOT yet ship `put_blob`,
    ...
    // the corresponding Rust APIs land in `adnet-node`.
    //
    ...
    #[unsafe(no_mangle)]
    pub unsafe extern "C" fn adnet_ffi_blob_fetch_ticket(
        handle: *mut AdnetFfiHandle,
        ...
        let result = (|| -> Result<BlobFetchInfo, AdnetFfiError> {
            ...
            let hash = adnet_types::ContentHash::from_hex(&ticket)
    ...
    /// `Node::put_blob` lands.
    #[unsafe(no_mangle)]
    pub unsafe extern "C" fn adnet_ffi_blob_put_bytes(
        handle: *mut AdnetFfiHandle,
        data_ptr: *const c_char,
        data_len: usize,
        out: *mut AdnetFfiBuffer,
        ...
        let result = (|| -> Result<BlobPutInfo, AdnetFfiError> {
            ...
            let hash = adnet_types::ContentHash::from_bytes(bytes);
            Ok(BlobPutInfo {
                hash: hash.as_hex().to_string(),
    ...
    #[unsafe(no_mangle)]
    pub unsafe extern "C" fn adnet_ffi_ipns_publish(
    ...
    /// in `adnet-namespace` and a future PR will wire it
    /// through `Node::ipns_resolve`.
    #[unsafe(no_mangle)]
    pub unsafe extern "C" fn adnet_ffi_ipns_resolve(
        _handle: *mut AdnetFfiHandle,
        name_ptr: *const c_char,
        name_len: usize,
        out: *mut AdnetFfiBuffer,
    ...
    ///
    /// Mirrors `adnet_ffi_node_profile` / `adnet_ffi_node_role`:
    ...
    /// expose the full `adnet-observability` snapshot.
    #[unsafe(no_mangle)]
    pub unsafe extern "C" fn adnet_ffi_node_metrics(
        handle: *mut AdnetFfiHandle,
        out: *mut AdnetFfiBuffer,
    ) -> AdnetFfiStatus {
        let result = (|| -> Result<NodeMetricsInfo, AdnetFfiError> {
            ...
            // Mirror `adnet_ffi_node_profile` /
            // `adnet_ffi_node_role`: require a created node
            ...
            // will surface the full `adnet-observability`
            // snapshot.
            Ok(NodeMetricsInfo {
                peer_count: 0,
                blob_count: 0,
    ...
    fn ffi_roster_add_list_search_roundtrip() {
        let tmp = tempdir_named("adnet-ffi-roster");
        ...
        let status = unsafe {
            adnet_ffi_roster_list_contacts(data_dir_buf.0, data_dir_buf.1, &mut out2)
        };
        assert_eq!(status, ADNET_FFI_OK);
        ...
        let status = unsafe {
            adnet_ffi_roster_search_contacts(
                data_dir_buf.0,
                data_dir_buf.1,
                query_buf.0,
                query_buf.1,
        ...
        let status = unsafe {
            adnet_ffi_roster_delete_contact(
                data_dir_buf.0,
                data_dir_buf.1,
                id_buf.0,
                id_buf.1,
    ...
    fn ffi_user_upsert_get_ensure_digit() {
        let tmp = tempdir_named("adnet-ffi-user");
        let data_dir = tmp.path().to_string_lossy().to_string();
        let data_dir_buf = encode_string(&data_dir);
        ...
        let mut out3 = AdnetFfiBuffer { ptr: std::ptr::null_mut(), len: 0 };
        let status = unsafe {
            adnet_ffi_user_ensure_digit(data_dir_buf.0, data_dir_buf.1, id_buf.0, id_buf.1, &mut out3)
        };
        assert_eq!(status, ADNET_FFI_OK);
        let body = decode_buffer(out3);
    ...
    fn ffi_roster_add_rejects_invalid_json() {
        let tmp = tempdir_named("adnet-ffi-roster-bad");
        let data_dir = tmp.path().to_string_lossy().to_string();
        let data_dir_buf = encode_string(&data_dir);
    ...
    fn ffi_roster_list_groups_is_empty_on_fresh_db() {
        let tmp = tempdir_named("adnet-ffi-roster-groups");
        let data_dir = tmp.path().to_string_lossy().to_string();
        let data_dir_buf = encode_string(&data_dir);

        let mut out = AdnetFfiBuffer { ptr: std::ptr::null_mut(), len: 0 };
        let status = unsafe {
            adnet_ffi_roster_list_groups(data_dir_buf.0, data_dir_buf.1, &mut out)
        };
        assert_eq!(status, ADNET_FFI_OK);
        let body = decode_buffer(out);
    ...
    fn ffi_user_list_profiles_is_empty_on_fresh_db() {
        let tmp = tempdir_named("adnet-ffi-user-profiles");
        let data_dir = tmp.path().to_string_lossy().to_string();
        let data_dir_buf = encode_string(&data_dir);

        let mut out = AdnetFfiBuffer { ptr: std::ptr::null_mut(), len: 0 };
        let status = unsafe {
            adnet_ffi_user_list_profiles(data_dir_buf.0, data_dir_buf.1, &mut out)
        };
        assert_eq!(status, ADNET_FFI_OK);
        let body = decode_buffer(out);
        assert_eq!(body, "{\"ok\":true,\"value\":[]}");
    ...
    fn ffi_roster_search_contacts_empty_query_returns_all() {
        ...
        let tmp = tempdir_named("adnet-ffi-roster-search-empty");
        let data_dir = tmp.path().to_string_lossy().to_string();
        let data_dir_buf = encode_string(&data_dir);
    ...
    fn ffi_roster_delete_rejects_empty_contact_id() {
        let tmp = tempdir_named("adnet-ffi-roster-bad-id");
        let data_dir = tmp.path().to_string_lossy().to_string();
        let data_dir_buf = encode_string(&data_dir);
    ...
    fn ffi_user_get_rejects_empty_user_id() {
        let tmp = tempdir_named("adnet-ffi-user-bad-id");
        let data_dir = tmp.path().to_string_lossy().to_string();
        let data_dir_buf = encode_string(&data_dir);
        let empty_id = encode_string("");
        let mut out = AdnetFfiBuffer { ptr: std::ptr::null_mut(), len: 0 };
    ...
    fn ffi_user_ensure_digit_rejects_empty_user_id() {
        let tmp = tempdir_named("adnet-ffi-user-bad-digit");
        let data_dir = tmp.path().to_string_lossy().to_string();
        let data_dir_buf = encode_string(&data_dir);
        let empty_id = encode_string("");
        let mut out = AdnetFfiBuffer { ptr: std::ptr::null_mut(), len: 0 };
        let status = unsafe {
            adnet_ffi_user_ensure_digit(
                data_dir_buf.0,
                data_dir_buf.1,
                empty_id.0,
                empty_id.1,
    ...
    fn ffi_rejects_empty_data_dir() {
        ...
        let mut out2 = AdnetFfiBuffer { ptr: std::ptr::null_mut(), len: 0 };
        let status = unsafe {
            adnet_ffi_roster_list_contacts(empty_dir.0, empty_dir.1, &mut out2)
        };
        assert_eq!(status, ADNET_FFI_E_INVALID_ARG);
    }

    /// `adnet_ffi_blob_put_bytes` MUST reject a NULL data
    ...
    fn ffi_blob_put_bytes_rejects_null_data() {
        ...
        let mut out = AdnetFfiBuffer { ptr: std::ptr::null_mut(), len: 0 };
        let status = unsafe {
            adnet_ffi_blob_put_bytes(handle, std::ptr::null(), 0, &mut out)
        };
        assert_eq!(status, ADNET_FFI_E_INVALID_ARG);
        let _ = unsafe { Box::from_raw(handle) };
    }

    /// `adnet_ffi_blob_put_bytes` with a 0-byte slice is valid
    ...
    /// `adnet_ffi_ipns_publish` rejects an empty value via
    ...
    fn ffi_blob_put_then_fetch_roundtrip() {
        let tmp = tempdir_named("adnet-ffi-roundtrip");
        let h = AdnetFfiHandle::new(tmp.path().to_path_buf()).unwrap();
        let handle = Box::into_raw(Box::new(h));

        let payload = b"adnet-ffi-roundtrip-payload".to_vec();
        let ptr = payload.as_ptr() as *const c_char;
        let len = payload.len();
        let mut put_out = AdnetFfiBuffer { ptr: std::ptr::null_mut(), len: 0 };
        let put_status = unsafe { adnet_ffi_blob_put_bytes(handle, ptr, len, &mut put_out) };
        ...
        let fetch_status = unsafe {
            adnet_ffi_blob_fetch_ticket(handle, ticket_buf.0, ticket_buf.1, &mut fetch_out)
        };
    ...
    /// `adnet_ffi_node_metrics` returns documented status codes
    ...
    fn ffi_node_metrics_handles_unbooted_node() {
        ...
        let handle = Box::into_raw(Box::new(h));
        let mut out = AdnetFfiBuffer { ptr: std::ptr::null_mut(), len: 0 };
        let status = unsafe { adnet_ffi_node_metrics(handle, &mut out) };
        assert!(
    ...
    /// `adnet_ffi_blob_fetch_ticket` rejects invalid hex
