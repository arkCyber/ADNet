...
//! `adnet-ffi`. Every function follows the canonical contract:
//! `(ptr, len)` byte slices for inputs and outputs, status
//! codes from `AdnetFfiStatus`, and JSON-encoded payloads.
//!
//! Functions:
//!
//! - `adnet_news_publish` — publish a bulletin (room, kind,
//!   severity, category, title, summary, body, signer, signature)
//! - `adnet_news_timeline` — paginated newest-first listing
//! - `adnet_news_get` — fetch a single bulletin by id
//! - `adnet_news_mark_read` — mark a bulletin read by the local
//!   node
//! - `adnet_news_subscribe` — start a live subscription; returns
//!   a handle
//! - `adnet_news_poll` — drain buffered events from the
//!   subscription
//! - `adnet_news_unsubscribe` — destroy the subscription handle
...
/// Input payload for `adnet_news_publish`.
...
/// Output payload for `adnet_news_publish`.
#[derive(Debug, Clone, Serialize, Deserialize)]
...
/// Output payload for `adnet_news_timeline`.
#[derive(Debug, Clone, Serialize, Deserialize)]
...
/// `u64` token and calls `adnet_news_poll` / `adnet_news_unsubscribe`.
#[repr(C)]
...
/// either consume all events via `adnet_news_poll` or release the
/// handle with `adnet_news_unsubscribe`. We hold the `NewsService`
...
struct SubState {
    ...
    runtime: Arc<Runtime>,
    receiver: tokio::sync::broadcast::Receiver<adnet_news::BulletinEvent>,
    room: RoomId,
}

/// Output payload for `adnet_news_poll`.
#[derive(Debug, Clone, Serialize, Deserialize)]
...
impl NewsEventJson {
    fn from_event(ev: adnet_news::BulletinEvent) -> Self {
        match ev {
            adnet_news::BulletinEvent::Insert(item) => NewsEventJson {
                kind: "insert".into(),
            ...
            adnet_news::BulletinEvent::Correction {
                superseded_id,
                corrected,
            } => NewsEventJson {
            ...
            adnet_news::BulletinEvent::Retraction {
                superseded_id,
                retraction,
            } => NewsEventJson {
            ...
            adnet_news::BulletinEvent::ReplayComplete { room, replayed } => {
                NewsEventJson {
...
/// [`NewsPublishRequest`].
#[unsafe(no_mangle)]
pub unsafe extern "C" fn adnet_news_publish(
    data_dir_ptr: *const c_char,
    data_dir_len: usize,
    ...
    let result = (|| -> Result<NewsPublishResult, AdnetFfiError> {
        ...
        if !req.signature_hex.is_empty() && !req.signer_address.is_empty() {
            ...
            item.attach_signature(
                adnet_types::WalletAddress::from_bytes(addr),
                sig_bytes,
            );
        }
        let stored = runtime
...
#[unsafe(no_mangle)]
pub unsafe extern "C" fn adnet_news_timeline(
    data_dir_ptr: *const c_char,
    data_dir_len: usize,
    payload_ptr: *const c_char,
    payload_len: usize,
...
/// `{ "room": "...", "id": "<hex>" }`.
#[unsafe(no_mangle)]
pub unsafe extern "C" fn adnet_news_get(
    data_dir_ptr: *const c_char,
    data_dir_len: usize,
    payload_ptr: *const c_char,
    payload_len: usize,
    ...
    let result = (|| -> Result<Option<NewsTimelineEntry>, AdnetFfiError> {
        ...
        let req: Req = serde_json::from_str(&payload)
            .map_err(|e| AdnetFfiError::Json(e.to_string()))?;
        let id = adnet_types::BulletinId::from_hex(&req.id)
...
/// `{ "room": "...", "id": "<hex>" }`.
#[unsafe(no_mangle)]
pub unsafe extern "C" fn adnet_news_mark_read(
    data_dir_ptr: *const c_char,
    data_dir_len: usize,
    payload_ptr: *const c_char,
    payload_len: usize,
...
#[unsafe(no_mangle)]
pub unsafe extern "C" fn adnet_news_subscribe(
    data_dir_ptr: *const c_char,
    data_dir_len: usize,
    payload_ptr: *const c_char,
    payload_len: usize,
...
/// `adnet_news_unsubscribe`).
#[unsafe(no_mangle)]
pub unsafe extern "C" fn adnet_news_poll(
...
#[unsafe(no_mangle)]
pub unsafe extern "C" fn adnet_news_unsubscribe(handle: *mut AdnetNewsSubHandle) -> AdnetFfiStatus {
    if handle.is_null() {
...
mod tests {
    ...
    use crate::{adnet_ffi_free, FfiResult};
    ...
    fn end_to_end_publish_then_timeline_then_get_then_mark_read() {
        ...
        let buf_bytes = with_buffer(|buf| unsafe {
            let st = adnet_news_timeline(dir_ptr, dir_len, tl_ptr, tl_len, buf);
        ...
        let buf_bytes = with_buffer(|buf| unsafe {
            let st = adnet_news_get(dir_ptr, dir_len, get_ptr, get_len, buf);
    ...
    fn end_to_end_subscribe_polls_replayed_event() {
        ...
        for _ in 0..16 {
            let buf_bytes = with_buffer(|buf| unsafe {
                let st = adnet_news_poll(out_handle, buf);
        ...
        let status = unsafe { adnet_news_unsubscribe(out_handle) };
