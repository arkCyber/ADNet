...
//! (`cargo build -p adnet-ffi`) without `--features iroh` or
//! `--features news`.
//!
//! Run with:
//!   cargo run -p adnet-ffi --example ffi_app
...
use adnet_ffi::{
    adnet_ffi_hash_bytes, adnet_ffi_node_destroy, adnet_ffi_roster_add_contact,
    adnet_ffi_roster_delete_contact, adnet_ffi_roster_list_contacts,
    adnet_ffi_roster_list_groups, adnet_ffi_user_ensure_digit, adnet_ffi_user_get_profile,
    adnet_ffi_user_upsert_profile, AdnetFfiBuffer, ADNET_FFI_E_INVALID_ARG, ADNET_FFI_OK,
};

fn main() {
    println!("=== adnet-ffi app example (no node required) ===");

    let tmp = temp_dir("adnet-ffi-app");
    let data_dir = tmp.to_string_lossy().to_string();

    // ─── 1. Hash arbitrary bytes ────────────────────────────────
    // `adnet_ffi_hash_bytes(ptr, len, out)` is the v0 hash
    // shim the embedders call before pushing a blob. We invoke
    // it directly so the Swift / Kotlin parity test stays
    // honest.
    let payload = b"adnet-ffi-app hello world".to_vec();
    ...
    // a) `adnet_ffi_node_destroy(NULL)` is a documented no-op.
    ...
    let status = unsafe {
        adnet_ffi_user_ensure_digit(std::ptr::null(), 5, b"u1".as_ptr() as *const _, 2, &mut out)
    };
