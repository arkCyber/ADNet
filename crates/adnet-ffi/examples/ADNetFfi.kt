...
object ADNetFfi {
    init {
        System.loadLibrary("adnet_ffi")
    }

    // ─────────────────────────── JNI signatures ───────────────────────────

    @JvmStatic external fun adnet_ffi_version(): UInt
    @JvmStatic external fun adnet_ffi_free(buf: AdnetFfiBuffer)
    @JvmStatic external fun adnet_ffi_node_destroy(handle: AdnetFfiHandle?): Int

    @JvmStatic external fun adnet_ffi_node_create(
        dataDirPtr: ByteBuffer, dataDirLen: Int,
        out: AdnetFfiBuffer
    ): Int
    @JvmStatic external fun adnet_ffi_node_id(
        handle: AdnetFfiHandle,
        out: AdnetFfiBuffer
    ): Int
    @JvmStatic external fun adnet_ffi_hash_bytes(
    ...
    @JvmStatic external fun adnet_ffi_roster_add_contact(
        dataDirPtr: ByteBuffer, dataDirLen: Int,
        payloadPtr: ByteBuffer, payloadLen: Int,
        out: AdnetFfiBuffer
    ): Int

    @JvmStatic external fun adnet_ffi_roster_list_contacts(
        dataDirPtr: ByteBuffer, dataDirLen: Int,
        out: AdnetFfiBuffer
    ): Int

    @JvmStatic external fun adnet_ffi_roster_list_groups(
        dataDirPtr: ByteBuffer, dataDirLen: Int,
        out: AdnetFfiBuffer
    ): Int

    @JvmStatic external fun adnet_ffi_roster_search_contacts(
        dataDirPtr: ByteBuffer, dataDirLen: Int,
        queryPtr: ByteBuffer, queryLen: Int,
        out: AdnetFfiBuffer
    ): Int

    @JvmStatic external fun adnet_ffi_roster_delete_contact(
        dataDirPtr: ByteBuffer, dataDirLen: Int,
        contactIdPtr: ByteBuffer, contactIdLen: Int,
        out: AdnetFfiBuffer
    ): Int

    @JvmStatic external fun adnet_ffi_user_upsert_profile(
        dataDirPtr: ByteBuffer, dataDirLen: Int,
        payloadPtr: ByteBuffer, payloadLen: Int,
        out: AdnetFfiBuffer
    ): Int

    @JvmStatic external fun adnet_ffi_user_list_profiles(
        dataDirPtr: ByteBuffer, dataDirLen: Int,
        out: AdnetFfiBuffer
    ): Int

    @JvmStatic external fun adnet_ffi_user_get_profile(
        dataDirPtr: ByteBuffer, dataDirLen: Int,
        userIdPtr: ByteBuffer, userIdLen: Int,
        out: AdnetFfiBuffer
    ): Int

    @JvmStatic external fun adnet_ffi_user_ensure_digit(
        dataDirPtr: ByteBuffer, dataDirLen: Int,
        userIdPtr: ByteBuffer, userIdLen: Int,
        out: AdnetFfiBuffer
    ): Int
    ...
    class AdnetFfiBuffer(var ptr: ByteBuffer? = null, var len: Int = 0)

    class AdnetFfiHandle {
        // Opaque pointer cast through Long. The Rust side keeps
        // the typed `Box<AdnetFfiHandle>`; we never dereference
        // on the JVM side.
        internal var rawHandle: Long = 0

        override fun toString(): String = "AdnetFfiHandle(raw=0x${rawHandle.toString(16)})"
    }
    ...
    class ADNetFfiException(val code: Int, message: String) : RuntimeException("adnet-ffi[$code]: $message")

    // ─────────────────────────── Public Kotlin API ───────────────────────────

    /** Library version. The Android build can refuse to load a
     *  library whose version does not match its compile-time
     *  pin (typically read from a `BuildConfig.ADNET_FFI_VERSION`). */
    fun version(): UInt = adnet_ffi_version()

    /** Boot a node rooted at `dataDir`. */
    fun nodeCreate(dataDir: String): AdnetFfiHandle {
        val bytes = dataDir.toByteArray(StandardCharsets.UTF_8)
        val out = AdnetFfiBuffer()
        val status = bytes.useBytes { ptr, len -> adnet_ffi_node_create(ptr, len, out) }
        ...
        freeBuffer(out)
        return AdnetFfiHandle().apply { rawHandle = handleRaw }
    }

    fun nodeId(handle: AdnetFfiHandle): String {
        val out = AdnetFfiBuffer()
        checkStatus(adnet_ffi_node_id(handle, out), out)
        val json = readJsonObject(out)
    ...
    fun hashBytes(bytes: ByteArray): String {
        val out = AdnetFfiBuffer()
        val status = bytes.useBytes { ptr, len -> adnet_ffi_hash_bytes(ptr, len, out) }
        checkStatus(status, out)
        val json = readJsonObject(out)
        val value = json["value"]?.jsonObject ?: error("missing value")
    ...
    fun nodeDestroy(handle: AdnetFfiHandle?): Int =
        adnet_ffi_node_destroy(handle).also { /* handle is freed inside Rust */ }
    ...
    fun rosterAddContact(dataDir: String, payload: ByteArray) {
        ...
        val status = dataDir.toByteArray(StandardCharsets.UTF_8).useBytes { dp, dl ->
            payload.useBytes { pp, pl -> adnet_ffi_roster_add_contact(dp, dl, pp, pl, out) }
        }
        checkStatus(status, out)
        freeBuffer(out)
    }
    ...
    fun rosterListContacts(dataDir: String): String {
        val out = AdnetFfiBuffer()
        val status = dataDir.toByteArray(StandardCharsets.UTF_8).useBytes { dp, dl ->
            adnet_ffi_roster_list_contacts(dp, dl, out)
        }
        checkStatus(status, out)
    ...
    fun rosterListGroups(dataDir: String): String {
        val out = AdnetFfiBuffer()
        val status = dataDir.toByteArray(StandardCharsets.UTF_8).useBytes { dp, dl ->
            adnet_ffi_roster_list_groups(dp, dl, out)
        }
        checkStatus(status, out)
        val body = readJsonText(out)
        freeBuffer(out)
    ...
    fun rosterSearchContacts(dataDir: String, query: String): String {
        ...
        val status = dataDir.toByteArray(StandardCharsets.UTF_8).useBytes { dp, dl ->
            query.toByteArray(StandardCharsets.UTF_8).useBytes { qp, ql ->
                adnet_ffi_roster_search_contacts(dp, dl, qp, ql, out)
            }
        }
        checkStatus(status, out)
        val body = readJsonText(out)
    ...
    fun rosterDeleteContact(dataDir: String, contactId: String) {
        ...
        val status = dataDir.toByteArray(StandardCharsets.UTF_8).useBytes { dp, dl ->
            contactId.toByteArray(StandardCharsets.UTF_8).useBytes { idp, idl ->
                adnet_ffi_roster_delete_contact(dp, dl, idp, idl, out)
            }
        }
        checkStatus(status, out)
        freeBuffer(out)
    ...
    fun userUpsertProfile(dataDir: String, payload: ByteArray) {
        val out = AdnetFfiBuffer()
        val status = dataDir.toByteArray(StandardCharsets.UTF_8).useBytes { dp, dl ->
            payload.useBytes { pp, pl -> adnet_ffi_user_upsert_profile(dp, dl, pp, pl, out) }
        }
        checkStatus(status, out)
        freeBuffer(out)
    }
    ...
    fun userListProfiles(dataDir: String): String {
        val out = AdnetFfiBuffer()
        val status = dataDir.toByteArray(StandardCharsets.UTF_8).useBytes { dp, dl ->
            adnet_ffi_user_list_profiles(dp, dl, out)
        }
        checkStatus(status, out)
        val body = readJsonText(out)
        freeBuffer(out)
    ...
    fun userGetProfile(dataDir: String, userId: String): String {
        ...
        val status = dataDir.toByteArray(StandardCharsets.UTF_8).useBytes { dp, dl ->
            userId.toByteArray(StandardCharsets.UTF_8).useBytes { idp, idl ->
                adnet_ffi_user_get_profile(dp, dl, idp, idl, out)
            }
        }
        checkStatus(status, out)
        val body = readJsonText(out)
    ...
    fun userEnsureDigit(dataDir: String, userId: String): String {
        ...
        val status = dataDir.toByteArray(StandardCharsets.UTF_8).useBytes { dp, dl ->
            userId.toByteArray(StandardCharsets.UTF_8).useBytes { idp, idl ->
                adnet_ffi_user_ensure_digit(dp, dl, idp, idl, out)
            }
        }
        checkStatus(status, out)
        val json = readJsonObject(out)
        val digit = json["value"]?.jsonPrimitive?.content
            ?: throw ADNetFfiException(Status.JSON, "missing value in ensure_digit")
        freeBuffer(out)
        return digit
    }
    ...
    private fun checkStatus(status: Int, out: AdnetFfiBuffer) {
        if (status == Status.OK) return
        val msg = readErrorMessage(out)
        freeBuffer(out)
        throw ADNetFfiException(status, msg)
    }

    private fun readErrorMessage(buf: AdnetFfiBuffer): String {
    ...
    private fun readJsonObject(buf: AdnetFfiBuffer): JsonObject {
    ...
    private fun readJsonText(buf: AdnetFfiBuffer): String {
    ...
    private fun freeBuffer(buf: AdnetFfiBuffer) {
        if (buf.ptr != null) {
            adnet_ffi_free(buf)
            buf.ptr = null
            buf.len = 0
