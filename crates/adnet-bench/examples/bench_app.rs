//! Real-world example — run a configurable subset of `adnet-bench`
...
//!
//! Run with:
//!   cargo run -p adnet-bench --example bench_app --release -- --groups hashing,dht --out target/bench-report
//!
...
use adnet_bench::report::{BenchmarkResult, ThroughputMetric};
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    for group in &requested {
        match *group {
            "hashing" => adnet_bench::crypto::hashing::register(&mut criterion),
            "signing" => adnet_bench::crypto::signing::register(&mut criterion),
            "kdf" | "key_derivation" => adnet_bench::crypto::key_derivation::register(&mut criterion),
            "dht" => adnet_bench::network::dht::register(&mut criterion),
            "gossip" => adnet_bench::network::gossip::register(&mut criterion),
            "transport" => adnet_bench::network::transport::register(&mut criterion),
            "bao" => adnet_bench::storage::bao::register(&mut criterion),
            "blob" => adnet_bench::storage::blob::register(&mut criterion),
            other => {
...
) -> std::collections::HashMap<String, adnet_bench::report::Comparison> {
