//! `a3net-gateway` 最小示例：构造一份 `GatewayConfig`，挂上 metrics，
//! 展示如何把它和 `BitswapAppState` 组合起来（不实际启动 axum 监听）。
//!
//! 运行：`cargo run -p a3net-gateway --example gateway_basic`

use a3net_gateway::{bitswap_api::BitswapAppState, metrics::GatewayMetrics, GatewayConfig};
