//! Internal IPC service for the A3Net HTTP Gateway.
//!
...
//! `a3net-ipc`. Every request carries an `id`; notifications have no
//! `id`.
//!
//! ## Methods
//!
...
//! | `gateway.version` | `{}` | `{"version": "...", "agent": "a3net-gateway"}` |
...
use a3net_ipc::server::{JsonRpcServer, JsonRpcServerHandle, NotificationSender, RpcHandler};
...
/// Typed client wrapper around [`a3net_ipc::client::json_rpc_call`].
...
/// [`a3net_ipc::client::json_rpc_call`] for one-off calls.
#[derive(Clone, Debug)]
...
pub enum IpcClientError {
    ...
    Transport(#[from] a3net_ipc::client::JsonRpcError),
...
impl GatewayIpcClient {
    ...
    async fn call(&self, method: &str, params: Value) -> Result<Value, IpcClientError> {
        let r = a3net_ipc::client::json_rpc_call(
            self.socket(),
            "A3Net Gateway IPC",
            method,
            params,
