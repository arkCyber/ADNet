//! `a3net-gateway` — IPFS-compatible HTTP Gateway for A3Net.
//!
...
//! A3Net network using paths like `/ipfs/<cid>` and `/ipns/<name>`.
//!
//! ## Supported Endpoints
//!
...
//!
//! ```rust,ignore
//! use a3net_gateway::{GatewayConfig, GatewayServer};
...
pub mod metrics {
    ...
    /// the `a3net-observability` HTTP server so callers can expose
    ...
    /// their own bootstrap (e.g. `a3net serve` in the CLI). It
    ...
    ) -> ::std::io::Result<a3net_observability::http::MetricsServer> {
        a3net_observability::http::serve(a3net_observability::http::MetricsServerConfig {
            bind_addr,
