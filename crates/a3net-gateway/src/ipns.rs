...
impl IpnService {
    ...
    /// greater. Mirrors [`a3net_namespace::IpnResolver::cache_record`].
    fn insert_monotonic(
        &self,
...
mod tests {
    ...
    async fn resolve_via_dht_transport() {
        // We use the in-process DHT transport from a3net-namespace.
        use a3net_namespace::{DhtIpnTransport, IpnPublisher};
        use std::sync::Arc;

        let store = a3net_dht::store::new_in_memory_store();
        let transport: Arc<dyn a3net_namespace::IpnTransport> =
        ...
        let _blob_store_1 = Arc::new(
            a3net_blobstore::BlobStore::new(temp_dir_1.path()).unwrap(),
        );
        let secret = a3net_namespace::Ed25519SecretKey::generate();
        let name = secret.ipns_name();
        let publisher =
            IpnPublisher::new(Arc::new(secret) as Arc<dyn a3net_namespace::SecretKey>);
        ...
        let blob_store_2 = Arc::new(
            a3net_blobstore::BlobStore::new(temp_dir_2.path()).unwrap(),
        );
    ...
    async fn publish_fans_out_to_all_transports() {
        use a3net_namespace::IpnTransport;
        use a3net_namespace::TransportHealth;
        use std::pin::Pin;
        ...
        impl IpnTransport for CountingTransport {
            ...
            async fn publish(
                &self,
                _record: &a3net_namespace::IpnRecord,
            ) -> Result<(), a3net_namespace::IpnsError> {
            ...
            async fn subscribe(
                ...
                Pin<
                    Box<
                        dyn futures::Stream<
                                Item = Result<a3net_namespace::IpnRecord, a3net_namespace::IpnsError>,
                            > + Send,
                    >,
                >,
                a3net_namespace::IpnsError,
            > {
                Ok(Box::pin(futures::stream::empty()))
            }
            async fn health(&self) -> Result<TransportHealth, a3net_namespace::IpnsError> {
