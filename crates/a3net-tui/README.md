...
> Zero-dependency terminal UI primitives for A3Net CLI — ASCII boxes, progress bars, colors and i18n.

## 概览 (Overview)

`a3net-tui` is the rendering layer that every A3Net command-line tool
...
`use a3net_tui::*;` import for the common widgets.
...
`a3net-tui` is a workspace-internal crate. It is consumed by
`a3net-cli` (the user-facing CLI). To depend on it from a new
workspace crate:

```toml
[dependencies]
a3net-tui = { workspace = true }
```
...
```rust
use a3net_tui::{Table, widget::status_widget};
...
```rust
use a3net_tui::i18n::{set_locale, Locale, t};

set_locale(Locale::ZhCn);
println!("{}", t("status.title"));     // "A3Net 节点状态"
println!("{}", t("wizard.save_success")); // "配置已保存到 {0}"
```

## 应用案例 (Use Cases / Examples)

- **`a3net-cli` status screen** — `Box::with_title` + `add_field` renders
