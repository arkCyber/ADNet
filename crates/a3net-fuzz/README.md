...
> Coverage-guided fuzzing for A3Net's wire-format parsers — `cargo fuzz` harness with six ready-to-run targets.

## 概览 (Overview)

`a3net-fuzz` is the security/perimeter testing layer for A3Net. Every
...
(`src/support.rs`, exposed as `a3net_fuzz_support`) keeps the
...
- **Compile-time hardening** — `[profile.release]` for the fuzz
  ...
  inlining / coverage.
- **Library stub** — `a3net_fuzz_support` (the `[lib]` target)
...
`a3net-fuzz` is a workspace-internal crate — it is **not** published.
...
- **Parser coverage** — combine `cargo fuzz coverage` output with
  the `a3net-types` and `a3net-blobstore` source map to find
