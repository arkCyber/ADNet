//! End-to-end iroh integration tests for `a3net-node`.
//!
//! Two layers of coverage:
//!
//! 1. **Single-node smoke tests** — verify each subsystem
//!    (`a3net_store`, `IrohGossipTransport`, `IrohDocsChat`) wires
...
use a3net_transport::iroh::{IrohIdentity, public_key_to_node_id};
use a3net_types::{Announcement, CdnContentKind, ContentHash, RoomId};
use chrono::Utc;
use iroh::Endpoint;
        ...
        let _ = tracing_subscriber::fmt()
            .with_env_filter(
                EnvFilter::try_from_default_env()
                    .unwrap_or_else(|_| EnvFilter::new("warn,a3net_node=info,a3net_blobstore=info,a3net_gossip=info,a3net_chatstore=info,iroh=warn")),
            )
            .with_test_writer()
            .try_init();
    });
...
async fn single_node_iroh_runtime_wires_transport() -> anyhow::Result<()> {
    ...
    let hash = {
        let store = runtime.a3net_store.clone();
        BlobImporter::put_bytes(&store, &payload).await?
    };
    let roundtrip = {
        let store = runtime.a3net_store.clone();
        a3net_blobstore::BlobReader::read_all(&store, &hash).await?
    };
...
async fn ticket_round_trip_via_iroh() -> anyhow::Result<()> {
    ...
    let hash: ContentHash = {
        let store = producer.a3net_store.clone();
        BlobImporter::put_bytes(&store, &payload).await?
    };

    // Build a BlobTicket pointing at the producer's endpoint.
    use a3net_blobstore::{IrohBlobTicket, content_hash_to_iroh_hash};
...
async fn blob_resume_after_disconnect() -> anyhow::Result<()> {
    ...
    let iroh_hash = a3net_blobstore::content_hash_to_iroh_hash(&hash)?;
...
async fn docs_two_node_sync_round_trip() -> anyhow::Result<()> {
    ...
    let ticket = alice_bridge
        .share_with_addr_options(
            "docs-e2e-conv",
            a3net_chatstore::IrohShareMode::Write,
...
async fn user_data_surfaces_on_both_runtimes() -> anyhow::Result<()> {
    ...
    use a3net_transport::iroh::discovery::{DiscoveryDiagnostics, UserData};
    ...
    let alice_ud = UserData::new("a3net/role=alice").unwrap();
    let bob_ud = UserData::new("a3net/role=bob").unwrap();
...
async fn runtime_without_user_data_is_none() -> anyhow::Result<()> {
    use a3net_transport::iroh::IrohIdentity;
    use a3net_transport::iroh::discovery::DiscoveryDiagnostics;
...
async fn user_data_constructor_rejects_oversized_payload() {
    use a3net_transport::iroh::discovery::{USER_DATA_MAX_LEN, UserData};
