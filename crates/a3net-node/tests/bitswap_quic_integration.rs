...
async fn test_alpn_rejects_mismatched_alpn() {
    ...
    match err {
        a3net_node::bitswap_transport::BitswapTransportError::AlpnMismatch {
            expected,
            actual,
        } => {
...
async fn test_want_block_full_flow() {
    ...
    let hash = a3net_types::ContentHash::from_bytes(b"data-block");
    ...
    let block_msg = a3net_blobstore::BitswapMessage::Block {
        block: hash.clone(),
...
async fn test_want_block_dont_have_path() {
    ...
    let hash = a3net_types::ContentHash::from_bytes(b"never-arrives");
    ...
    run_tx
        .send(BitswapEvent::MessageFrom {
            peer: peer.clone(),
            msg: a3net_blobstore::BitswapMessage::DontHave { block: hash.clone() },
        })
        .await
        .unwrap();
...
async fn test_max_message_size() {
    ...
    let res = adapter
        .send(
            &peer,
            a3net_blobstore::BitswapMessage::Block {
                block: a3net_types::ContentHash::from_bytes(b"x"),
                data: big,
            },
        )
        .await;
    assert!(matches!(
        res,
        Err(a3net_node::bitswap_transport::BitswapTransportError::MessageTooLarge { .. })
    ));
}
...
async fn test_send_want_block_and_wait_full_flow() {
    ...
    use a3net_node::bitswap_transport::BitswapBlockOutcome;
    ...
    let hash = a3net_types::ContentHash::from_bytes(b"want-block-full-flow");
...
async fn test_quic_bitswap_message_dispatch() {
    ...
    let hash = a3net_types::ContentHash::from_bytes(b"e2e-block");
    let (tx_waker, rx_waker) = tokio::sync::oneshot::channel();
    pending_b
        .write()
        .await
    ...
    let msg = a3net_blobstore::BitswapMessage::Block {
        block: hash.clone(),
...
async fn test_quic_handshake_blocks_pre_hello_writes() {
    ...
    let hash = a3net_types::ContentHash::from_bytes(b"pre-hello");
    adapter_a
