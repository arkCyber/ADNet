//! Two A3Net nodes chatting through a shared `InProcessGossip`.
//!
...
//! backends that `a3net-node` can use for "node A sends message X to
//! node B":
//!
...
//!   [`a3net_types::Announcement`] (this is what gossip carries).
//!
...
//! `--features iroh` flag in `a3net-gossip`). Since this demo stays on
...
//!
//! ```bash
//! cargo run -p a3net-node --example chat_via_gossip
//! ```
//!
...
//! mutual transport-identity verification using `a3net-pairing`:
//!
...
//!   `IrohGossipTransport` (`a3net-gossip` with `--features iroh`).
...
use a3net_types::{Announcement as CdnAnnouncement, CdnContentKind, ContentHash, NodeId, RoomId};
...
async fn main() -> Result<()> {
    tracing_subscriber::fmt()
        ...
        .with_env_filter(
            tracing_subscriber::EnvFilter::try_from_default_env()
                .unwrap_or_else(|_| "info,a3net=info".into()),
        )
        .init();
    ...
    let alice_invitation = SignedInvitation::create(
        &alice_id_from_key,
        &a3net_identity::wallet::Wallet::generate(),
    ...
    let alice_response = PairingResponseBuilder {
        ...
        ttl_seconds: 0,
        issuer_wallet: &a3net_identity::wallet::Wallet::generate(),
    }
    .build()
...
async fn publish_pair_envelope(
    ...
    let ann = CdnAnnouncement {
        ...
        mime_type: Some("application/x-a3net-pairing".into()),
        source_url: None,
        ticket: None,
        timestamp: Utc::now(),
        signer: None,
...
/// Write a `TrustedDeviceRecord` to $TMPDIR/a3net_gossip_pairing/ so
