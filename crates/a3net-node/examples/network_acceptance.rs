//! Real-network acceptance probe for A3Net's iroh transport stack.
//!
...
//!
//! Build with:
//! `cargo build -p a3net-node --features iroh --example network_acceptance --release`
...
use a3net_chatstore::Message;
use a3net_transport::frame::{Frame, FrameCodec};
use a3net_transport::iroh::{ADNET_FRAME_ALPN, IrohIdentity};
...
async fn serve(args: &CliArgs) -> Result<()> {
    ...
    let blob_payload = format!("a3net-real-network-acceptance-blob-{run_id}");
...
async fn blob_roundtrip(
    ...
    let bytes = BlobReader::read_all(
        &runtime.a3net_store,
        &a3net_blobstore::iroh_hash_to_content_hash(&ticket.hash()),
    )
    .await?;
