//! Two A3Net nodes that chat with each other over real QUIC, with
//! **mutual transport-identity verification** on top.
//!
//! This example answers two questions:
//!
//!  1. "Can two A3Net nodes send arbitrary framed messages to each
...
//!     connection. The issuer prints an `a3net-pairing://` QR (or a
...
//! cargo run -p a3net-node --example chat_two_nodes -- serve \
//!     --bind 127.0.0.1:0 \
//!     --name alice \
//!     --qr-out /tmp/alice-pairing.svg
//!
//! # Terminal B — point at A's pairing QR
//! cargo run -p a3net-node --example chat_two_nodes -- call \
...
//!   `<name>: <text>\n`.
//! - QR rendering is delegated to `a3net-qr::create_qr_svg` (qrcodegen,
//!   chatmail-compatible).
...
#[command(
    name = "chat_two_nodes",
    about = "Two A3Net nodes chatting over real QUIC, with mutual pairing verification."
)]
struct Cli {
    #[command(subcommand)]
    cmd: Cmd,
...
enum Cmd {
    ...
    Serve {
        ...
        /// node prints the canonical `a3net-pairing://…` URL too. The
    ...
    tracing_subscriber::fmt()
        ...
        .with_env_filter(
            tracing_subscriber::EnvFilter::try_from_default_env()
                .unwrap_or_else(|_| "info,a3net=info,quinn=warn".into()),
        )
        .init();
...
impl ChatNode {
    ...
    async fn call_peer_with_pairing(
        self: Arc<Self>,
        remote: &str,
        invitation: &a3net_pairing::SignedInvitation,
    ) -> Result<()> {
...
async fn issue_pairing_invitation(
    ...
    wallet: &Wallet,
    note: Option<String>,
) -> Result<a3net_pairing::SignedInvitation> {
    let caps = CapabilitySet::from_names(["chat"]);
    let inv = a3net_pairing::SignedInvitation::create(&issuer_node, wallet, caps, 900, note)
...
async fn run_serve(name: String, bind: SocketAddr, qr_out: Option<PathBuf>) -> Result<()> {
    ...
    let invitation = if qr_out.is_some() {
        ...
        println!("    cargo run -p a3net-node --example chat_two_nodes -- call \\");
...
async fn run_call(
    ...
    let invitation = if let Some(path) = pair_qr {
        ...
        let url = if url_file.exists() {
            ...
            // Fallback: scan any `a3net-pairing://` literal in the
...
fn extract_url_from_svg(svg: &str) -> Option<String> {
    // Look for the literal `a3net-pairing://` substring; return
    // everything up to the first non-URL char.
    const PREFIX: &str = "a3net-pairing://";
...
fn _unused_rng_marker() {
    // Touch rand so the dep stays honest; a3net-pairing generates
