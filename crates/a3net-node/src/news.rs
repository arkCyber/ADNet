//! News + authoritative announcement seam for `a3net-node`.
//!
//! Wraps [`a3net_news::NewsService`] so callers can:
//!
...
use a3net_types::{BulletinId, NodeId, RoomId};
use thiserror::Error;
...
impl Default for NewsNodeConfig {
    fn default() -> Self {
        Self {
            storage_dir: std::env::temp_dir().join("a3net-news"),
...
impl NewsHandle {
    ...
    pub fn timeline(
        ...
        limit: usize,
    ) -> Result<Vec<a3net_news::StoredBulletin>, NewsNodeError> {
        self.inner
    ...
    pub fn get(
        ...
        room: &RoomId,
        id: &BulletinId,
    ) -> Result<Option<a3net_news::StoredBulletin>, NewsNodeError> {
...
fn news_to_seam_error(e: a3net_news::NewsError) -> NewsNodeError {
    use a3net_news::NewsError;
    match e {
