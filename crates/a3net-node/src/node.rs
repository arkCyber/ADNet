...
use a3net_ipc::validation::ValidationPolicy;
use a3net_mesh::MeshConfig;
use a3net_mesh::MeshServerHandle;
use a3net_relay::{RelayConfig, RelayServerHandle};
use a3net_resilience::{
    CancellationScope, PeerLimiter, ResourceLimiter, RoomLimiter, TagLimiter, default_peer_config,
    default_room_config, default_tag_config,
};
use a3net_transport::{
...
use a3net_workspace::{WORKSPACE_ROOM_ID, Workspace, WorkspaceFileEntry};
use anyhow::Result;
use chrono::Utc;
...
pub struct NodeConfig {
    ...
    /// `a3net_ipc::validation` module for the rationale.
...
impl NodeConfig {
    pub fn new(data_dir: impl Into<PathBuf>, node_id: NodeId) -> Self {
        let display_name = format!("a3net-{}", node_id.short());
        Self {
    ...
    pub fn load_or_create(data_dir: impl Into<PathBuf>) -> Result<Self> {
        ...
        let (node_id, quic_identity) = if pem_path.exists() {
            ...
            let node_id = a3net_transport::derive_node_id_from_cert(identity.cert_der())
...
pub struct NodeBuilder {
    ...
    /// Peer-manager config. When `Some`, the node will instantiate
    /// a [`crate::peer_manager::PeerManager`] and start the
    /// auto-heartbeat service. When `None` (the default), peer
    /// liveness is untracked. Set via
    /// [`NodeBuilder::with_peer_manager_config`].
    peer_manager_config: Option<crate::peer_manager::PeerManagerConfig>,
    /// Inbound heartbeat dispatcher. When the transport receives
    /// a `HeartbeatMessage`, it calls this closure to apply it to
    /// the node's peer manager. The dispatcher is responsible
    /// for calling `HeartbeatService::on_heartbeat_received` so
    /// RTT accounting happens. When `None` and a peer-manager is
    /// configured, the node wires a passthrough dispatcher that
    /// only updates the local peer table (no transport send).
    heartbeat_sender: Option<Arc<dyn crate::heartbeat_service::HeartbeatSender>>,
}
...
impl NodeBuilder {
    pub fn new(config: NodeConfig) -> Self {
        Self {
            ...
            #[cfg(feature = "dht")]
            auto_init_ipns: None,
            peer_manager_config: None,
            heartbeat_sender: None,
        }
    }

    /// Configure the bounded peer-manager. When set, the node will
    /// instantiate a [`crate::peer_manager::PeerManager`] and, if
    /// `auto_heartbeat` is `true` in the config, an auto-heartbeat
    /// background task.
    pub fn with_peer_manager_config(
        mut self,
        cfg: crate::peer_manager::PeerManagerConfig,
    ) -> Self {
        self.peer_manager_config = Some(cfg);
        self
    }

    /// Configure the outbound heartbeat dispatcher. Pair with
    /// [`NodeBuilder::with_peer_manager_config`] so the auto-tick
    /// has somewhere to send pings. The sender is typically
    /// implemented by the transport layer.
    pub fn with_heartbeat_sender(
        mut self,
        sender: Arc<dyn crate::heartbeat_service::HeartbeatSender>,
    ) -> Self {
    ...
    /// 1. Wrap the local [`BlobStore`](a3net_blobstore::BlobStore)
    ///    with a [`NodeBlockStore`](crate::graphsync::NodeBlockStore)
    ///    so it satisfies the
    ///    [`a3net_types::graphsync::BlockStore`] trait.
    ...
    /// `a3net/frame/1` ALPN is already registered on its router;
    /// this method hands the matching incoming-channel receiver
    /// to an [`IrohTransport`](a3net_transport::IrohTransport) that
    ...
    /// [`IrohIdentity::load_or_create`](a3net_transport::iroh::IrohIdentity::load_or_create)
    ...
    pub async fn with_iroh_runtime_from_data_dir(
        ...
        identity: &a3net_transport::iroh::IrohIdentity,
    ...
    /// [`a3net_transport::iroh::discovery::UserData`].
    ///
    ...
    /// [`a3net_transport::iroh::discovery::UserData::new`]; an
    ...
    async fn with_iroh_runtime_from_data_dir_inner(
        ...
        if let Some(ud) = &user_data
            && ud.len() > a3net_transport::iroh::discovery::USER_DATA_MAX_LEN
        {
            anyhow::bail!(
                "user_data length {} exceeds {} bytes (cap is {})",
                ud.len(),
                a3net_transport::iroh::discovery::USER_DATA_MAX_LEN,
                a3net_transport::iroh::discovery::USER_DATA_MAX_LEN,
            );
        }
        ...
        if self_mut.config.node_id != aligned_node_id {
            ...
            self_mut.config.display_name = format!("a3net-{}", aligned_node_id.short());
        }
    ...
    /// to `a3net-room-{WORKSPACE_ROOM_ID}`, and `publish_to_workspace`
    ...
    pub async fn build_with_bus(mut self, bus: GossipBus) -> Result<Node> {
        ...
        let transport: Option<SharedTransport> = if let Some(runtime_ref) = runtime.as_mut() {
            ...
            let transport = a3net_transport::IrohTransport::with_endpoint(endpoint, frame_rx);
        ...
        let bus = if let Some(runtime_ref) = runtime.as_ref() {
            let local_node =
                a3net_transport::iroh::public_key_to_node_id(&runtime_ref.endpoint().id());
        ...
        if let Some(t) = &transport {
            ...
            if cfg.node_id != transport_id {
                ...
                cfg.display_name = format!("a3net-{}", cfg.node_id.short());
            }
        }
        let store = Arc::new(
        ...
        let relay = if let Some(mut cfg) = self.relay_config.clone() {
            ...
            if cfg.serve_enabled {
                ...
                let policy = a3net_relay::ServerPolicy::from_config(&cfg);
                match a3net_relay::RelayServer::start_with_policy(
                    &cfg.serve_bind,
                    cfg.serve_port,
                    billing_mode,
                    policy,
        ...
        if let Some(transport) = &self.transport {
            ...
            tokio::spawn(async move {
                ...
                while let Some((peer_id, mut conn)) = rx.recv().await {
                    ...
                    tokio::spawn(async move {
                        let r = a3net_transport::serve_blob_request(&mut conn, &s).await;
                        if let Err(e) = r {
        ...
        {
            use a3net_observability::health::{register_health_check, HealthCheck, HealthCheckError};
            use std::pin::Pin;
                ...
                impl HealthCheck for TransportHealthCheck {
                    ...
                    fn check(self: std::sync::Arc<Self>) -> Pin<Box<dyn std::future::Future<Output = Result<(), HealthCheckError>> + Send + 'static>> {
                        ...
                        Box::pin(async move {
                            ...
                            let t_ref: &dyn a3net_transport::Transport = &*t;
                            t_ref.health_check()
        ...
        // the global `a3net-observability` registry. The task
        ...
        if let Some(runtime) = runtime.as_ref() {
            if let Some(diag) = runtime.clone_diagnostics() {
                ...
                tokio::spawn(async move {
                    ...
                    use a3net_transport::iroh::metrics_bridge::{
                        publish_discovery_metrics, publish_endpoint_metrics_into,
                    };
                    let endpoint_metrics = a3net_observability::bridge::ENDPOINT.clone();
                    let recorder = std::sync::Arc::new(
                        a3net_transport::iroh::endpoint_diagnostics::EndpointDiagnosticsRecorder::new(8),
                    );
                    ...
                    loop {
                        ...
                        let snap = a3net_transport::iroh::endpoint_diagnostics::snapshot_endpoint(
                            &endpoint_for_snap,
                            None,
                        );
        ...
        // Capture data_dir + node_id before we move `cfg` into the
        // Node — we need them to initialise the identity store and
        // contacts manager.
        let data_dir = cfg.data_dir.clone();
        let node_id_for_identity = cfg.node_id.clone();

        // P1-P2P: bound the peer-manager table and (when auto is
        // enabled) spawn the heartbeat background task. The task
        // uses the local node identity derived from the config so
        // its messages carry the same node_id that the transport
        // exposes.
        let pm_for_node: Option<Arc<crate::peer_manager::PeerManager>> =
            self.peer_manager_config.clone().map(crate::peer_manager::PeerManager::new);
        let mut hh_for_node: Option<crate::heartbeat_service::HeartbeatHandle> = None;
        if let Some(pm) = pm_for_node.as_ref() {
            if pm.config().auto_heartbeat {
                let sender = self
                    .heartbeat_sender
                    .clone()
                    .unwrap_or_else(|| Arc::new(crate::heartbeat_service::MockHeartbeatSender::new()));
                let svc = Arc::new(crate::heartbeat_service::HeartbeatService::new(
                    pm.clone(),
                    sender,
                    cfg.node_id.clone(),
        ...
        let mut node = Node {
            ...
            #[cfg(feature = "graphsync")]
            graphsync: Arc::new(parking_lot::Mutex::new(None)),
            #[cfg(feature = "agent")]
            agent: crate::agent::NodeAgentBridge::new(),
            // V14: persistent identity + contacts. Both loaders are
            // best-effort: a missing file creates an empty /
            // placeholder document, a malformed file is fatal so
            // the operator must intervene. The data dir is created
            // idempotently by each loader.
            identity: NodeIdentityStore::open(&data_dir, node_id_for_identity)
                .map_err(|e| anyhow::anyhow!("failed to open node identity store: {e}"))?,
            contacts: ContactsManager::open(data_dir.as_path()).map_err(|e| {
                anyhow::anyhow!("failed to open contacts manager: {e}")
            })?,
            // P1-3: every node owns a fresh cancellation scope.
            // Long-lived background tasks can register with
            // `node.cancel_scope().spawn(...)` and the token clones
            // are observed in their `select!` arms.  `Node::shutdown`
            // triggers `scope.cancel()` + `scope.join()` to guarantee
            // a bounded exit.
            cancel_scope: Arc::new(CancellationScope::new()),
            // P1-5: global concurrency limiters.  Limiters are
            // intentionally not configurable from `NodeConfig`
            // yet — defaults are sized for a small (≤ 16-core) node
            // and can be tuned via the accessor methods below.
            peer_limiter: Arc::new(ResourceLimiter::new(default_peer_config())),
            room_limiter: Arc::new(ResourceLimiter::new(default_room_config())),
            tag_limiter: Arc::new(ResourceLimiter::new(default_tag_config())),
            // P1-P2P: peer manager + heartbeat. When the builder
            // was configured with `with_peer_manager_config`, the
            // `PeerManager` is created and the auto-heartbeat
            // service (if enabled) is spawned as a background task.
            // The task is cleaned up by `Node::shutdown`.
            peer_manager: pm_for_node.clone(),
            heartbeat_handle: Arc::new(parking_lot::Mutex::new(hh_for_node.take())),
        };
...
/// `a3net_types::Endpoint` because the UI only needs the routable string.
...
pub type IncomingConn = (NodeId, Box<dyn a3net_transport::OutgoingConnection>);
...
pub struct RemoteWorkspaceEntry {
    ...
    pub ticket: Option<a3net_types::BlobTicket>,
...
pub struct Node {
    ...
    /// `a3net-room-{WORKSPACE_ROOM_ID}`. Keyed by owning node id.
    ...
    blob_store_refresh_handle:
        Arc<tokio::sync::Mutex<Option<a3net_blobstore::BlobStoreHandle>>>,
    ...
    /// AI-agent bridge (agent + tool registry + ACL). Present when
    /// the `agent` feature is enabled. Built by
    /// [`NodeBuilder::with_agent_bridge`] or lazily created.
    #[cfg(feature = "agent")]
    agent: crate::agent::NodeAgentBridge,
    /// Persistent node-identity store. Loaded from
    /// `{data_dir}/node_identity.json` (created with a placeholder
    /// if absent) during [`NodeBuilder::build_with_bus`]. Every
    /// mutation goes through the typed setters on
    /// [`NodeIdentityStore`], which re-validate the invariants
    /// before writing to disk. Cheap to clone (internally
    /// `Arc<RwLock<…>>`).
    identity: NodeIdentityStore,
    /// Persistent contacts list manager. Loaded from
    /// `{data_dir}/contacts.json` (created empty if absent) during
    /// [`NodeBuilder::build_with_bus`]. Mutations are atomic —
    /// either the full new list reaches disk or nothing changes.
    /// Cheap to clone.
    contacts: ContactsManager,
    /// Coordinated shutdown scope.  Every long-lived background task
    /// that the node spawns should be tracked here so `shutdown()` can
    /// fire a cancellation token and drain the tasks with a bounded
    /// timeout.  See [`CancellationScope`] for details.
    cancel_scope: Arc<CancellationScope>,
    /// P1-5: per-peer concurrency cap (256 global / 16 per peer).
    /// Inbound mesh fetches, DHT lookups, and other per-peer work
    /// should acquire a permit from this limiter before proceeding;
    /// without it, a single hostile / buggy peer can pin the entire
    /// node.
    peer_limiter: Arc<PeerLimiter>,
    /// P1-5: per-room gossip fan-out cap (64 global / 32 per room).
    /// A single hot room must not be able to starve every other
    /// subscription.
    room_limiter: Arc<RoomLimiter>,
    /// P1-5: catch-all tag-bucketed limiter (512 global / 64 per tag)
    /// for subsystems that don't fit a peer or room key (e.g.
    /// `blobstore.fetch`, `relay.proxy`).
    tag_limiter: Arc<TagLimiter>,
    /// Bounded P2P peer table with heartbeat-based liveness.
    /// `Some` only when the builder was configured with
    /// [`NodeBuilder::with_peer_manager_config`].
    #[allow(dead_code)] // surfaced via `Node::peer_manager` accessors
    peer_manager: Option<Arc<crate::peer_manager::PeerManager>>,
    /// Auto-heartbeat service that drives periodic pings to every
    /// alive peer. `Some` only when peer-manager `auto_heartbeat`
    /// is enabled and the builder has wired the service in.
    /// Stored behind a `Mutex` so `Node::shutdown` can take it out
    /// through `&self`.
    #[allow(dead_code)] // surfaced via `Node::heartbeat_handle` accessor
    heartbeat_handle: Arc<parking_lot::Mutex<Option<crate::heartbeat_service::HeartbeatHandle>>>,
}
...
impl Node {
    ...
    /// P1-3: access the node's cancellation scope.  Long-lived
    /// background tasks should register with
    /// [`CancellationScope::spawn`] and observe the token via
    /// [`a3net_resilience::CancellationToken::cancelled`] in their
    /// `select!` arms.  `Node::shutdown` fires the scope's cancel
    /// signal and joins tracked tasks with a bounded timeout.
    pub fn cancel_scope(&self) -> &Arc<CancellationScope> {
        &self.cancel_scope
    }

    /// Peer-manager handle. `Some` only when the builder was
    /// configured with [`NodeBuilder::with_peer_manager_config`].
    /// Returns `None` otherwise so callers can detect "liveness
    /// tracking is off" without unwrapping.
    pub fn peer_manager(&self) -> Option<&Arc<crate::peer_manager::PeerManager>> {
        self.peer_manager.as_ref()
    }

    /// P1-5: per-peer concurrency limiter (256 global / 16 per
    /// peer by default).  Use this for any work that is keyed by
    /// a remote peer's `NodeId` — inbound mesh fetches, DHT
    /// queries, gossip handshakes.
    pub fn peer_limiter(&self) -> &Arc<PeerLimiter> {
        &self.peer_limiter
    }

    /// P1-5: per-room gossip fan-out limiter (64 global / 32 per
    /// room by default).  Use this for any work that is keyed by
    /// a `RoomId` — announce fan-out, room-feed rebuilds, broadcast
    /// acknowledgements.
    pub fn room_limiter(&self) -> &Arc<RoomLimiter> {
        &self.room_limiter
    }

    /// P1-5: catch-all tag-bucketed limiter (512 global / 64 per
    /// tag by default) for subsystems that don't fit a peer or
    /// room key (e.g. `blobstore.fetch`, `relay.proxy`).
    pub fn tag_limiter(&self) -> &Arc<TagLimiter> {
        &self.tag_limiter
    }

    // ── Agent bridge ─────────────────────────────────────────────────────────

    /// Register a [`a3net_agent::ChatModel`] (e.g. an OpenAI-compatible
    /// client, a local LLM, a remote Hermes adapter, etc.) so the node
    /// can act as an AI agent.
    ///
    /// Use [`Self::agent_bridge`] to also register tools on the same
    /// bridge.
    ///
    /// Only available when the `agent` feature is enabled.
    #[cfg(feature = "agent")]
    pub fn register_agent(&self, model: Arc<dyn a3net_agent::ChatModel>) {
        self.agent.register_model(model);
    }

    /// Return the node's agent endpoint for the IPC / RPC layer.
    ///
    /// Only available when the `agent` feature is enabled.
    #[cfg(feature = "agent")]
    pub fn agent_endpoint(&self) -> crate::agent::AgentEndpoint {
        crate::agent::AgentEndpoint::new(self.agent.clone())
    }

    /// Access the agent bridge directly (advanced use — prefer
    /// [`Self::agent_endpoint`]).
    ///
    /// Only available when the `agent` feature is enabled.
    #[cfg(feature = "agent")]
    pub fn agent_bridge(&self) -> &crate::agent::NodeAgentBridge {
        &self.agent
    }
    ...
    pub async fn init_dht(&self, cfg: crate::dht::DhtConfig) -> anyhow::Result<()> {
        ...
        let bridge: Arc<dyn a3net_dht::transport::TransportBridge> =
    ...
    pub async fn init_ipns(
        ...
        secret_key: Option<Arc<dyn a3net_namespace::SecretKey>>,
    ) -> anyhow::Result<()> {
        let dht = self
            .dht
            .read()
        ...
        if let Some(key) = secret_key {
            ...
            let backend: Arc<dyn a3net_namespace::transport::dht::DhtBackend> =
                Arc::new(a3net_namespace::transport::dht::DhtQueryBackend::new(query_arc));
            let transport: Arc<dyn a3net_namespace::transport::IpnTransport> =
                Arc::new(a3net_namespace::transport::dht::DhtIpnTransport::new(backend));
    ...
    /// Persistent handle to the local node's self-description
    /// (email / 12-digit DNS id / nickname / avatar / 128-char
    /// description / wallet). Mutations are atomic on disk.
    /// Cheap to clone.
    pub fn identity(&self) -> &NodeIdentityStore {
        &self.identity
    }

    /// Persistent handle to the local node's contacts list
    /// (manual + gossip-synced, capped at 5k entries). Cheap to
    /// clone.
    pub fn contacts(&self) -> &ContactsManager {
        &self.contacts
    }

    /// Snapshot the local identity in a form suitable for the
    /// [`NodeIdentityCard`] gossip frame. Combines the
    /// [`NodeIdentity`] with the local node's [`NodeProfile`] and
    /// the BLAKE3 digest of the local contacts list so peers can
    /// decide whether to request a sync.
    pub fn identity_card(&self) -> a3net_types::NodeIdentityCard {
        let identity = self.identity.snapshot();
        let profile = a3net_types::NodeProfile::standard(
            self.cfg.node_id.clone(),
            env!("CARGO_PKG_VERSION"),
        );
        let contacts_digest = self.contacts.digest();
        a3net_types::NodeIdentityCard::new(identity, Some(profile), Some(contacts_digest))
    }

    /// Render the local node's identity + reputation summary as a
    /// self-contained HTML5 document. The output is suitable for
    /// both `a3net profile render <out.html>` (write to a file the
    /// operator serves with any web server) and the embedded HTTP
    /// profile endpoint.
    ///
    /// The rendered page embeds an inline CSS stylesheet and
    /// (when the avatar is the `Data` variant) a base64-encoded
    /// `data:` URI — no external CDN, no JS, safe to host on
    /// any static file server.
    pub fn render_profile_html(&self) -> String {
        let identity = self.identity.snapshot();
        let profile = a3net_types::NodeProfile::standard(
            self.cfg.node_id.clone(),
            env!("CARGO_PKG_VERSION"),
        );
        let rep = self.contacts.reputation_summary();
        let count = self.contacts.len();
        let inputs = crate::profile_page::ProfilePageInputs::new(
            &identity,
            Some(&profile),
            rep,
            count,
        );
        crate::profile_page::render_profile_html(&inputs)
    }

    /// Start an embedded HTTP server that exposes the profile
    /// page at `http://{addr}/profile`. Returns a handle whose
    /// `local_addr()` reports the bound address and whose
    /// `shutdown()` triggers a graceful stop.
    pub async fn start_profile_server(
        self: Arc<Self>,
    ) -> std::io::Result<(
        crate::profile_server::ProfileServerHandle,
        tokio::task::JoinHandle<()>,
    )> {
        crate::profile_server::ProfileServerBuilder::new()
            .start(self)
            .await
    }
    ...
    /// QUIC blob fetch in `a3net_node::download`).
    ...
    /// iroh runtime.
    #[cfg(feature = "iroh")]
    pub async fn chat_bridge(&self) -> Option<a3net_chatstore::IrohDocsChat> {
    ...
    /// `a3net-room-{WORKSPACE_ROOM_ID}` gossip topic. Remote nodes
    ...
    pub async fn ensure_mesh(&self) -> Result<NodeAddr> {
        ...
        if let Some(h) = guard.as_ref() {
            return Ok(NodeAddr::new(self.cfg.node_id.clone())
                .with_direct(a3net_types::Endpoint::new(&h.host, h.port)));
        }
        let handle = a3net_mesh::MeshServer::start(self.store.clone())
            .await
            .map_err(|e| anyhow::anyhow!("mesh start: {e}"))?;
        let endpoint = a3net_types::Endpoint::new(&handle.host, handle.port);
        let addr = NodeAddr::new(self.cfg.node_id.clone()).with_direct(endpoint);
        *guard = Some(handle);
        Ok(addr)
    }

    pub fn mesh_endpoint(&self) -> Option<a3net_types::Endpoint> {
        self.mesh.try_lock().ok().and_then(|g| {
            g.as_ref()
                .map(|h| a3net_types::Endpoint::new(&h.host, h.port))
        })
    }
    ...
    /// bind.
    pub async fn relay_info(&self) -> Option<a3net_relay::RelayServerInfo> {
    ...
    pub async fn ensure_relay(&self) -> Result<String> {
        ...
        let billing_mode = cfg.billing_mode();
        let handle = a3net_relay::RelayServer::start(&cfg.serve_bind, cfg.serve_port, billing_mode)
            .await
    ...
    /// [`a3net_transport::serve_blob_request`]).
    ...
    pub async fn shutdown(&self) -> Result<()> {
        // P1-3: flip the cancellation token first so any tracked
        // background task observing it can start winding down in
        // parallel with the synchronous teardown below.  We don't
        // block on `scope.join()` here — the existing teardown
        // (mesh, relay, transport, iroh) already drains the heavy
        // resources — but tracked tasks that don't observe those
        // signals (e.g. a future metrics refresh loop, a tracing
        // export pump) will still see cancellation within
        // `DEFAULT_SHUTDOWN_TIMEOUT`.
        self.cancel_scope.cancel();
        let join_summary = self.cancel_scope.join(std::time::Duration::from_secs(5)).await;
        if !join_summary.is_clean() {
            warn!(
                "[{}] shutdown: {} tracked task(s) were force-aborted",
                self.cfg.node_id.short(),
                join_summary.aborted
            );
        }
        ...
        // Stop the auto-heartbeat task (if any). Dropping the handle
        // signals the task; it observes the notify on its next
        // selection and exits cleanly.
...
mod tests {
    use super::*;
    use a3net_transport::QuicTransportBuilder;
    use a3net_types::CdnContentKind;
    ...
    async fn node_shutdown_cleans_up_active_gossip_task() {
        ...
        let shared_transport: std::sync::Arc<dyn a3net_gossip::GossipTransport> =
            std::sync::Arc::new(a3net_gossip::InProcessGossip::new());
    ...
    async fn info_reports_node_state() {
        ...
        assert!(info.display_name.starts_with("a3net-"));
    ...
    async fn gossip_fanin_ingests_remote_announcements() {
        ...
        // alice's publish.
        let shared_transport: StdArc<dyn a3net_gossip::GossipTransport> =
    ...
    async fn node_relay_honors_supplied_policy() {
        use a3net_relay::RelayConfig;
        use a3net_relay::proxy_policy::HostPolicy;
        let dir = tempfile::tempdir().unwrap();
        let cfg = RelayConfig {
            serve_port: 0,
            serve_bind: "127.0.0.1".into(),
    ...
    async fn make_quic_transport(
        bind_port: u16,
        identity: Option<a3net_transport::TransportIdentity>,
        ...
        let t: Arc<a3net_transport::QuicTransport> = Arc::new(b.build().unwrap());
    ...
    fn shared(t: Arc<a3net_transport::QuicTransport>) -> SharedTransport {
        t
    }
    ...
    async fn fetch_blob_over_real_quic_transport() {
        ...
        let alice_id = a3net_transport::TransportIdentity::generate().unwrap();
        let alice_node = a3net_transport::derive_node_id_from_cert(alice_id.cert_der()).unwrap();
        let bob_id = a3net_transport::TransportIdentity::generate().unwrap();
        let bob_node = a3net_transport::derive_node_id_from_cert(bob_id.cert_der()).unwrap();
        ...
        let shared_bus: StdArc<dyn a3net_gossip::GossipTransport> =
            StdArc::new(InProcessGossip::new());
        ...
        let ann = alice
            .import_and_announce(
                ...
                &src,
                "blob",
                a3net_types::CdnContentKind::GenericFile,
            )
            .await
            .unwrap();
        ...
        let ticket = a3net_types::BlobTicket::whole(&alice_node, &alice_addr, &hash);
    ...
    async fn fetch_blob_rejects_ticket_with_forged_node_id() {
        ...
        // ticket-consistency validation in a3net-types.
        let _forged_ticket = a3net_types::BlobTicket::whole(
            &fake_node,
            &alice_addr,
            &a3net_types::ContentHash::from_bytes(b"x"),
        );
        ...
        match err {
            a3net_transport::TransportError::PeerIdentityMismatch { expected, actual } => {
    ...
    fn node_config_load_or_create_persists_quic_identity() {
        ...
        let identity = a3net_transport::TransportIdentity::generate().unwrap();
        let node = a3net_transport::derive_node_id_from_cert(identity.cert_der()).unwrap();
        identity
    ...
    async fn with_transport_aligns_node_id_to_certificate() {
        ...
        let identity = a3net_transport::TransportIdentity::generate().unwrap();
        let cert_node = a3net_transport::derive_node_id_from_cert(identity.cert_der()).unwrap();
        ...
        let cfg = NodeConfig::new(dir.path(), stale_node.clone())
            .with_quic_identity(a3net_transport::TransportIdentity::generate().unwrap());
    ...
    async fn workspace_ingest_strict_drops_invalid_announcement() {
        ...
        let bob = Node::builder(
            NodeConfig::new(&bob_dir, bob_node.clone())
                .with_gossip_validation(a3net_ipc::validation::ValidationPolicy::Strict),
        )
        .with_workspace(true)
        .build_with_bus(GossipBus::new(bob_node.clone(), shared_bus.clone()))
        .await
    ...
    async fn node_with_iroh_runtime_wires_transport_and_chat_bridge() {
        ...
        let identity = a3net_transport::iroh::IrohIdentity::load_or_create(&data_dir).unwrap();
        ...
        let expected_id = a3net_transport::iroh::public_key_to_node_id(
            &node
    ...
    async fn two_node_iroh_runtime_exchanges_announcements() {
        ...
        let alice_id = a3net_transport::iroh::IrohIdentity::load_or_create(&alice_dir).unwrap();
        let bob_id = a3net_transport::iroh::IrohIdentity::load_or_create(&bob_dir).unwrap();
        ...
        let room: RoomId = "a3net-room-tw-node".into();
        alice.join_room(&room).await.unwrap();
        bob.join_room(&room).await.unwrap();
    ...
    async fn peers_for_returns_local_blob() {
        use a3net_blobstore::BlobImporter;
    ...
    fn node_config_new_sets_defaults() {
        ...
        // display_name follows the a3net-{short_id} convention
        assert!(cfg.display_name.starts_with("a3net-"));
