...
//! and the A3Net transport layer, enabling Bitswap to send/receive
...
//!   ┌────────────────────▼────────────────────────────────────────┐
//!   │  BitswapQuicBridge  (dial / accept via a3net-transport)    │
//!   └─────────────────────────────────────────────────────────────┘
//! ```
//!
//! ## ALPN negotiation
...
use a3net_blobstore::BitswapMessage;
use a3net_transport::{Frame, OutgoingConnection, SharedTransport};
...
pub const BITSWAP_ALPN: &[u8] = b"a3net/bitswap/1";
...
// ════════════════════════════════════════════════════════════════════
//  QUIC bridge: dial/accept over a3net-transport
// ════════════════════════════════════════════════════════════════════
...
/// Real QUIC bridge backed by `a3net_transport::Transport`.
...
fn reason_label(reason: &str) -> a3net_observability::labels::LabelSet {
    match a3net_observability::labels::LabelSet::new(std::iter::once((
        "reason".to_string(),
        reason.to_string(),
    ))) {
        Ok(ls) => ls,
        Err(_) => a3net_observability::labels::LabelSet::EMPTY,
    }
}

/// Build a single-pair label set for the `messages_sent_total{type=...}`
/// series.
fn msg_type_label(msg: &BitswapMessage) -> a3net_observability::labels::LabelSet {
    match a3net_observability::labels::LabelSet::new(std::iter::once((
        "type".to_string(),
...
// pull model as the rest of the `a3net-*` crates. The counters track
...
use a3net_observability::registry::{GLOBAL as OBS_REGISTRY, Registry};
...
/// `Arc`-shared counters. The `a3net-observability` registry panics
...
mod tests {
    ...
    fn test_alpn_constant() {
        assert_eq!(BITSWAP_ALPN, b"a3net/bitswap/1");
    }
