//! Transport adapter that connects an `a3net_dht::DhtNetworkSender`
...
//! - `Arc<dyn a3net_transport::Transport>` (the transport side).
//!
...
use a3net_transport::{Frame, SharedTransport};
...
///
/// Re-exported from `a3net_dht::DynResponseSink` so callers don't
/// have to pull `a3net_dht` directly to name the type.
pub type DynResponseSink = a3net_dht::DynResponseSink;
...
/// Adapter that satisfies `a3net_dht::network::TransportDhtSender`
...
#[async_trait::async_trait]
impl a3net_dht::network::TransportDhtSender for BridgeSenderAdapter {
