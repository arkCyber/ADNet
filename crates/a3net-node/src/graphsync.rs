//! GraphSync over `a3net-transport` — IPFS-style DAG sync on top of
//! QUIC, with the same ALPN-guarded pattern used by
//! `a3net-node::bitswap_transport`.
//!
//! `a3net-blobstore` already defines the protocol layer
//! ([`a3net_blobstore::graphsync::GraphSyncClient`] /
//! [`a3net_blobstore::graphsync::GraphSyncServer`], the wire
//! envelope, the dispatcher-friendly trait [`a3net_blobstore::graphsync::GraphSyncTransportBridge`]).
...
//!
//! ```ignore
//! use a3net_node::graphsync::{GraphSyncConfig, GraphSyncService};
//!
...
/// `a3net_transport::SharedTransport`.
///
...
/// the helper that used to live in `a3net_blobstore::graphsync`
...
/// re-exported from `a3net-blobstore`). Reserved for future
...
pub struct NodeBlockStore {
    inner: Arc<a3net_blobstore::BlobStore>,
}

impl NodeBlockStore {
    pub fn new(inner: Arc<a3net_blobstore::BlobStore>) -> Self {
        Self { inner }
    }
}

impl a3net_types::graphsync::BlockStore for NodeBlockStore {
...
/// encoding that `a3net-types` writes out (raw blake3 digest) and
/// ignore anything else (returns `None`).
fn cid_to_content_hash(cid: &Cid) -> Option<a3net_types::ContentHash> {
    use a3net_types::multihash::HashCode;
    let mh = cid.hash();
    let code = mh.code();
    ...
    Some(a3net_types::ContentHash::from_bytes(&bytes))
}
...
mod tests {
    ...
    //  `a3net-blobstore::BlobStore` integration tests. Here we
    ...
    fn graphsync_transport_error_display_is_useful() {
        let e = GraphSyncTransportError::AlpnMismatch {
            expected: b"a3net/gs/1".to_vec(),
            actual: b"a3net/bitswap/1".to_vec(),
        };
        let s = e.to_string();
    ...
    fn graphsync_transport_error_display_alpn_mismatch_includes_both() {
        ...
        let expected_bytes = format!("{:?}", b"a3net/gs/1".to_vec());
        let actual_bytes = format!("{:?}", b"a3net/bitswap/1".to_vec());
