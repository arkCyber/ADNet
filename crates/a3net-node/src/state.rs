...
//!   [`ValidationPolicy`](a3net_ipc::validation::ValidationPolicy)
//!   before calling).
...
use a3net_types::{Announcement, BlobTicket, ContentHash, PeerMap, PeerSource, RoomAsset, RoomId};
...
impl SwarmIndex {
    ...
    /// [`a3net_types::MAX_TRACKED_HASHES`] (enforced inside
    /// [`PeerMap`]).
    ...
    /// [`a3net_types::MAX_TRACKED_HASHES`].
...
mod tests {
    ...
    use a3net_types::MAX_PEER_SOURCES_PER_HASH;
    use chrono::Utc;
    ...
    fn ann(room: &str, hash: &ContentHash, ticket: Option<BlobTicket>) -> Announcement {
        ...
        let node_id = ticket
            ...
            .unwrap_or_else(a3net_types::NodeId::random);
        Announcement {
    ...
    fn ticket_dedup() {
        let mut s = SwarmIndex::default();
        let nid = a3net_types::NodeId::random();
        let hash = ContentHash::from_bytes(b"x");
        let addr = a3net_types::NodeAddr::new(nid.clone())
            .with_direct(a3net_types::Endpoint::new("127.0.0.1", 9000));
    ...
    fn ingest_skips_invalid_peer_source_keeps_asset() {
        ...
        // covered exhaustively in a3net-types::peer_source.
        let nid = a3net_types::NodeId::random();
        let hash = ContentHash::from_bytes(b"x");
        let addr = a3net_types::NodeAddr::new(nid.clone())
            .with_direct(a3net_types::Endpoint::new("127.0.0.1", 9000));
    ...
    fn ingest_drops_peer_when_ticket_node_id_mismatches() {
        ...
        let announced_by = a3net_types::NodeId::random();
        let ticket_owner = a3net_types::NodeId::random();
        let hash = ContentHash::from_bytes(b"x");
        let addr = a3net_types::NodeAddr::new(ticket_owner.clone())
            .with_direct(a3net_types::Endpoint::new("127.0.0.1", 9000));
    ...
    fn ingest_caps_per_hash_peer_fanout() {
        ...
        for _ in 0..(MAX_PEER_SOURCES_PER_HASH + 10) {
            let n = a3net_types::NodeId::random();
            let addr = a3net_types::NodeAddr::new(n.clone())
                .with_direct(a3net_types::Endpoint::new("127.0.0.1", 9000));
    ...
    fn ingest_caps_total_tracked_hashes() {
        ...
        let mut s = SwarmIndex::default();
        for i in 0..(a3net_types::MAX_TRACKED_HASHES + 5) {
    ...
    fn ingest_caps_total_tracked_hashes_with_tickets() {
        ...
        assert!(s.tracked_hashes() <= a3net_types::MAX_TRACKED_HASHES);
        assert!(
            s.total_peer_entries() <= a3net_types::MAX_TRACKED_HASHES * MAX_PEER_SOURCES_PER_HASH
        );
    }
    ...
    fn peers_for_skips_malformed_in_memory_entries() {
        ...
        for t in &tickets {
            a3net_types::ticket::validate_blob_ticket(t)
