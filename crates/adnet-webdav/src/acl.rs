...
//!
//! ## Token shape (binds to `adnet-pairing::Capability`)
//!
//! ```text
//! Authorization: Capability <base64url(capability_id + nonce + expires_unix_ms)>
//! ```
//!
//! `capability_id` is the same string `adnet-pairing` writes to
...
pub struct ResolvedCapability {
    pub caps: adnet_pairing::CapabilitySet,
    pub nonce: [u8; 32],
...
impl<R: CapabilityResolver + ?Sized> AclMiddleware<R> {
    ...
    pub fn authorise(
        ...
        verb: &str,
        caps: &adnet_pairing::CapabilitySet,
    ) -> AclDecision {
    ...
    pub fn resolve_and_authorise(
        ...
        capability_id: Option<&str>,
        verb: &str,
        clock: &dyn adnet_blobstore::Clock,
    ) -> AclDecision {
