//! User system integration for `a3net-mail`.
//!
//! This module bridges the email layer (`a3net-mail`) with the A3Net identity
//! and user systems (`a3net-identity`, `a3net-userstore`).
//!
//! ## Integration Points
//!
//! - **Identity binding** — map email addresses to A3Net `Address` (secp256k1)
//! - **Key lookup** — find user's encryption keys for E2EE mail
//! - **Profile enrichment** — attach display name, avatar to outgoing mail
//! - **Authentication** — use A3Net credentials for SMTP/IMAP auth

use a3net_identity::{Address, X25519PublicKey};
use a3net_userstore::{UserProfile, UserStore};
...
/// A user's email address bound to their A3Net identity.
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct EmailIdentity {
    /// The user's A3Net wallet address.
    pub a3net_address: Address,
...
/// Resolver that maps email addresses to A3Net identities.
///
/// This is the primary integration point between the email system
/// and the A3Net identity system.
...
impl IdentityResolver {
    ...
    /// Look up a user's email identity by their A3Net address.
    pub async fn resolve_by_address(&self, addr: &Address) -> Result<Option<EmailIdentity>> {
        ...
        // Check cache.
        {
            ...
            for identity in cache.values() {
                if identity.a3net_address.to_hex() == addr_hex {
                    return Ok(Some(identity.clone()));
                }
            }
        }
    ...
    /// Bind an email address to a user's A3Net identity.
    ...
    async fn build_identity(&self, profile: &UserProfile, email: &str) -> Result<EmailIdentity> {
        let a3net_address = Address::from_hex(&profile.user_id)
        ...
        Ok(EmailIdentity {
            ...
            email: if email.is_empty() {
                format!("{}@a3net.local", profile.username)
            } else {
                email.to_string()
            },
...
/// Helper to extract the A3Net address from an email identity.
pub fn extract_a3net_address(identity: &EmailIdentity) -> &Address {
    &identity.a3net_address
}

/// Check if an email address belongs to the A3Net domain.
pub fn is_a3net_domain(email: &str) -> bool {
    email.ends_with("@a3net.local") || email.ends_with("@a3net.mesh")
}
...
mod tests {
    ...
    use a3net_userstore::{UserDevice, UserPreferences, UserProfile, UserPublicKey};
    ...
    impl UserStore for MockUserStore {
        async fn put_profile(&self, _profile: UserProfile) -> a3net_userstore::UserStoreResult<()> {
            Ok(())
        }

        async fn get_profile(&self, user_id: &str) -> a3net_userstore::UserStoreResult<Option<UserProfile>> {
        ...
        ) -> a3net_userstore::UserStoreResult<()> {
            Ok(())
        }

        async fn list_profiles(&self) -> a3net_userstore::UserStoreResult<Vec<UserProfile>> {
            Ok(self.profiles.values().cloned().collect())
        }

        async fn delete_profile(&self, _user_id: &str) -> a3net_userstore::UserStoreResult<usize> {
            Ok(0)
        }

        async fn put_public_key(&self, _key: UserPublicKey) -> a3net_userstore::UserStoreResult<()> {
            Ok(())
        }

        async fn revoke_public_key(&self, _key_id: &str) -> a3net_userstore::UserStoreResult<()> {
            Ok(())
        }

        async fn list_public_keys(&self, user_id: &str) -> a3net_userstore::UserStoreResult<Vec<UserPublicKey>> {
            Ok(self.keys.get(user_id).cloned().unwrap_or_default())
        }

        async fn put_device(&self, _device: UserDevice) -> a3net_userstore::UserStoreResult<()> {
            Ok(())
        }

        async fn revoke_device(&self, _device_id: &str) -> a3net_userstore::UserStoreResult<()> {
            Ok(())
        }

        async fn list_devices(&self, _user_id: &str) -> a3net_userstore::UserStoreResult<Vec<UserDevice>> {
            Ok(Vec::new())
        }

        async fn ensure_user_digit(&self, _user_id: &str) -> a3net_userstore::UserStoreResult<String> {
            Ok("000000000000".to_string())
        }

        async fn resolve_user_digit(&self, _user_id: &str) -> a3net_userstore::UserStoreResult<Option<String>> {
            Ok(None)
        }
    }
    ...
    async fn identity_resolver_creates_synthetic_identity() {
        ...
        assert_eq!(identity.email, "alice@a3net.local");
        assert_eq!(identity.display_name, Some("Alice Example".into()));
        assert_eq!(identity.a3net_address.to_hex(), addr.to_hex());
    }

    #[tokio::test]
    async fn is_a3net_domain_works() {
        assert!(is_a3net_domain("alice@a3net.local"));
        assert!(is_a3net_domain("bob@a3net.mesh"));
        assert!(!is_a3net_domain("alice@gmail.com"));
        assert!(!is_a3net_domain("bob@example.org"));
    }

    #[test]
    fn email_identity_display_name_round_trip() {
        let identity = EmailIdentity {
            a3net_address: Address::from_bytes([0x42u8; 20]),
            email: "alice@a3net.local".into(),
        ...
        assert_eq!(addr.address, "alice@a3net.local");
