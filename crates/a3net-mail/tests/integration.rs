...
//!   3. Connects the real `a3net-mail` API against the listener and
...
use a3net_mail::error::ErrorClass;
...
use a3net_mail::smtp::SendOutcome;
use a3net_mail::smtp::connect;
use a3net_mail::smtp::send as smtp_send;
...
mod smtp_mock {
    ...
    async fn handle_session(
        ...
        // Greeting
        write.write_all(b"220 a3net-mock ESMTP ready\r\n").await?;
        ...
        loop {
            ...
            if cmd.starts_with("EHLO") || cmd.starts_with("HELO") {
                write.write_all(b"250-a3net-mock\r\n250 OK\r\n").await?;
...
mod imap_mock {
    ...
    async fn handle_session(
        ...
        write
            .write_all(b"* OK [CAPABILITY IMAP4rev1] a3net-imap-mock\r\n")
            .await?;
        write.flush().await?;
...
async fn smtp_send_classifies_250_ok() {
    ...
    let _ = a3net_mail::smtp::send::quit(transport).await;
}
...
async fn imap_fetch_decodes_canned_message() {
    ...
    let mut session = a3net_mail::imap::ImapSession::connect(account)
        .await
        .expect("imap connect");
...
async fn imap_fetch_skips_oversized_message_body() {
    ...
    let mut handle =
        a3net_mail::imap::FetchHandle::new(&mut session, info).with_max_message_size(10); // absurdly small — everything is "oversized"
...
async fn mail_account_builder_to_full_account() {
    ...
    assert_eq!(
        ...
        a3net_mail::login_param::SocketSecurity::Starttls
    );
}
