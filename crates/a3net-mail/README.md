# `a3net-mail` — SMTP send + IMAP receive for the A3Net workspace
...
`chatmail@core` is a ~14 000-line crate. A3Net uses only the SMTP /
IMAP / MIME parts, and wants a build without Delta Chat's SQLite /
runtime dependencies. `a3net-mail` is that cut-down version.
...
# use a3net_mail::prelude::*;
# async fn doc() -> Result<()> {
# let mut transport: a3net_mail::smtp::Transport = unimplemented!();
...
Run with `cargo run -p a3net-mail --example <name>`.

## Tests

```
cargo test -p a3net-mail
```
