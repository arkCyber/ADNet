...
//! this module exists so `a3net-node` and callers have a stable seam to
//! build against.
...
impl ChainNode {
    ...
    pub async fn start(self) -> ChainResult<Option<ChainNodeHandle>> {
        if !self.config.enabled {
            tracing::debug!("a3net-chain: disabled, skipping start");
            return Ok(None);
        }
        tracing::info!(
            kind = %self.config.kind,
            role = ?self.config.role,
            "a3net-chain: enabled but no backend is implemented yet"
        );
