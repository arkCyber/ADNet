...
///
/// Aerospace note (DO-178C §6.4.2 — interface robustness): the
/// original implementation used `chain(repeat(0)).take(32)`,
/// which always produced a 32-byte buffer **padded with zeros**
/// for keys shorter than 32 bytes. Two completely different
/// short keys (e.g. `b"a"` and `b"b"`) could end up with the
/// same NodeId because only the first byte was non-zero. We
/// now hash short keys with BLAKE3 to spread them across the
/// full 256-bit ID space so the routing table sees the real
/// distribution of keys rather than a degenerate "first byte
/// matters, rest is zero" projection.
pub fn node_id_from_key(key: &DhtKey) -> NodeId {
    let raw = key.as_bytes();
    if raw.len() >= 32 {
        // Common path: hash is at least 32 bytes — use the
        // first 32 directly.
        let mut arr = [0u8; 32];
        arr.copy_from_slice(&raw[..32]);
        return NodeId::from_bytes(&arr).unwrap_or_else(|_| NodeId::random());
    }
    // Short key: hash to a 32-byte representation. This is
    // collision-resistant up to the 128-bit security level of
    // BLAKE3 against pre-image attacks, which is more than
    // enough for the routing purpose (which only needs
    // reasonable distribution, not cryptographic uniqueness).
