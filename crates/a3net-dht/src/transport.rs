//! DHT transport adapter for integrating with a3net-transport.
//!
//! This module provides the integration layer between the DHT and the
//! A3Net transport layer, enabling DHT messages to be sent over the
...
/// Adapter that bridges DHT protocol to a3net-transport.
...
impl DhtTransportAdapter {
    ...
    fn key_to_node_id(&self, key: &[u8]) -> NodeId {
        ...
        // with BLAKE3 instead of zero-padded so two distinct
        // short keys always produce two distinct routing IDs.
        if key.len() >= 32 {
            let mut arr = [0u8; 32];
            arr.copy_from_slice(&key[..32]);
            return NodeId::from_bytes(&arr).unwrap_or_else(|_| self.transport.local_node_id().clone());
        }
...
mod tests {
    ...
    #[test]
    fn test_dht_alpn() {
        assert_eq!(DHT_ALPN, b"a3net/dht/1");
    }
}
