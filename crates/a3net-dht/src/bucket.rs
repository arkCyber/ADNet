...
impl KBucket {
    ...
    /// implementation used `position(...).unwrap()` and
    /// `remove(...).unwrap()`. Both unwraps are unreachable in the
    /// current single-threaded bucket but a future refactor that
    /// shares the bucket behind a lock could violate the
    /// invariant. We use a safe `find + swap_remove` to make the
    /// function panic-free.
    pub fn mark_seen(&mut self, id: &NodeId) {
        // Find the position of `id`. `find_mut` only borrows
        // immutably; we still need the index for the swap.
        let pos = self.contacts.iter().position(|x| &x.id == id);
        let Some(pos) = pos else { return };
        let mut c = self.contacts.remove(pos).expect("position-known");
        c.mark_seen();
...
impl RoutingTable {
    ...
    ///
    /// Returns the position of the most significant bit in the XOR
    /// distance: `1` for the closest bucket, `255` for the furthest.
    /// When the distance is zero (same node), returns `255` so the
    /// caller routes the entry into a dedicated far-away bucket
    /// rather than poisoning bucket #0 (which is reserved for
    /// "distance == 0, never insert here").
    ///
    /// Aerospace note (DO-178C §6.3.4): this is the only function
    /// that decides which `KBucket` a contact lands in, so a wrong
    /// bit position silently corrupts the entire routing table.
    /// The companion property test
    /// `bucket_index_symmetry_and_zero_handling` pins down the
    /// contract so a regression can't slip through.
    pub fn bucket_index(node_a: &NodeId, node_b: &NodeId) -> usize {
        let distance = node_a.xor_distance(node_b);
        // Find the most significant set bit. Standard Kademlia
        // semantics: bit `b` (0-indexed from the MSB) maps to
        // bucket `b`. We work from the front because `xor_distance`
        // returns big-endian-ordered bytes — the first non-zero
        // byte holds the highest bit.
        for (i, &byte) in distance.iter().enumerate() {
            if byte != 0 {
                for bit in (0..8).rev() {
                    if (byte >> bit) & 1 == 1 {
                        // Bit index from MSB of the entire 256-bit
                        // distance, 0-indexed. The first non-zero
                        // bit sits at position `i * 8 + (7 - bit)`.
                        // The range is [0, 255]; we shift up by 1
                        // so bucket 0 stays empty (it's reserved
                        // for distance == 1 in classical Kademlia
                        // and we keep the slot for the
                        // "same-node" sentinel).
                        let pos_from_msb = i * 8 + (7 - bit);
                        let bucket_idx = pos_from_msb + 1;
                        // Saturate at 255 so the call sites'
                        // `idx.min(255)` is always a no-op and
                        // out-of-range inputs can never crash
                        // `bucket_for_mut`.
                        return bucket_idx.min(255);
                    }
                }
            }
        }
        // All bits zero: same node. Return 255 (furthest bucket).
        255
    }
