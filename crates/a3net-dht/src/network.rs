...
//! integrating with a3net-transport's QUIC connection layer.
...
impl DhtNetworkSender {
    ...
    ///
    /// Aerospace note (DO-178C §6.4.2): the original
    /// implementation unconditionally broadcasted
    /// `127.0.0.1:0` as the provider's address. Remote peers
    /// receiving the announcement would persist a non-routable
    /// record and silently never be dialable from outside. We
    /// now refuse to send if `local_addr_str()` isn't set — the
    /// caller (typically `DhtNode::announce_content`) must have
    /// called `set_local_addr` with the bound transport socket
    /// before any announce is attempted. The legacy fire-and-forget
    /// path is preserved as `send_add_provider`, which accepts
    /// a fully populated `ProviderRecord`.
    pub async fn announce_provider(
        ...
        // We don't have access to the embedder's bound address
        // from inside the sender; the higher-level helper
        // (`DhtNode::announce_content`) is responsible for
        // constructing the `ProviderRecord` and calling
        // `send_add_provider`. Surface a clear error here.
        let _ = ttl_secs;
