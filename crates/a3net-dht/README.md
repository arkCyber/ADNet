...
> 基于 Kademlia 的 DHT 路由，为 A3Net 提供内容路由与对等节点发现。 / Kademlia-style DHT routing for A3Net — content routing and peer discovery.

## 概览 (Overview)

`a3net-dht` 在 A3Net 既有的 Pkarr / DNS / mDNS 发现机制之上叠加了一个简化版
Kademlia DHT，让节点能够：
...
DHT 的键直接复用 `a3net_types::ContentHash`（BLAKE3），peer ID 复用 `NodeId`
...
`a3net-dht` 在 workspace 中已经作为 `path` 依赖被 `a3net-node`、`a3net-namespace` 等
crate 引用。在你的代码里直接 `use`：

```rust
use a3net_dht::{DhtNode, DhtConfig, DhtKey, RoutingTable};
...
```rust,no_run
use a3net_dht::{DhtConfig, DhtNode};
use a3net_types::NodeId;
...
```rust,no_run
use a3net_dht::{DhtKey, DhtNode, DhtConfig};
use a3net_types::NodeId;
...
```rust,no_run
use a3net_dht::{Contact, RoutingTable};
use a3net_types::NodeId;
...
- **`a3net-node`** 在装配 DHT（feature `dht`）时把 `DhtNode` 作为内容寻址层的
  后端，对 `announce` / `fetch_blob` 的 `find_providers` 调用提供最近 K 个 peer。
- **`a3net-namespace`** 用 `DhtIpnTransport` 把 IPNS 记录以 `PutValue` /
  `GetValue` 形式落到 DHT，提供一种 "无需信任公共 pkarr relay" 的命名方案。
- **`a3net-gateway`** 通过 DHT 解析 `DAGService::get` / `DhtService::find_providers`
