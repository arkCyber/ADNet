...
//! [`PeeringGrant`](adnet_mesh_coordinator::PeeringGrant)
//! envelopes from the `adnet-transit/v1` gossip topic.
//!
//! ## What it does
//!
...
//!   `adnet-identity` integration). See RFC §10.
...
impl GossipFederatedTopology {
    ...
    pub fn observe_grant_verified<R: CoordinatorPubkeyRegistry>(
        ...
        PeeringGrantVerifier::new()
            ...
            .map_err(|e| match e {
                adnet_mesh_coordinator::CoordinatorError::PeeringSelfLoop => {
                    GossipApplyError::SelfLoop
                }
                adnet_mesh_coordinator::CoordinatorError::PeeringSignatureInvalid(s) => {
                    GossipApplyError::SignatureInvalid(s)
                }
                adnet_mesh_coordinator::CoordinatorError::PeeringUnknownCoordinator(s) => {
                    GossipApplyError::UnknownCoordinator(s)
                }
                adnet_mesh_coordinator::CoordinatorError::PeeringExpired {
                    grant_id,
                    valid_until,
...
mod tests {
    use super::*;
    use adnet_mesh_coordinator::Peerings;
    use adnet_mesh_coordinator::InMemoryPeerings;
    use adnet_mesh_coordinator::PeeringDirection;
