...
> 网关 / 出口节点路由：让 mesh 成员能 "借用" 其他成员的网络出口。 / Gateway / exit-node routing for the ADNet mesh VPN.

## 概览 (Overview)

`adnet-exit-node` 把 mesh VPN 拆成两个角色：
...
Crate 只负责"决策"，不发包。Linux/macOS 上的真实 IP 转发、iptables、NAT 由 `adnet-tun`
或运维脚本完成。

附加能力：

- `bandwidth` — 每个 client / 全局的带宽计量。
- `billing` — 用 `adnet-token` 做出口流量计费。
...
```rust
use adnet_exit_node::{Gateway, Client, Router, RouterConfig, RouteAction};
use std::net::IpAddr;
```
...
```rust,no_run
use adnet_exit_node::{RouteAction, Router, RouterConfig};
use std::net::IpAddr;
...
```rust,no_run
use adnet_exit_node::{Gateway, GatewayAdvert};
...
```rust,no_run
use adnet_exit_node::Client;

let me  = adnet_types::NodeId::random();
let cli = Client::new(me);
let gw  = adnet_types::NodeId::random();
cli.use_gateway(gw);
// ... 用完
cli.unset();
```
...
```rust,no_run
use adnet_exit_node::{ClientMeter, RateLimitConfig};
...
- **`adnet-cli`** 用 `Client::use_gateway` 实现 `ray gateway use <name>` 子命令。
- **`adnet-tun`** 把每个出 mesh 包送给 `Router::decide`，按结果分发给 mesh / gateway。
