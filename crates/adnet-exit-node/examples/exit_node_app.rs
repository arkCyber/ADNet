//! `adnet-exit-node` 应用示例：演示完整的 mesh 内网 + 网关出口 + 限流决策。
//! 不真正发送包，只在 `Router` 层做决策验证。
//!
//! 运行：`cargo run -p adnet-exit-node --example exit_node_app`
...
use adnet_types::{NodeId, VirtualIp};

fn main() -> Result<(), Box<dyn std::error::Error>> {
    println!("--- adnet-exit-node app demo ---");
