...
impl PersonalSignature {
    ...
    ///
    /// **Why accept `v=28`:** EIP-191 specifies either `27` or `28`,
    /// and external signers (MetaMask, `ethers.Wallet.signMessage`,
    /// hardware wallets) commonly emit `28`. We pin *our own* output
    /// to `27` (see [`PERSONAL_SIGN_VERSION`]) so the wire format
    /// stays deterministic, but we accept both recovery ids on the
    /// read path so any EIP-191 signature recovers to the same
    /// signer regardless of which side issued it.
...
mod tests {
    ...
    #[test]
    fn accepts_v_28_from_external_signers() {
        // We pin our *emitted* v to 27 (see PERSONAL_SIGN_VERSION),
        // but verify() must accept either 27 or 28 so an external
        // EIP-191 signer that emits 28 still recovers to the same
        // address.
        let mut bytes = [0u8; 65];
        bytes[64] = 28;
        assert!(PersonalSignature::from_compact(&bytes).is_ok());
    }
}
