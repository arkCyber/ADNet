...
//!
//! Most A3Net nodes need **one stable identity** (used for signing pledges,
...
impl Treasury {
    ...
    ///
    /// **Lenient on corrupt ephemerals:** ephemeral entries with
    /// invalid hex or an unparseable secret are silently dropped.
    /// This matches the historical behaviour (callers that round-trip
    /// a [`Treasury::view`] through disk don't want a single corrupted
    /// ephemeral to nuke the whole file), but it also means a treasury
    /// can be partially reconstructed without anyone noticing. Use
    /// [`Self::from_view_strict`] if you need loud-failure on
    /// corruption.
    pub fn from_view(view: TreasuryView) -> Self {
        let (ephemeral, _dropped) = Self::parse_view_ephemerals(&view);
        Self {
            root: None,
    ...
    /// Reconstruct a treasury from a saved view, *strictly*. Returns
    /// an error listing the first invalid ephemeral secret rather than
    /// silently dropping it. Prefer this over [`Self::from_view`]
    /// whenever the on-disk file might have been tampered with.
    pub fn from_view_strict(view: TreasuryView) -> Result<Self> {
        let (ephemeral, dropped) = Self::parse_view_ephemerals(&view);
        if let Some(reason) = dropped {
            return Err(IdentityError::Keystore(format!(
                "treasury view dropped {reason} ephemeral entries"
            )));
        }
        Ok(Self {
            root: None,
    ...
    /// Shared helper. `Ok(())` is encoded by `dropped == None`.
    fn parse_view_ephemerals(view: &TreasuryView) -> (Vec<ReceiptWallet>, Option<usize>) {
        let mut ephemeral = Vec::with_capacity(view.ephemeral.len());
        let mut dropped = 0usize;
        for v in &view.ephemeral {
            match Self::parse_receipt_view(v) {
                Some(w) => ephemeral.push(w),
                None => dropped += 1,
            }
        }
        (ephemeral, if dropped == 0 { None } else { Some(dropped) })
    }

    fn parse_receipt_view(v: &ReceiptWalletView) -> Option<ReceiptWallet> {
...
mod tests {
    ...
    #[test]
    fn from_view_strict_rejects_corrupted_ephemeral() {
        // A corrupted `secret_hex` (odd number of hex chars) must NOT
        // be silently dropped — `from_view_strict` is the loud path
        // and must error so the operator notices the corruption.
        let view = TreasuryView {
            root_public: Wallet::generate().public().clone(),
            ephemeral: vec![ReceiptWalletView {
                purpose: "ok".into(),
                secret_hex: "not-even-hex".into(),
                expires_unix: 0,
            }],
        };
        let err = Treasury::from_view_strict(view).unwrap_err();
        assert!(err.to_string().contains("dropped"), "got {err}");
    }

    #[test]
    fn from_view_strict_accepts_clean_view() {
        // Round-trip: a freshly serialised view must survive
        // strict-mode loading.
        let (mut t, _s) = Treasury::new();
        t.issue_receipt_wallet("relay-session-1", 0);
        t.issue_receipt_wallet("relay-session-2", 1_000);
        let view = t.view();
        let t2 = Treasury::from_view_strict(view).unwrap();
        assert_eq!(t2.ephemeral_count(), 2);
    }
}
