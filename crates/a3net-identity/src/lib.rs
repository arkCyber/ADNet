//! `a3net-identity` — identity, signatures, and end-to-end encryption for A3Net.
//!
//! This crate is intentionally **separate from `a3net-types`** so that the
...
//! ## Design
//!
//! - **Wallet keys** are `secp256k1` (EVM-compatible). An A3Net wallet is a
...
//! `a3net-token` (which builds on this crate).
//!
...
pub use a3net_types::WalletAddress as ProtocolWalletAddress;
...
pub use eip712::{Eip712Domain, TypedData, typed_data_hash};
pub use envelope::{ADR_ENVELOPE_MAGIC, ADR_ENVELOPE_VERSION, EncryptedPayload, SealedEnvelope};
pub use error::{IdentityError, Result};
pub use hd::{DEFAULT_EVM_PATH, HdWallet};
pub use keystore::{CipherParams, KdfParams, Keystore, KeystoreFile};
...
/// [`a3net_types::Announcement`]. `0` = EIP-191 over secp256k1 (the
...
// -- interop with `a3net_types::WalletAddress` ---------------------------
//
// `a3net-types` deliberately defines its own `WalletAddress` (20 raw
// bytes, no crypto deps) so protocol crates don't pull in secp256k1.
// `a3net-identity` is where the crypto lives; we provide cheap lossless
