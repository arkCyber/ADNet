...
> A3Net 身份层 — secp256k1 `Wallet` / EVM `Address` / EIP-191 `PersonalSignature` / X25519 ECIES `SealedEnvelope`,为签名、加密、地址生成提供统一原语。

## 概览 (Overview)

`a3net-identity` 是 A3Net 所有密码学原语的中心:
...
- **`ADR_ENVELOPE_MAGIC` / `ADR_ENVELOPE_VERSION`** — A3Net envelope 的协议常量,避免误用其他 ECIES 格式。
- **`sign_announcement` / `verify_announcement`** — 为 `a3net_types::Announcement` 提供的"消息签名 + 验签"工具。
...
a3net-types = { workspace = true }   # 公告签名需要
```
...
```rust
use a3net_identity::{EciesSecretKey, EncryptedPayload, SealedEnvelope};
...
```rust
use a3net_identity::{sign_announcement, verify_announcement};
...
- **`a3net-token`** — `Pledge::sign(body, &wallet)` 用 `Wallet::sign_personal` 对 `PledgeBody` 摘要签名,`verify_for_relay` 用 `recover_personal` 反解 pledgor 地址。
- **`a3net-gossip`** — gossip 上传递的 `Announcement` 若带 `signature`,接收端走 `verify_announcement` 校验。
- **`a3net-ipc` / `a3net-rpc`** — daemon 之间传输敏感 payload 时使用 `SealedEnvelope`,接收方只用自己的 X25519 secret key 打开。
- **`a3net-mail`** — 邮件 / 通知附带的 sender 字段走 `Address` + `PersonalSignature` 校验身份。
- **`a3net-share`** — share 文件时把 owner 标记为 `Address`,接收端用签名校验控制权变更。
- **AI agent wallet** — 自动续费 / 自动签名的 agent 直接持有 `Wallet`,在 `a3net-token` 中发起 pledge。
