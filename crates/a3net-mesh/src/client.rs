...
/// [`BlobTicket::http_base`](a3net_types::BlobTicket::http_base).
...
mod tests {
    ...
    async fn parallel_chunk_fetch_from_mesh() {
        ...
        let res = fetch_from_mesh(&store, &hash, &[base], &dest, a3net_types::RangeSpec::All)
            .await
            .unwrap();
    ...
    async fn range_fetch_single() {
        ...
        let res = fetch_from_mesh(
            ...
            &dest2,
            RangeSpec::Multi(vec![
                a3net_types::ByteRange::new(0, 50).unwrap(),
                a3net_types::ByteRange::new(payload.len() as u64 - 50, payload.len() as u64)
                    .unwrap(),
            ]),
        )
        .await
