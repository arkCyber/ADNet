...
//
// Performance baseline tests for `a3net-mesh`.
//
...
use a3net_blobstore::{BlobStore, chunked::CHUNK_SIZE};
use a3net_mesh::{MeshConfig, MeshServer, fetch_from_mesh};
...
fn stage_blob(dir: &TempDir, size: usize) -> (Arc<BlobStore>, a3net_types::ContentHash) {
...
async fn concurrent_range_fetches_byte_exact() {
    ...
    for r in &ranges {
        ...
        let r = *r;
        tasks.push(tokio::spawn(async move {
            let dest = std::env::temp_dir().join(format!("a3net-mesh-range-{}-{}", r.start, r.end));
            let res = fetch_from_mesh(
                &store,
                &hash,
                std::slice::from_ref(&base),
...
async fn sustained_fetches_under_load() {
    ...
    for i in 0..N_FETCHES {
        ...
        let hash = hash.clone();
        tasks.push(tokio::spawn(async move {
            let dest = std::env::temp_dir().join(format!("a3net-mesh-load-{i}.bin"));
            let t0 = Instant::now();
            let res = fetch_from_mesh(
                &store,
                &hash,
