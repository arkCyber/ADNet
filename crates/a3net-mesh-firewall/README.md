...
> A3Net mesh VPN 的用户态包过滤：方向性规则 + conntrack + 默认策略。 / Userspace packet filter for the A3Net mesh VPN — directional rules, conntrack, default policy.

## 概览 (Overview)

`a3net-mesh-firewall` 在 A3Net mesh VPN 里充当 "第二道闸"（第一道是宿主内核防火墙）。
...
- `FirewallStats` — 度量计数器（接 `a3net-observability`）。
...
```rust,no_run
use a3net_mesh_firewall::{Decision, Packet, rule::Direction};
...
```rust,no_run
use a3net_mesh_firewall::ConnProto;
...
```rust,no_run
use a3net_mesh_firewall::declarative::{FirewallSpec, StaticPeerResolver};
...
- **`a3net-tun`** 在每个包的出口/入口都问 `FirewallEngine::decide`，按结果路由或丢弃。
- **`a3net-cli`** 用 `FirewallSpec` 从 `~/.config/ray/firewall.yaml` 加载规则，
