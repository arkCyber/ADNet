//! Integration tests for `a3net-mesh-firewall`. These
...
fn deny_rule_blocks_despite_default_allow() {
    ...
    let eng = engine_with(
        ...
        vec![Rule {
            id: a3net_mesh_firewall::rule::RuleId::from_index(0),
            direction: Direction::In,
            action: Action::Deny,
            proto: ProtoSpec::Tcp,
            port: PortSpec::Single(23),
            peer: a3net_mesh_firewall::rule::PeerSpec::any(),
        }],
    );
    let (d, _) = eng.decide(Packet {
        direction: Direction::In,
...
fn conntrack_returns_allow_after_outbound_open() {
    ...
    assert!(d.is_allowed());
    assert_eq!(r, a3net_mesh_firewall::DecisionReason::ConntrackAllow);
}
...
fn conntrack_inbound_lookup_works_with_real_local_port() {
    use a3net_mesh_firewall::InboundProbe;

    let ct = a3net_mesh_firewall::ConnTracker::new(
        a3net_mesh_firewall::ConnTrackerConfig::default(),
    );
    let peer = NodeId::random();
    let ip = ipv4(100, 64, 0, 9);
...
fn conntrack_full_returns_default_deny() {
    use a3net_mesh_firewall::conntrack::{ConnTracker, ConnTrackerConfig};
...
fn declarative_spec_unknown_peer_errors() {
    ...
    assert!(matches!(
        err,
        a3net_mesh_firewall::declarative::DeclarativeError::UnknownPeer(_)
    ));
}
