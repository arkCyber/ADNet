...
//!
//! Run with: `cargo run -p a3net-backup --example backup_smoke`.

use a3net_backup::{describe, restore, snapshot, verify};
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    let base = std::env::temp_dir().join("a3net-backup-smoke");
    let _ = std::fs::remove_dir_all(&base);
    std::fs::create_dir_all(&base)?;
    let src = base.join("src");
    let snap_out = base.join("snap.a3net-snap");
