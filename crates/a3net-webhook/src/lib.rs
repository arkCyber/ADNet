//! `a3net-webhook` — fan A3Net events out to HTTP endpoints.
//!
...
impl WebhookSink {
    ...
    /// Borrow the endpoint list (for `a3net webhook list`).
...
async fn post_http(
    ...
    if use_tls {
        return Err(WebhookError::Transport(
            "https endpoints not yet supported by a3net-webhook; use http for now"
                .to_string(),
        ));
    }
    let host = parsed
...
/// let handle = a3net_webhook::pump::run(sink, rx);
/// handle.await.unwrap();
/// ```
///
/// The pump:
...
mod tests {
    ...
    #[test]
    fn a3net_event_announcement_carries_payload() {
    ...
    // We exercise it without depending on `a3net-gossip` by
    ...
    fn sample_announcement(
        room: &str,
        node: &a3net_types::NodeId,
        msg_id: Option<&str>,
    ) -> a3net_types::Announcement {
        a3net_types::Announcement {
            room_id: a3net_types::RoomId::from(room),
            content_hash: a3net_types::ContentHash::from_bytes(b"hello"),
            node_id: node.clone(),
            title: "hello".into(),
    ...
    async fn pump_delivers_announcement_and_persists_to_spool() {
        ...
        let (tx, rx) = tokio::sync::broadcast::channel::<a3net_types::Announcement>(16);
        let handle = pump::run(Arc::clone(&sink), rx);

        let local = a3net_types::NodeId::random();
        for i in 0..3 {
    ...
    async fn pump_exits_cleanly_when_receiver_closed() {
        ...
        let (tx, rx) = tokio::sync::broadcast::channel::<a3net_types::Announcement>(4);
        let handle = pump::run(Arc::clone(&sink), rx);
    ...
    async fn pump_end_to_end_signature_and_payload() {
        ...
        let (tx, rx) =
            tokio::sync::broadcast::channel::<a3net_types::Announcement>(4);
        let pump_handle = pump::run(Arc::clone(&sink), rx);
        let node = a3net_types::NodeId::random();
