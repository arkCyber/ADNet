...
//!
//! - `a3net_reputation_event_total{event}` — counter of events
//!   applied (labelled by the [`ReputationEvent::kind_tag`]).
//! - `a3net_reputation_score{peer_hash}` — gauge of the current
...
//! [`a3net_observability::registry::GLOBAL`] registry by default;
...
pub struct ReputationMetrics {
    /// `a3net_reputation_event_total{event=…}`
    pub event_total: Arc<Counter>,
    /// `a3net_reputation_score{peer_hash=…}`
