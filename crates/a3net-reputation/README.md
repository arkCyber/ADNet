...
> A3Net 跨子系统的全局对等信誉(PeerScore) — bitswap / gossip / pairing / chat 共享同一份评分表,统一"接受 / 丢弃对等"决策。

## 概览 (Overview)

`a3net-reputation` 把 A3Net 原本散落在各子系统里的 `HashMap<String, usize>` 计数器统一为一个共享的 `PeerScoreTable`:
...
- **可观测** — `metrics` feature 把 score / event 接到 `a3net-observability::Registry`。
...
a3net-types      = { workspace = true }   # 事件带 NodeId
```
...
```rust
use a3net_reputation::{PeerScoreTable, ReputationEvent, ReputationParams, InvalidReason};
...
```rust
use a3net_reputation::{ReputationStore, ReputationStoreConfig};

let store = ReputationStore::open(ReputationStoreConfig::default_in("/var/lib/a3net/rep"))?;
// 每产生 delta 自动 append 到 jsonl
```

## 应用案例 (Use Cases / Examples)

- **`a3net-blobstore` / bitswap** — 每个 `want_have` / `block_received` 事件通过 `BitswapSignal` 上报,负分直接踢出 ledger。
- **`a3net-gossip` / gossipsub** — `GossipSignal::valid` / `GossipSignal::invalid` 与 gossip 校验路径并行,gossip 层直接读 score 决定 mesh 维护。
- **`a3net-pairing`** — 首次 QR 配对写入一次 `PairingSignal::promote`,撤销时 `demote`,分数直接反馈到外发决策。
- **`a3net-chat`** — 用户对某 peer 标"不可信"产生 `TrustSignal`,与全局 score 融合后立刻影响其他子系统。
- **`a3net-observability`** — `metrics` feature 开启后,`a3net_reputation_score` / `a3net_reputation_event_total` 可在 Grafana 仪表盘上展示。
