//! `a3net-share` — P2P file & directory sharing over iroh.
//!
//! This crate ports the sendme ([`n0-computer/sendme`]) `send` /
//! `receive` flow onto A3Net's existing primitives:
//!
...
//!   `receive` path (`a3net_share_receive_bytes_total`,
...
//! have in `a3net-transport` (iroh endpoint / Router) and
//! `a3net-blobstore::iroh_store` (Bao-verified FsStore). Pulling the
//! algorithms into their own crate lets `a3net-cli`, `a3net-ffi`, and
