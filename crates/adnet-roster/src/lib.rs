//! `adnet-roster` — local contact directory (people, agents, IoT devices),
...
//! ADNet-friendly port: it exposes a [`RosterStore`] trait with two
...
//! ## What is NOT here
//!
//! Conversation / message tables live in `adnet-chatstore`. User accounts
//! (`username`, `display_name`, `last_seen`, 12-digit ID generation) live
//! in `adnet-userstore`. The roster deliberately keeps just the
