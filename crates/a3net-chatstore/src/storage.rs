...
//! [`a3net_types::group_chat`]; `message_type` is the
//! [`a3net_types::invariants::MessageType`] enum (serialised as
...
//! # Validation
//!
//! Every write validates its record against [`a3net_types`]
...
pub use a3net_types::group_chat::MessageAttachment;
use a3net_types::group_chat::{DirectMessage, GroupMessage, MAX_SEQUENCE, MessageReceipt};
use a3net_types::invariants::{MessageType, validate_id, validate_name};
...
// `MessageAttachment` is re-exported from `a3net_types::group_chat`
// above so the lib root can pull it into its public surface without
// forcing users of this module to reach into `a3net_types`.
...
fn message_type_to_str(t: &MessageType) -> &'static str {
    use a3net_types::invariants::MessageType as M;
    match t {
        M::Text => "text",
        M::Image => "image",
        M::File => "file",
