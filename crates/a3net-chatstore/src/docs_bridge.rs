//! Bridge between `a3net-chatstore` and `iroh-docs`.
//!
//! # Scope (Phase 5a)
//!
...
use a3net_blobstore::{BlobImporter, BlobReader, IrohBlobStore};
use a3net_types::invariants::validate_id;
...
/// Convert the iroh `Hash` on an `Entry` into the A3Net
/// `ContentHash` representation that the blob store expects.
#[inline]
fn entry_hash_to_content_hash(ih: IrohHash) -> a3net_types::ContentHash {
    use a3net_blobstore::iroh_store::iroh_hash_to_content_hash;
...
impl IrohDocsChat {
    ...
    /// (e.g. to import a ticket shipped via `a3net-types`).
    ...
    pub async fn append_message_as(
        ...
        for attempt in 0..MAX_APPEND_RETRIES {
            ...
            let iroh_hash = a3net_blobstore::iroh_store::content_hash_to_iroh_hash(&content_hash)
            ...
            let seq_iroh_hash =
                a3net_blobstore::iroh_store::content_hash_to_iroh_hash(&seq_content_hash)
