...
pub enum ChatStoreError {
    ...
    /// `a3net-chatstore` understands and the caller has not opted
    /// into auto-migration.
    ///
    /// **Recoverability:** `Fatal` — the operator must either
    /// upgrade `a3net-chatstore` or run the explicit migration
    ...
    /// A trust level was outside `[-3, +3]`.
    ///
    /// **Recoverability:** `UserError` — the caller asked for an
    /// impossible level; the schema check exists to catch
    /// malformed client input before it reaches the table.
    #[error("invalid trust level: {level}")]
    InvalidTrustLevel {
        /// The offending level value.
        level: i8,
    },

    /// A required id (user-id, conversation-id, …) was empty or
    /// failed its schema check.
    ///
    /// **Recoverability:** `UserError`.
    #[error("invalid id: {0}")]
    InvalidId(String),
...
impl ChatStoreError {
    ...
    ///
    /// ```ignore
    /// use a3net_chatstore::error::{ChatStoreError, ErrorClass};
    ///
...
/// `a3net_types` uses its own `AdnetError` enum (validation, ticket,
