...
> A3Net 设备配对协议核心:签名邀请、能力位、可信设备存储 / Secure device-pairing protocol for A3Net — signed invitations, capability bits, and a trusted-device store.

## 概览(Overview)

`a3net-pairing` 是 A3Net 网络中负责**设备配对 (Pairing)** 的核心 crate。它处于 A3Net 身份层 (`a3net-identity`、`a3net-types`) 与传输层 (`a3net-transport`) 之间,提供:
...
这个 crate 是**纯数据**的:不依赖 iroh,不依赖 tokio,所有 IO 都由调用方决定如何落地。它的输出喂给 `a3net-qr`、`a3net-invite` 和 `a3net-ssh` 等用户可见流程。
...
| `PairingInvitation`(`wire`) | `a3net-pairing://<base64url>` URL 形式,可 QR 扫描 |
...
`a3net-pairing` 已经是 A3Net workspace 的 path 依赖,使用方式:
...
CLI 子命令:`a3net-cli` 暴露 `a3net pair ...` 系列子命令,内部就是 `a3net-pairing` 的 API。

## 使用(Usage)

```rust
use a3net_pairing::{SignedInvitation, CapabilitySet, PairingInvitation};
...
```rust
// 6. 落盘保存(可选)
use a3net_pairing::{TrustedDeviceStore, TrustedDeviceStoreConfig};
...
1. **手机扫描家庭服务器 QR。** 用户在 A3Net 桌面端点 `生成邀请`,得到一个 `a3net-pairing://` URL,渲染成 QR 贴在客厅桌面上。手机扫码后,服务端验签 + 接受一个 Ed25519 签名,落地一条 `TrustedDeviceRecord`,从此刻起手机和家庭服务器之间便能以 `chat + files.read` 的能力互相通信。
