...
//! ## Metric names
//!
//! All names are prefixed with `a3net_gossip_*` so the
...
pub struct GossipMetrics {
    /// `a3net_gossip_publishes_total` — total local
...
impl GossipMetrics {
    ...
    /// `TransportMetrics::get()` uses in `a3net-transport`.
    pub fn get() -> Self {
        GLOBAL.clone()
    }
...
mod tests {
    ...
    fn register_creates_all_metrics_with_unique_names() {
        ...
        assert!(names.contains(&"a3net_gossip_publishes_total".to_string()));
        assert!(names.contains(&"a3net_gossip_publish_errors_total".to_string()));
        assert!(names.contains(&"a3net_gossip_deliveries_total".to_string()));
        assert!(names.contains(&"a3net_gossip_decode_failures_total".to_string()));
        assert!(names.contains(&"a3net_gossip_subscribes_total".to_string()));
        assert!(names.contains(&"a3net_gossip_unsubscribes_total".to_string()));
        assert!(names.contains(&"a3net_gossip_joins_total".to_string()));
        assert!(names.contains(&"a3net_gossip_leaves_total".to_string()));
