...
//! to A3Net's room/topic naming convention. Subscribers receive
//! payloads from iroh's HyParView+PlumTree epidemic broadcast trees
//! and forward them into a per-topic [`tokio::sync::broadcast`]
//! channel so the rest of the A3Net gossip stack sees the same
//! surface as it does over the in-process bus.
//!
//! ## Topic mapping
//!
//! A3Net topics are 32-byte BLAKE3 digests of labels like
//! `"a3net-room-{room}"`. iroh-gossip's `TopicId` is also 32 bytes but
//! is taken directly (no hashing). We bridge by re-hashing the
//! A3Net hex digest as 32 raw bytes — both sides treat the value as
...
/// Decode a hex-encoded A3Net `Topic` (64 hex chars = 32 bytes) into
...
impl IrohGossipTransport {
    ...
    /// `a3net-node` wiring this transport into a `Router`).
...
mod tests {
    ...
    fn topic_hex_to_iroh_topic_id_round_trip() {
        let topic = Topic::from_label("a3net-room-lobby");
        let iroh_topic = topic_to_iroh_topic_id(&topic);
        let bytes = iroh_topic.as_bytes();
        assert_eq!(bytes.len(), 32);
        // Re-encoding the bytes back to hex should match the A3Net
