...
//!   (the P1.2 audit gap).
//!
//! The tests intentionally avoid pulling in `adnet-node` so
//! they stay inside the `adnet-dht` crate's CI surface. The
//! transport layer they exercise is `adnet-transport`'s
...
use adnet_dht::handler::DhtProtocolHandler;
use adnet_dht::network::{DhtNetworkSender, TransportDhtSender};
use adnet_dht::protocol::DhtCodec;
use adnet_dht::transport::TransportBridge;
use adnet_dht::{
    DhtKey, DhtNode, QueryError,
};
use adnet_transport::{Frame, SharedTransport, Transport, TransportIdentity};
...
/// `adnet-node::dht_bridge::DynTransportBridge`.
struct TestBridge {
...
impl TransportBridge for TestBridge {
    ...
    async fn get_peer_addr(&self, peer: &NodeId) -> Option<adnet_types::NodeAddr> {
        let socket = self.transport.resolve_peer(peer).await?;
        Some(
            adnet_types::NodeAddr::new(peer.clone()).with_direct(adnet_types::Endpoint::new(
                socket.ip().to_string(),
                socket.port(),
            )),
        )
...
async fn spawn_dht_node() -> (
    ...
    let local_node = adnet_transport::derive_node_id_from_cert(identity.cert_der())
        .expect("derive node");
    let transport = Arc::new(
        adnet_transport::QuicTransportBuilder::new(local_node.clone(), "127.0.0.1:0".parse().unwrap())
    ...
    let config = adnet_dht::DhtConfig {
...
fn spawn_inbound_handler(
    transport: Arc<adnet_transport::QuicTransport>,
...
async fn announce_over_quic_reaches_other_node() {
    ...
    // production wiring in `adnet-node` and by the second
    ...
    let _ = tracing_subscriber::fmt()
        .with_env_filter(
            tracing_subscriber::EnvFilter::try_from_default_env()
                .unwrap_or_else(|_| tracing_subscriber::EnvFilter::new("adnet_dht=trace,info")),
        )
        .try_init();
...
async fn frame_codec_roundtrip() {
    use adnet_dht::protocol::DhtWireMessage;
    let msg = DhtWireMessage::Ping(adnet_dht::protocol::PingPayload {
        request_id: adnet_dht::protocol::RequestId("test".to_string()),
