//! `adnet-dht` 应用示例：模拟一个 ADNet 节点在本地通告自己持有的资源，
...
//!
//! 运行：`cargo run -p adnet-dht --example dht_app`

use std::net::SocketAddr;

use adnet_dht::{DhtConfig, DhtKey, DhtNode};
...
async fn main() {
    ...
    println!(
        "  hint: enable `adnet-transport` to wire a `DhtNetworkSender` and trigger real GetProviders"
    );
}