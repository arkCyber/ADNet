//! DHT protocol handler - integration with adnet-transport.
//!
//! This module provides the handler for DHT protocol messages
//! and integration with the ADNet transport layer.
...
impl DhtProtocolHandler {
    ...
    pub fn take_event_receiver(&mut self) -> mpsc::Receiver<DhtEvent> {
        let (tx, rx) = mpsc::channel(64);
        // Note: We can't move the sender, so we create a new channel
        // The handler should emit events through its own tx
        rx
    }
}
