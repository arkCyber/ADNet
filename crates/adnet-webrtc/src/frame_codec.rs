...
//!
//! For the simple case (frames ≤ 16 KiB), the ADNet [`Frame`](adnet_types)
...
/// The wire format of a single ADNet frame carried over the WebRTC
/// DataChannel.
///
/// Each frame is:
...
/// `adnet-transport::frame::FrameCodec`. We re-state it here so the
