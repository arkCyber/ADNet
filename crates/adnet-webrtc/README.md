...
WebRTC DataChannel transport for ADNet. Symmetric: native ↔ native and
...
| `adnet-transport::webrtc::WebRtcTransport` (impl `Transport`) | ⏳ Round-2 |
| Browser JS shim (`adnet-ffi/js/`) | ⏳ Round-3 |
...
```rust,no_run
use adnet_webrtc::{config::WebRtcConfig, noise_dc, rtc_engine};
