//! `a3net-webrtc` — WebRTC DataChannel transport for A3Net.
//!
//! Symmetric: it lets a native A3Net node speak to another native A3Net node
//! over WebRTC, and lets a browser running JS speak to a native A3Net node
//! over the same path. The browser gets a small JS shim in
//! `crates/a3net-ffi/js/` that ports the Noise + Frame primitives.
//!
//! ## Why WebRTC?
//!
...
//! ## Wire model
//!
//! Each A3Net node has a long-term Noise static key (32 bytes, Ed25519,
//! persisted via [`a3net_identity`]). When two peers meet over WebRTC:
//!
...
//!    `"a3net/0"` is opened in ordered+reliable mode.
...
//!    from the remote static key and we hand off to the regular A3Net frame
//!    codec.
//!
