//! `adnet-news` — bulletin (announcement + news) service.
//!
//! A single crate covers both **authoritative announcements** and
//! **news feed entries** through the unified [`adnet_types::BulletinItem`]
//! record. The service:
//!
...
//!    [`adnet_gossip::GossipTransport`] overlay (same `InProcessGossip`
//!    or `IrohGossipTransport` the rest of the node uses) on a
//!    dedicated topic name (`adnet-news-{room}`).
...
//! adnet-ffi ──┐    │    ┌────────────────┐
//!             ▼    ▼    ▼                │
//!         adnet-news::NewsService ──► BulletinStore (SQLite)
//!                  │
//!                  ▼
//!             BulletinBus ──► adnet_gossip::GossipTransport
//! ```
//!
//! [`BulletinItem`]: adnet_types::BulletinItem
...
// `adnet-gossip` just to wire a stand-alone `NewsService`.
pub use adnet_gossip::InProcessGossip;

// Re-export the user-facing payload types from `adnet_types` so
// callers can build bulletin items without depending on
// `adnet-types` themselves.
pub use adnet_types::{
