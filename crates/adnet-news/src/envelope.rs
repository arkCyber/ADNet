...
/// derived from `<prefix>-<room_id>` (matching the `adnet-room-…`
/// convention used by `adnet-gossip`), so external observers can
/// recognise the family.
pub const BULLETIN_TOPIC_PREFIX: &str = "adnet-news";
...
impl BulletinEnvelope {
    ...
    /// `signature[0]`; verifier is `adnet-identity::Wallet`).
...
pub fn topic_id(room: &adnet_types::RoomId) -> adnet_types::Topic {
    use adnet_types::Topic;
...
pub enum BulletinEvent {
    ...
    /// cards.
    Correction {
        superseded_id: adnet_types::BulletinId,
            ...
            Self::ReplayComplete { .. } => {
                ...
                EMPTY.get_or_init(|| {
                    BulletinItem::new(
                        ...
                        adnet_types::BulletinCategory::General,
                        adnet_types::BulletinSeverity::Info,
                        adnet_types::RoomId::new("__internal__"),
                        NodeId::random(),
                        "",
                        "internal",
...
/// sender. Mirrors `AnnouncementRx` in `adnet-gossip`.
...
/// envelope layer doesn't grow dependent on `adnet-types` for the
/// auxiliary helpers.
...
mod tests {
    use super::*;
    use adnet_types::{BulletinCategory, BulletinSeverity, RoomId};
    use chrono::Utc;
