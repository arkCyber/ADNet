//! End-to-end pairing ceremony example for adnet-pairing.
//!
//! Walks through every step that the ADNet transport follows when
...
//!
//! ```bash
//! cargo run -p adnet-pairing --example pairing_app
//! ```

use adnet_identity::wallet::Wallet;
use adnet_pairing::transport_identity::{derive_credential_id, Ed25519Signer};
use adnet_pairing::{
...
use adnet_pairing::trusted_device::TrustedDeviceRole;
use adnet_types::node::NodeId;

fn main() -> Result<(), Box<dyn std::error::Error>> {
    let tmpdir = std::env::temp_dir().join(format!("adnet-pairing-app-{}", std::process::id()));
