...
//! `adnet-pairing://<base64url>` URL. For QR scanning this is ideal —
...
//!
//! ```rust,ignore
//! use adnet_pairing::{SignedInvitation, InvitationCode};
//!
...
impl std::str::FromStr for InvitationCode {
    ...
    fn from_str(s: &str) -> Result<Self, Self::Err> {
        ...
        let raw = if let Some(stripped) = s.strip_prefix(CODE_PREFIX) {
            stripped
        } else if let Some(stripped) = s.strip_prefix("adnet:") {
            stripped
        } else {
            s
        };
...
mod tests {
    ...
    fn code_format_validation() {
        ...
        let lowercase = "adnet:abcd-efgh-jklm-npqr";
