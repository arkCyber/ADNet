...
//! invitation can be presented to the user. The ADNet QR scanner
//! (`adnet-qr`) will classify `adnet-pairing://` URLs as this type,
//! and the email invitation (`adnet-invite`) will attach a
//! `application/x-adnet-pairing` JSON blob.
//!
//! Supported URL schemes:
//!
//! | Scheme                        | Context              |
//! |-------------------------------|----------------------|
//! | `adnet-pairing://<base64url>` | QR code, deep link   |
//!
...
const PAIRING_URL_PREFIX: &str = "adnet-pairing://";

/// Unified QR / email invitation type. `adnet-qr`'s scanner will
/// classify any `adnet-pairing://` URL as this variant.
///
/// Variants:
...
impl PairingInvitation {
    /// Parse an `adnet-pairing://<base64url>` URL into a
...
// `adnet-types::ticket`. We could depend on `base64` but this is
