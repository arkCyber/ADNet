...
//!
//! The format is `b"adnet-pairing-request/v1\0" || fields…` for
//! requests and `b"adnet-pairing-response/v1\0" || fields…` for
...
use adnet_identity::signing::PersonalSignature;
use adnet_identity::wallet::{Wallet, WalletPublic};
...
/// convention started by `adnet-types::SignedPeerTicket` (tag `1`
...
/// and invitee. Smaller than `adnet-types::announce::MAX_TIMESTAMP_SKEW_HOURS`
...
const REQUEST_DOMAIN_TAG: &[u8] = b"adnet-pairing-request/v1\0";
const RESPONSE_DOMAIN_TAG: &[u8] = b"adnet-pairing-response/v1\0";
/// Domain tag for the invitation payload digest (QR / email side).
pub const INVITATION_DOMAIN_TAG: &[u8] = b"adnet-pairing-invitation/v1\0";
...
/// `blake3::hash("adnet-pairing/credential-id/v1" || issuer || invitee || salt)[..16]`.
...
impl<'a> PairingResponseBuilder<'a> {
    pub fn build(self) -> PairingResult<PairingResponse> {
        ...
        // `adnet_token::Pledge` so the existing `Wallet` flow
...
pub fn derive_credential_id(issuer: &NodeId, invitee: &NodeId, salt: &[u8; 32]) -> CredentialId {
    ...
    let mut h = Hasher::new();
    h.update(b"adnet-pairing/credential-id/v1\0");
