...
pub struct TrustedDeviceStore {
    ...
    /// table so the rest of ADNet can react to a pairing / a
    /// revocation without each call site wiring it up. Opt-in to
    /// avoid forcing a dependency on `adnet-reputation` when the
    /// pairing layer is used standalone.
    #[cfg(feature = "reputation")]
    reputation: std::sync::Mutex<Option<adnet_reputation::ReputationReporter>>,
}
...
impl TrustedDeviceStore {
    ...
    /// Attach a [`adnet_reputation::ReputationReporter`] so that
    ...
    fn record_pairing_established(&self, rec: &TrustedDeviceRecord) {
        ...
        if let Some(rep) = guard.as_ref() {
            ...
            if let Ok(peer) = peer {
                adnet_reputation::reporter::PairingSignal(rep).established(
                    peer,
    ...
    fn record_pairing_revoked(&self, rec: &TrustedDeviceRecord) {
        ...
        if let Some(rep) = guard.as_ref() {
            let peer = parse_node_id(&rec.node_id);
            if let Ok(peer) = peer {
                adnet_reputation::reporter::PairingSignal(rep).revoked(
                    peer,
                    short_cred(&rec.credential_id),
                );
            }
...
/// Parse a 64-hex `node_id` field into an `adnet_types::NodeId`. We
...
#[cfg(feature = "reputation")]
fn parse_node_id(hex: &str) -> Result<adnet_types::NodeId, String> {
    if hex.len() != 64 || !hex.chars().all(|c| c.is_ascii_hexdigit()) {
        return Err(format!("invalid node_id hex ({} chars)", hex.len()));
    }
    adnet_types::NodeId::from_hex(hex).map_err(|e| e.to_string())
}
