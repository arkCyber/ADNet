...
> Core type definitions shared across the A3Net workspace — `NodeId`, `ContentHash`, `Announcement`, `WalletAddress`, `BlobTicket`, plus wire-format invariants for the gossip / RPC / FFI layers.

## 概览 (Overview)

`a3net-types` is the **wire-format bedrock** of A3Net. Every other crate in the workspace depends on it for the stable, low-level types that flow over gossip, RPC, and the FFI boundary: 32-byte `NodeId` hex identifiers, BLAKE3 `ContentHash` digests, `Announcement` payloads that peers broadcast into rooms, tickets that pin a node id to a network address and a content hash, and the configured limits (`MAX_*_LEN`) that the gossip validator enforces.

The crate is **deliberately crypto-free**. The `secp256k1` / `x25519-dalek` / `aes-gcm` dependencies live in `a3net-identity` so that this crate stays cheap to compile and can be depended on by `a3net-blobstore`, `a3net-rpc`, `a3net-ffi`, and any other leaf that should not pull in a full crypto stack. Anything that needs to *sign* or *verify* an `Announcement` reaches into `a3net-identity`.
...
- **`BlobTicket` / `PeerTicket` / `NodeAddrTicket`** — `a3net-blob://`, `a3net-peer://`, `a3net-addr://` URLs with `encode()` / `parse()` and a `http_base()` helper for the mesh fallback.
...
- **`WalletAddress`** — 20-byte EVM-style address (no crypto, just byte storage and hex conversion; crypto lives in `a3net-identity`).
...
- **`feature = "protobuf"`** — exposes `proptest` / `chrono` plumbing for protobuf-backed announcements (used by `a3net-types` itself in tests, opt-in by default).

## 安装 (Installation)

`a3net-types` is a workspace crate, so any other A3Net crate can simply `use a3net_types::...`. No `Cargo.toml` change is required.

```toml
# In a new crate that depends on a3net-types:
[dependencies]
...
let url = ticket.encode();           // a3net-blob://<id>@10.0.0.42:9000/<hash>
...
- **A3Net gossip bus** — every JSON gossip frame is an `Announcement` (or a `PeerSource` map). The gossip validator runs `Announcement::validate()` against the `MAX_*_LEN` constants before forwarding.
- **`a3net-blobstore`** — uses `ContentHash` to key blocks, `BlobTicket` to publish download URLs, and `RangeSpec` to honour HTTP `Range:` requests.
- **`a3net-rpc` / `a3net-ffi`** — exposes typed commands that take / return `ContentHash`, `NodeId`, and `BlobTicket`, so the IPC layer never has to parse raw strings.
- **`a3net-identity`** — wraps `PlainWalletAddress` with EVM-style signing primitives; the two types interop via the `From` impls in `a3net-identity`.
