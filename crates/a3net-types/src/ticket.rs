...
//! - [`PeerTicket`][]:  `a3net-peer://<node_id>@<host>:<port>`
//! - [`NodeAddrTicket`][]: iroh-style `NodeAddr` printable form
//! - [`BlobTicket`][]:  `a3net-blob://<node_id>@<host>:<port>/<hash>[/range]`
//!
...
impl PeerTicket {
    pub fn encode(node_id: &NodeId, endpoint: &NodeAddr) -> String {
        format!("a3net-peer://{}", endpoint.display_with_id(node_id))
    }

    pub fn parse(raw: &str) -> Result<Self> {
        let rest = raw
            .strip_prefix("a3net-peer://")
...
impl NodeAddrTicket {
    pub fn encode(addr: &NodeAddr) -> String {
        format!("a3net-addr://{}", addr.display())
    }

    pub fn parse(raw: &str) -> Result<Self> {
        let rest = raw
            .strip_prefix("a3net-addr://")
            .ok_or_else(|| AdnetError::InvalidTicket(raw.to_string()))?;
...
impl BlobTicket {
    ...
    pub fn parse(raw: &str) -> Result<Self> {
        let rest = raw
            .strip_prefix("a3net-blob://")
            .ok_or_else(|| AdnetError::InvalidTicket(raw.to_string()))?;
        // The blob ticket is `a3net-blob://<addr-display>/<hash>[/<range>]`.
...
///
/// A blob ticket looks like `a3net-blob://<addr-display>/<hash>[/<range>]`.
...
/// produced it.
///
/// The signature is *opaque* from `a3net-types`' perspective — 64 raw bytes
...
///
/// Wire format: `a3net-signed-peer://<base64-url-payload>` where the
...
pub struct SignedPeerTicket {
    ...
    /// are enforced at the verifier in `a3net-identity`.
...
impl SignedPeerTicket {
    ...
    pub fn encode(&self) -> String {
        ...
        format!("a3net-signed-peer://{}", base64_url_encode(&bytes))
    }

    pub fn parse(raw: &str) -> Result<Self> {
        let rest = raw
            .strip_prefix("a3net-signed-peer://")
            .ok_or_else(|| AdnetError::InvalidTicket(raw.to_string()))?;
...
mod tests {
    ...
    fn invalid_ticket_rejected() {
        ...
        assert!(BlobTicket::parse("a3net-blob://nope@1.2.3.4:80/short").is_err());
    }
