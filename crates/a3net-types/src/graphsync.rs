...
//!
//! 上层（`a3net-blobstore` / `a3net-node` 等）把 wire
...
///
/// A3Net 实现覆盖 IPLD Selector v1 的核心子集：
///
/// | A3Net 变体                  | IPLD Selector 等价                     |
/// |-----------------------------|----------------------------------------|
/// | `All`                       | `{"matcher": "All"}`                  |
/// | `None`                      | `{"matcher": "None"}`                 |
/// | `Leaf`                      | `{"matcher": "Leaf"}`                 |
...
mod tests {
    ...
    /// `BlockStore`. We can't reuse `a3net_blobstore::MemDagStore`
    /// here because `a3net-types` cannot depend on `a3net-blobstore`
