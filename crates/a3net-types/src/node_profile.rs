...
//! self-describing [`NodeProfile`] that every A3Net node carries.
//!
//! ## Design goals
//!
...
//! ## Node role taxonomy
//!
//! The four roles cover the full device spectrum A3Net must support:
//!
//! | Role          | Devices                                              |
//! |---------------|------------------------------------------------------|
...
/// Node role — the position this node occupies in the A3Net topology.
///
/// Used by:
...
pub enum NodeRole {
    ...
    /// for a newly-provisioned A3Net install. Supports full protocol suite:
...
///
/// ```
/// use a3net_types::NodeCapability;
///
...
/// Custom caps use bits 32-63; bits 0-31 are reserved for the A3Net
/// standard set.
...
impl NodeCapability {
    // Individual capability constants — lower 32 bits reserved for A3Net.
    // BIT 0
    pub const RELAY: NodeCapability = NodeCapability(1 << 0);
    /// MQTT broker / bridge capability.  This node can forward
    /// MQTT messages between the A3Net gossip layer and an external
    /// MQTT broker (see `a3net-mqtt`).
    ...
    /// SSH connections over QUIC (see `a3net-ssh`).
    ...
    /// A3Net set; bits 32-63 may be used for custom capability extensions).
...
/// The self-describing identity packet that every A3Net node carries.
///
/// Persisted to `{data_dir}/node_profile.json` and published via
/// [`NodeProfileAnnouncement`] on the `a3net-room-profile` gossip topic.
///
/// ## Wire format
///
...
pub struct NodeProfile {
    ...
    /// A3Net software version string, e.g. `"0.1.0"`.
...
impl NodeProfile {
    ...
    /// Infer a default [`NodeRole`] + capability set from a
    /// [`crate::node_identity::NodeKind`] (operator-class taxonomy).
    ///
    /// Use this when bootstrapping a freshly-provisioned node and
    /// the operator hasn't yet picked a role. The two taxonomies are
    /// orthogonal, but the defaults below are the ones that make
    /// sense for the common case of each kind:
    ///
    /// | `NodeKind`       | Default `NodeRole` | Default `NodeCapability`              |
    /// |------------------|--------------------|---------------------------------------|
    /// | `AiAgent`        | `Specialized`      | `AI_AGENT` + `AI_INFERENCE`           |
    /// | `AiCompute`      | `Specialized`      | `AI_INFERENCE` + `BLOB_STORAGE`       |
    /// | `Human`          | `Standard`         | `BLOB_STORAGE` + `WORKSPACE_HOST`     |
    /// | `NetworkManager` | `Specialized`      | `RELAY` + `MESH_MONITOR` + `DNS_RESOLVER` |
    ///
    /// Callers can subsequently call [`NodeProfile::set_role`] /
    /// [`NodeProfile::with_capabilities`] to override.
    ///
    /// ## `AiAgent` vs `AiCompute` capabilities
    ///
    /// `AiAgent` advertises `AI_AGENT` (it can receive `agent.v1`
    /// requests) and `AI_INFERENCE` (it may itself embed a model for
    /// offline operation). `AiCompute` advertises only `AI_INFERENCE`
    /// — it is the canonical "I serve tokens" endpoint that other
    /// nodes reach via `RemoteAgentModel`. It does **not** advertise
    /// `AI_AGENT` because it doesn't host an agent loop.
    pub fn infer_from_node_kind(
        node_id: crate::node::NodeId,
        kind: crate::node_identity::NodeKind,
        version: impl Into<String>,
    ) -> Self {
        match kind {
            crate::node_identity::NodeKind::AiAgent => Self {
                node_id,
                role: NodeRole::Specialized,
                capabilities: NodeCapability::AI_AGENT | NodeCapability::AI_INFERENCE,
                resources: None,
                description: None,
            ...
            crate::node_identity::NodeKind::AiCompute => Self {
                node_id,
                role: NodeRole::Specialized,
                capabilities: NodeCapability::AI_INFERENCE | NodeCapability::BLOB_STORAGE,
                resources: None,
                description: None,
                tags: vec!["ai".to_string(), "compute".to_string(), "inference".to_string()],
                version: version.into(),
                published_at: current_timestamp(),
            },
            crate::node_identity::NodeKind::Human => Self::standard(node_id, version),
            crate::node_identity::NodeKind::NetworkManager => Self {
                node_id,
                role: NodeRole::Specialized,
                ...
                resources: None,
                description: None,
                tags: vec!["ops".to_string(), "manager".to_string()],
                version: version.into(),
                published_at: current_timestamp(),
            },
        }
...
mod tests {
    ...
    #[test]
    fn profile_infer_from_kind_ai_agent() {
        let id = crate::node::NodeId::random();
        let p = NodeProfile::infer_from_node_kind(
            id.clone(),
            crate::node_identity::NodeKind::AiAgent,
            "0.1.0",
        );
        assert_eq!(p.role, NodeRole::Specialized);
        assert!(p.capabilities.contains(NodeCapability::AI_AGENT));
        assert!(p.capabilities.contains(NodeCapability::AI_INFERENCE));
        // AiAgent advertises AI_AGENT but is NOT a pure inference provider.
        assert!(p.tags.contains(&"agent".to_string()));
    }

    #[test]
    fn profile_infer_from_kind_ai_compute() {
        let id = crate::node::NodeId::random();
        let p = NodeProfile::infer_from_node_kind(
            id.clone(),
            crate::node_identity::NodeKind::AiCompute,
            "0.1.0",
        );
        assert_eq!(p.role, NodeRole::Specialized);
        // AiCompute is the producer end-point — AI_INFERENCE without
        // AI_AGENT (no agent loop on this node).
        assert!(p.capabilities.contains(NodeCapability::AI_INFERENCE));
        assert!(p.capabilities.contains(NodeCapability::BLOB_STORAGE));
        assert!(
            !p.capabilities.contains(NodeCapability::AI_AGENT),
            "AiCompute node must not advertise AI_AGENT"
        );
        assert!(p.tags.contains(&"compute".to_string()));
        assert!(p.tags.contains(&"inference".to_string()));
    }

    #[test]
    fn profile_infer_from_kind_human() {
        let id = crate::node::NodeId::random();
        let p = NodeProfile::infer_from_node_kind(
            id.clone(),
            crate::node_identity::NodeKind::Human,
            "0.1.0",
        );
        assert_eq!(p.role, NodeRole::Standard);
        assert!(p.capabilities.contains(NodeCapability::BLOB_STORAGE));
    ...
    #[test]
    fn profile_infer_from_kind_network_manager() {
        let id = crate::node::NodeId::random();
        let p = NodeProfile::infer_from_node_kind(
            id.clone(),
            crate::node_identity::NodeKind::NetworkManager,
            "0.1.0",
        );
        assert_eq!(p.role, NodeRole::Specialized);
        assert!(p.capabilities.contains(NodeCapability::RELAY));
        assert!(p.capabilities.contains(NodeCapability::MESH_MONITOR));
        assert!(p.capabilities.contains(NodeCapability::DNS_RESOLVER));
    }
