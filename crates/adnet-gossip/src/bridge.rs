...
//! [`Announcement`](adnet_types::Announcement) and the JSON-friendly
//! [`AnnouncementPayload`](adnet_types::AnnouncementPayload) used over the
//! wire.

use adnet_types::{Announcement, AnnouncementPayload};

/// Wrap an announcement for the wire, attributing it to `from_node`.
pub fn wrap(ann: &Announcement, from_node: &adnet_types::NodeId) -> AnnouncementPayload {
    AnnouncementPayload {
...
impl GossipBridge {
    pub fn wrap(&self, ann: &Announcement, from_node: &adnet_types::NodeId) -> AnnouncementPayload {
        wrap(ann, from_node)
    }
...
mod tests {
    use super::*;
    use adnet_types::{CdnContentKind, ContentHash, NodeId, RoomId, Topic};
    use chrono::Utc;
    ...
    fn bridge_uses_topic_naming_convention() {
        ...
        // forcing callers to reach into `adnet_types` directly.
        let _ = Topic::from_label("adnet-room-x");
    }
}
