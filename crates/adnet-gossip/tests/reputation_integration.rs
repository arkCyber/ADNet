//! Integration tests for the `adnet-reputation` crate's gossip signal
//! path. Requires the `reputation` feature on `adnet-gossip` so that
...
use adnet_types::{AnnouncementPayload, CdnContentKind, ContentHash, NodeId, RoomId};
use chrono::Utc;

fn make_ann(publisher: &NodeId, room: &RoomId, tag: &str) -> adnet_types::Announcement {
