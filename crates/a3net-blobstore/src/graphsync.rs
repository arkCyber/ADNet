...
//! `a3net_types::graphsync` to the async network plane. It does not
...
//! ```
//!
//! Frames are framed by `a3net_transport::Frame` (length-prefixed).
//!
//! ## Sync vs async
//!
...
/// ALPN for GraphSync.
pub const GRAPHSYNC_ALPN: &[u8] = b"a3net/graphsync/1";
...
// (GraphSync QUIC bridge and service live in `a3net-node`'s
//  `graphsync` module — `a3net-blobstore` cannot depend on
//  `a3net-transport` without a cycle.)
