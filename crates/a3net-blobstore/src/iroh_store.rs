...
//! format. A3Net's legacy [`ContentHash`] can be converted via
...
/// Convert an A3Net [`ContentHash`] (hex-encoded BLAKE3 over blob
...
/// Convert an iroh [`IrohHash`] back to an A3Net [`ContentHash`].
...
mod tests {
    ...
    fn content_hash_iroh_hash_round_trip() {
        ...
        let a3net_hash = ContentHash::from_hex(&hex::encode(store_bytes)).unwrap();
        let iroh_hash = content_hash_to_iroh_hash(&a3net_hash).unwrap();
        assert_eq!(iroh_hash.as_bytes(), &store_bytes);
        let back = iroh_hash_to_content_hash(&iroh_hash);
        assert_eq!(back.as_hex(), a3net_hash.as_hex());
    }
