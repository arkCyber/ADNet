...
//!
//! Run with:
//!     cargo test -p a3net-blobstore --test distributed_storage_aerospace --features aerospace

#![cfg(feature = "aerospace")]

use std::sync::Arc;

use a3net_blobstore::{BlobStore, DEFAULT_REPLICATION_FACTOR};
...
async fn sr_6_three_replicas_baseline() {
    ...
    for block_idx in 0..4 {
        ...
        let block_hash_inner = a3net_types::ContentHash::from_bytes(&payload[start..end]);
...
async fn sr_7_dropout_re_replicates() {
    ...
    let block_hash_0 = a3net_types::ContentHash::from_bytes(&payload[..256 * 1024]);
    let block_hash_1 = a3net_types::ContentHash::from_bytes(&payload[256 * 1024..]);
...
async fn byzantine_peer_rejected_by_hash_check() {
    ...
    assert!(matches!(err, a3net_blobstore::ReplicatorError::Refused(_)));
}
...
async fn block_replica_count_includes_local() {
    use a3net_blobstore::replicator::BlockReplica;
