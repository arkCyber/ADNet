...
//!
//! A3Net's transport crate already speaks DERP **as a client** —
//! [`a3net_transport::iroh::IrohTransport`] binds an `iroh::Endpoint`
...
//! fully self-hosted network so A3Net nodes don't need to talk to
...
//! Wrapping it gives us A3Net-style configuration (persistable
...
//!   SIGINT handler.
//! - [`access`] — A3Net-flavoured [`iroh_relay::server::AccessControl`]
...
//! enabled on `a3net-relay`. Building `a3net-relay` without the
...
//! `cargo build -p a3net-relay --features derp`.
...
///
/// This is the A3Net-side mirror of the upstream
...
/// Build the upstream [`ServerConfig`] from an A3Net [`DerpConfig`].
