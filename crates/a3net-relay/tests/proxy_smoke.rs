...
//! run in `cargo test -p a3net-relay --test proxy_smoke`.
//!
//! Run with:
//!
//! ```text
//! cargo test -p a3net-relay --test proxy_smoke
//! ```

use std::net::SocketAddr;

use a3net_relay::{HostPolicy, ServerPolicy};
...
async fn start_relay(policy: ServerPolicy, port: u16) -> a3net_relay::RelayServerHandle {
    a3net_relay::RelayServer::start_with_policy(
        "127.0.0.1",
        port,
        a3net_relay::BillingMode::Disabled,
        policy,
    )
    .await
    .expect("relay start")
