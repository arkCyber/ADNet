...
//
// Performance & stability baseline tests for `a3net-relay`.
//
...
use a3net_relay::{HostPolicy, RelayServer, ServerPolicy};
use serde_json::json;
...
async fn spawn_relay(policy: ServerPolicy) -> (a3net_relay::RelayServerHandle, TempDir) {
    let dir = TempDir::new().expect("tempdir");
    let handle =
        RelayServer::start_with_policy("127.0.0.1", 0, a3net_relay::BillingMode::Disabled, policy)
            .await
            .expect("relay start");
    (handle, dir)
}
