//! `a3net-relay` self-hosted DERP entrypoint.
//!
//! Run with:
//!
//! ```bash
//! cargo run -p a3net-relay --example derp_server --features derp
//! ```
//!
...
//! A3Net nodes at it via `relay_base_url`. The server speaks the
//! upstream `iroh-relay` wire protocol, so any iroh / A3Net
...
//!   `a3net_relay::derp::DerpServer::spawn(cfg)`.
...
//!    cert, key }` — see `a3net_relay::derp::DerpTlsConfig`.
...
//! `a3net-relay` exposes the server as a public example rather
//! than a separate crate — operators who want a "fat binary"
//! self-host daemon can copy this file into their own
//! `a3net-selfhost` crate and add HTTP admin endpoints, ACME
//! renewal jobs, etc. on top.

use std::path::PathBuf;

use a3net_relay::derp::{DerpConfig, DerpServer, DerpServerHandle};
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    // Load the config. Operators can pass `--config /etc/a3net/derp.json`
...
///   "tls": { "manual": { "cert": "/etc/a3net/fullchain.pem", "key": "/etc/a3net/privkey.pem" } },
