...
`adnet-media` 是 ADNet 中负责短视频(短片)处理的整条管线。它不是通用视频框架,而是面向"用户上传一段短视频到 NAS,后台做格式转换 + 分段 + 完整性校验"这一具体场景的工业级实现。设计上遵守 DO-178C DAL-B:每一个解码阶段的输出都带长度前缀与 BLAKE3 哈希,任何截断 / 篡改都会被 `verify_dag` 立刻发现。
...
5. **DAG (`dag`)** —— `MediaDagBuilder` 把所有 variant + segment + audio 节点合成一棵内容寻址 DAG,根哈希 = `blake3("adnet-media-v1" || ...)`,用 `MediaDag::verify(root_hash)` 验证整段。

辅助模块:`ffmpeg` 提供 `FFmpegTranscoder` + `FFmpegLocator`,`ffmpeg_probe` 读取元数据,`integrity` 给独立的 segment / root hash 工具,`persist` 提供 `MediaStore` 把 DAG 落到 `adnet-blobstore::BlobStore`。
...
```rust
use adnet_media::ffmpeg::{FFmpegConfig, FFmpegTranscoder};
...
```rust
use adnet_media::ffmpeg::transcode_synthetic;
...
```rust
use adnet_media::{MediaStore, MediaStoreReport};
...
- **朋友圈短视频发布**:`adnet-socialfeed` 收到附件上传时,调 `MediaIngester::ingest` 生成 DAG,把根哈希写到 `PostAttachment::blob_hash`。
- **NAS 离线转码**:夜间任务把 `*.mov` 转成 720p H.264 + 480p H.264 双档,各自 `Segmenter` 切片,DAG 落到 `adnet-blobstore` 走 CDN。
