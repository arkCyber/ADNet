...
async fn transcoder_emits_progress_callback() {
    ...
    let cb: adnet_media::ffmpeg::ProgressCallback = std::sync::Arc::new(move |_label, pct| {
        if pct > 0 {
