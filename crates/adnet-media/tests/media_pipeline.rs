...
//!
//! ```sh
//! cargo test -p adnet-media --test media_pipeline
//! ```

use adnet_media::codec::{AudioCodec, SampleFormat, VideoCodec};
use adnet_media::dag::MediaDagBuilder;
use adnet_media::ingest::MediaIngester;
use adnet_media::manifest::MediaManifest;
...
fn full_pipeline_2s_clip() {
    ...
    let video_variants = report
        .segments
        .iter()
        .filter(|s| matches!(s.kind, adnet_media::segment::SegmentKind::Video))
        .count();
    let audio_segments = report
        .segments
        .iter()
        .filter(|s| matches!(s.kind, adnet_media::segment::SegmentKind::Audio))
        .count();
...
fn longer_clip_60s() {
    ...
    let video_segments_240p = report
        .segments
        .iter()
        .filter(|s| matches!(s.kind, adnet_media::segment::SegmentKind::Video) && s.index == 0)
        .count();
