//! Persistence — bind the media DAG to `adnet-blobstore`.
//!
//! ## Storage layout
//!
//! ```text
...
// constructing an adnet_types ContentHash.
...
impl BlobStoreHexExt for BlobStore {
    fn get_sync_by_hex(&self, hex: &str) -> Option<Vec<u8>> {
        let hash = adnet_types::content::ContentHash::from_hex(hex).ok()?;
        self.get_sync(&hash)
    }
    fn has_complete_by_hex(&self, hex: &str) -> bool {
        let Ok(hash) = adnet_types::content::ContentHash::from_hex(hex) else {
            return false;
        };
