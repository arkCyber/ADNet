//! `a3net-tun` userspace roundtrip — drives the
//! [`a3net_tun::UserspaceTun`] through one full packet cycle
//! without involving a real kernel interface.

use a3net_tun::{TunDevice, UserspaceTun, UserspaceTunConfig};
use a3net_types::{VirtualIpv4, VirtualIp};

#[tokio::main]
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    let cfg = UserspaceTunConfig {
        name: "a3net-tun-demo".into(),
        mtu: 1420,
