...
> 把 `a3net-blobstore` 命名空间以 WebDAV 协议暴露给 Finder / Explorer / 第三方客户端;每个动词经能力令牌校验、NDJSON 审计;DO-178C DAL-A 安全规范。
>
> Expose the `a3net-blobstore` namespaces over RFC 4918 WebDAV so Finder / Explorer / any third-party client can mount the home NAS; every verb is capability-gated and NDJSON-audited; DO-178C DAL-A safety profile.

## 概览(Overview)

`a3net-webdav` 是一个小型 DO-178C **DAL-A** 模块,作用是把 A3Net 的 NAS 命名空间(`a3net-blobstore::Nas`)以 RFC 4918 WebDAV 协议挂载出来,让 macOS Finder / Windows Explorer / 任何 WebDAV 客户端都能像挂载 SMB 一样挂载家里的 NAS。设计上由五块组成:
...
a3net-webdav = { workspace = true }
a3net-pairing = { workspace = true }
```
...
```rust
use a3net_webdav::{ResolvedCapability, StaticCapabilityResolver};
use a3net_pairing::CapabilitySet;
...
```rust
use a3net_webdav::{CapabilityToken, TokenVerifier};
...
```rust
use a3net_webdav::{AclMiddleware, AclDecision};
use a3net_pairing::CapabilitySet;
...
- **DO-178C 合规检查**:`cargo test -p a3net-webdav --features aerospace` 跑 `tests/dal_a_compliance.rs`,确认 `SAFETY_REVISION` 与 `HAZARD_REGISTER_REV` 匹配。
- **跨设备配对**:`a3net-pairing` 颁发的 capability 天然带 `nonce` / `expires_unix_ms`,挂在 Finder 上 30 天后自动失效,无需重新配对。
