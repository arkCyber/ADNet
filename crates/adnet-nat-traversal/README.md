...
`adnet-nat-traversal` 把 4 种穿透手段装到同一个 `NatTraversalManager` 后面：
...
`NatInfo` 写回共享状态；上层（`adnet-transport`、`adnet-node`）拿到 `NatInfo` 后
...
```rust
use adnet_nat_traversal::{NatTraversalManager, NatConfig, NatInfo};
```
...
```rust,no_run
use adnet_nat_traversal::{NatConfig, NatTraversalManager};
...
```rust,no_run
use adnet_nat_traversal::{StunClient, StunServer};
...
```rust,no_run
use adnet_nat_traversal::{UpnpClient, PortMappingProtocol, PortMapping};
...
- **`adnet-transport`** 在 `QuicTransport` 拨号失败时调用 `NatTraversalManager::discover()`
  重新拿公网地址，更新自己的 `Endpoint`。
- **`adnet-node`** 把 `NatInfo::external_addr` 放到 provider record 里，其它 peer 能直接 dial。
