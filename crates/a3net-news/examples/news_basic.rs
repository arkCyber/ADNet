...
//! Run with:
//! ```bash
//! cargo run -p a3net-news --example news_basic
//! ```

use a3net_gossip::InProcessGossip;
use a3net_news::{NewsService, NewsServiceConfig, ValidationPolicy};
use a3net_types::{BulletinCategory, BulletinKind, BulletinSeverity, NodeId, RoomId};
use std::sync::Arc;
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let mut item = a3net_types::BulletinItem::new(
        BulletinKind::Announcement,
        BulletinCategory::General,
        BulletinSeverity::Info,
        RoomId::new("lobby"),
        local.clone(),
        "Welcome to A3Net news",
        "First bulletin",
        "Hello world from a3net-news.",
        &[0xAA; 16],
        None,
    )?;
