...
//! verification — that lives behind `a3net-identity::Wallet`. The
...
use a3net_gossip::{GossipTransport, TopicId};
use a3net_types::{
...
impl Default for NewsServiceConfig {
    fn default() -> Self {
        Self {
            store_dir: std::env::temp_dir().join("a3net-news"),
...
impl NewsService {
    ...
    pub async fn publish(&self, mut item: BulletinItem) -> NewsResult<BulletinItem> {
        ...
        let wire = a3net_gossip::AnnouncementPayload {
    ...
    pub async fn ingest_envelope(&self, envelope: BulletinEnvelope) -> NewsResult<BulletinItem> {
        ...
        let event = if envelope.item.kind == a3net_types::BulletinKind::Correction
        ...
        } else if envelope.item.kind == a3net_types::BulletinKind::Retraction
            && envelope.item.supersedes.is_some()
        {
...
// a3net-gossip's `broadcast` re-export shim: ensure a3net-gossip is
// in scope even when callers don't pull it in.
#[allow(dead_code)]
fn _gossip_transport_bound(_t: &dyn GossipTransport) {}

// a3net-gossip topic id alias so we don't accidentally drift.
#[allow(dead_code)]
