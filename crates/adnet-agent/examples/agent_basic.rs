...
//!
//! ```bash
//! cargo run -p adnet-agent --example agent_basic
//! ```
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    let model: Arc<dyn ChatModel> = Arc::new(MockChatModel::text(
        "Hello, human! I am an ADNet agent running on a mock model.",
    ));
    let registry = ToolRegistry::new();

    let agent = Agent::new(model, registry);
    let handle = agent.handle();
    handle.push_system("You are a concise ADNet assistant.");
