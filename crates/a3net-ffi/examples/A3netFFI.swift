...
//
//   - `A3netNode` owns a tokio runtime inside the FFI handle.
...
//     needed).
//   - `A3netNode` is **not** `Send`/`@unchecked Sendable` —
...
/// Errors surfaced by the FFI layer. Mirrors `A3netFfiError` in
/// the Rust crate; numeric codes are stable and may be matched
/// in `switch` statements.
public enum A3netFfiError: Error, Equatable {
    ...
    init(status: Int32, message: String) {
        switch status {
        case A3NET_FFI_E_INVALID_ARG: self = .invalidArg(message)
        case A3NET_FFI_E_UTF8:        self = .utf8(message)
        case A3NET_FFI_E_JSON:        self = .json(message)
        case A3NET_FFI_E_NODE:        self = .node(message)
        case A3NET_FFI_E_RUNTIME:     self = .runtime(message)
        case A3NET_FFI_E_FEATURE:     self = .feature(message)
    ...
    public var statusCode: Int32 {
        switch self {
        case .invalidArg: return A3NET_FFI_E_INVALID_ARG
        case .utf8:       return A3NET_FFI_E_UTF8
        case .json:       return A3NET_FFI_E_JSON
        case .node:       return A3NET_FFI_E_NODE
        case .runtime:    return A3NET_FFI_E_RUNTIME
        case .feature:    return A3NET_FFI_E_FEATURE
...
public final class A3netNode {
    ...
    public init(dataDir: String) throws {
        ...
        var buffer = A3netFfiBuffer(ptr: nil, len: 0)
    ...
    public func nodeId() throws -> String {
        ...
        guard let data = Self.dataFromBuffer(buffer) else {
            throw A3netFfiError.node("empty response")
        }
    ...
    fileprivate static func check(status: Int32, buffer: A3netFfiBuffer) throws {
        if status == A3NET_FFI_OK { return }
        ...
        throw A3netFfiError(status: status, message: message)
    }

    fileprivate static func dataFromBuffer(_ buffer: A3netFfiBuffer) -> Data? {
...
// `A3netFfiBuffer` for the response, and `defer`s
// `a3net_ffi_free` so we never leak.

extension A3netFfiError {
    /// Convenience init that decodes an `FfiResult<()>` JSON body.
    static func fromResponse(status: Int32, buffer: A3netFfiBuffer) -> A3netFfiError {
        let message = (buffer.ptr != nil) ? A3netNode.dataFromBuffer(buffer).flatMap {
            String(data: $0, encoding: .utf8)
        } ?? "" : ""
        if buffer.ptr != nil { a3net_ffi_free(buffer) }
        return A3netFfiError(status: status, message: message)
    }
}

/// Free-function helpers. The `A3netRoster` type is just a thin
/// namespace; the store is owned by the FFI for the duration of
/// each call.
public enum A3netRoster {
    ...
    typealias PayloadFn = (UnsafePointer<CChar>?, Int, UnsafePointer<CChar>?, Int, UnsafeMutablePointer<A3netFfiBuffer>) -> Int32
    typealias NoPayloadFn = (UnsafePointer<CChar>?, Int, UnsafeMutablePointer<A3netFfiBuffer>) -> Int32
    typealias StringFn = (UnsafePointer<CChar>?, Int, UnsafePointer<CChar>?, Int, UnsafeMutablePointer<A3netFfiBuffer>) -> Int32
    typealias StringBoolFn = (UnsafePointer<CChar>?, Int, UnsafePointer<CChar>?, Int, UnsafeMutablePointer<A3netFfiBuffer>) -> Int32

    static func call(dataDir: String, payload: Data, f: PayloadFn) throws {
        var out = A3netFfiBuffer(ptr: nil, len: 0)
        ...
        try A3netNode.check(status: status, buffer: out)
    ...
    static func callNoPayload(dataDir: String, f: NoPayloadFn) throws -> Data {
        ...
        // We can't use A3netNode.check because we want the body.
        if status != A3NET_FFI_OK {
            if let data = A3netNode.dataFromBuffer(out), let s = String(data: data, encoding: .utf8) {
                a3net_ffi_free(out)
                throw A3netFfiError(status: status, message: s)
            }
            a3net_ffi_free(out)
            throw A3netFfiError(status: status, message: "<no body>")
        }
        defer { if out.ptr != nil { a3net_ffi_free(out) } }
        return A3netNode.dataFromBuffer(out) ?? Data()
    }
...
/// conventions as `A3netRoster`.
public enum A3netUser {
    public static func upsertProfile(dataDir: String, payload: Data) throws {
        try A3netRoster.call(dataDir: dataDir, payload: payload) { dp, dl, pp, pl, out in
    ...
    public static func listProfiles(dataDir: String) throws -> Data {
        try A3netRoster.callNoPayload(dataDir: dataDir) { dp, dl, out in
            a3net_ffi_user_list_profiles(dp, dl, out)
        }
    }

    public static func getProfile(dataDir: String, userId: String) throws -> Data {
        try A3netRoster.callStrStr(dataDir: dataDir, second: userId) { dp, dl, idp, idl, out in
    ...
    public static func ensureDigit(dataDir: String, userId: String) throws -> String {
        let body = try A3netRoster.callStrStr(dataDir: dataDir, second: userId) { dp, dl, idp, idl, out in
            a3net_ffi_user_ensure_digit(dp, dl, idp, idl, out)
        }
        guard let json = try? JSONSerialization.jsonObject(with: body) as? [String: Any],
              let digit = json["value"] as? String else {
            throw A3netFfiError(status: A3NET_FFI_E_JSON, message: "missing value")
        }
        return digit
    }
}
