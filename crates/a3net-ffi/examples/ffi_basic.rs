//! Minimal example — exercise the always-available `a3net-ffi`
//! public surface (result envelope, status codes, error mapping
//! and a real (non-FFI) hash round-trip mirroring what
//! `a3net_ffi_hash_bytes` does internally).
//!
//! Run with:
//!   cargo run -p a3net-ffi --example ffi_basic
...
fn main() {
    println!("=== a3net-ffi basic example ===");
    ...
    // `a3net_ffi_hash_bytes` on the C side is a thin shim around
    // `a3net_types::ContentHash::from_bytes`. Running the same
    // call from Rust lets the embedder (or a CI golden test)
    // pin the deterministic hash.
    let hash = a3net_types::ContentHash::from_bytes(b"hello embedder");
    ...
    for bad in ["not 64 chars", &"g".repeat(64), "DEADBEEF"] {
        assert!(
            a3net_types::ContentHash::from_hex(bad).is_err(),
            "should reject {bad:?}"
        );
    }
    ...
    //     `a3net_ffi_dial` decodes from the embedder's hex)
    let node_id = a3net_types::NodeId::random();
    let hex = node_id.as_hex().to_string();
    let parsed = a3net_types::NodeId::from_hex(&hex).expect("hex parse");
    ...
    // iroh-build-only exports (`a3net_ffi_node_addr`,
    // `a3net_ffi_dial`) are absent. The companion pure-Rust
    // check `a3net_ffi::ADNET_FFI_E_FEATURE` is the documented
