...
/// `NodeInfo` record in `a3net.udl` — keep these in sync
...
#[uniffi::export]
pub fn a3net_ffi_placeholder_gossip_subscribe(
    topic: String,
...
/// Opaque handle to an A3Net node. Mobile callers create one
...
/// `a3net_node::Node` is constructed on first use and held by
...
pub struct AdnetHandle {
    runtime: Runtime,
    inner: Arc<std::sync::Mutex<Option<a3net_node::Node>>>,
    data_dir: PathBuf,
...
impl AdnetHandle {
    ...
    pub fn ensure_booted(&self) -> Result<String, AdnetError> {
        ...
        let node = if let Some(n) = guard.as_ref() {
            ...
            let cfg = self
                .runtime
                .block_on(async {
                    a3net_node::NodeConfig::load_or_create(&self.data_dir)
                })
            ...
            let n = self
                .runtime
                .block_on(async { a3net_node::Node::builder(cfg).build().await })
    ...
    ///
    /// **Implementation note**: A3Net does not yet expose a
    ...
    pub fn put_bytes(&self, data: Vec<u8>) -> Result<BlobPutInfo, AdnetError> {
        let hash = a3net_types::ContentHash::from_bytes(&data);
        Ok(BlobPutInfo {
    ...
    /// `a3net_ffi_blob_fetch_ticket` — accepts a hex
    ...
    pub fn fetch_ticket(&self, ticket: String) -> Result<BlobFetchInfo, AdnetError> {
        ...
        let hash = a3net_types::ContentHash::from_hex(&ticket).map_err(|e| {
    ...
    /// `/a3net/blob/<hex-hash>` path. For v0.1 we accept the
...
mod tests {
    ...
    fn placeholder_gossip_rejects_empty_topic() {
        let r = a3net_ffi_placeholder_gossip_subscribe(String::new());
        assert!(matches!(
            r,
            Err(AdnetError::InvalidArgument { .. })
        ));
        let ok = a3net_ffi_placeholder_gossip_subscribe("room-42".into()).unwrap();
