...
> IPFS 兼容的 HTTP 网关：让现有 IPFS 客户端能通过 `/ipfs/<cid>`、`/ipns/<name>` 路径访问 ADNet 上的内容。 / IPFS-compatible HTTP Gateway — fetch ADNet content via `/ipfs/<cid>` and `/ipns/<name>`.

## 概览 (Overview)

`adnet-gateway` 提供一个 axum-based HTTP 服务，对外实现 IPFS HTTP Gateway 规范的核心子集：
...
`GatewayRouter` 是 axum router；上层（`adnet-cli` 的 `adnet-gateway` 子命令）只是薄壳。
...
```rust,no_run
use adnet_gateway::GatewayConfig;
...
```rust,no_run
use adnet_gateway::{GatewayConfig, TlsConfig};
...
```rust,no_run
use adnet_gateway::metrics::GatewayMetrics;
...
```rust,no_run
use adnet_gateway::bitswap_api::create_bitswap_router;
let router = create_bitswap_router(/* … */);
```

## 应用案例 (Use Cases / Examples)

- **`adnet-cli`** 把 `GatewayRouter` 暴露成 `adnet-gateway` 二进制，监听 `0.0.0.0:8080`，
  接受 IPFS HTTP 客户端的只读请求。
- **`adnet-blobstore`** 的 `BlobStore` 作为网关后端的存储源（`/blobs/<hash>`）。
- **第三方 IPFS 工具**：`ipfs cat <cid>`、`ipfs add`、`pin` 命令都能通过本 gateway 工作，
  适合把现有 IPFS 应用栈平迁到 ADNet。
