...
//! name becomes resolvable over DNS-TXT at `_a3net-<ipns-name>.<pkarr-zone>`.
//!
//! Pkarr is the *federated* DNS-published pubkey record system that
//! `iroh` uses for `node_id → relay_url` resolution (see
//! `iroh::address_lookup`). We reuse the same wire format so an A3Net
...
//! own relay (see `crates/a3net-dns-server`) should override via
//! `with_relays`.
...
/// record for an A3Net IPNS name is published under
/// `_a3net.<name>` so a zone owner can audit / filter them.
const PKARR_KEY: &str = "_a3net";
