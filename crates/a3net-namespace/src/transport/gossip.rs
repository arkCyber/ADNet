...
//!
//! Wraps the existing `a3net-gossip` bus so an IPNS publish becomes
//! a gossip message on a stable, well-known topic
//! (`a3net-ipns-v1`). Subscribers receive records the moment they
//! are gossiped, regardless of whether pkarr is reachable.
//!
//! The transport deliberately does **not** own a GossipBus — the
//! bus is constructed by `a3net-node` (or the CLI) and shared with
...
//! revisions will use `a3net-ipns-v2` and live alongside v1 for
...
/// Stable topic id. Computed once with `BLAKE3("a3net-ipns-v1")`
/// so an external observer can recompute it.
pub const IPNS_TOPIC: &str = "a3net-ipns-v1";

/// Adapter that lets this transport broadcast on a generic
/// `mpsc::Sender` (the same shape `a3net-gossip` exposes internally).
/// We deliberately decouple from `a3net-gossip::GossipBus` to avoid a
