...
//!
//! The vanilla [`a3net_dht::query::DhtQuery::put_value`] is opaque:
...
use a3net_dht::record::DhtKey;
use a3net_dht::store::SharedDhtStore;
...
/// depending on the full `a3net-dht::query::DhtQuery`.
///
...
/// Default implementation backed by an [`a3net_dht::query::DhtQuery`].
pub struct DhtQueryBackend {
    inner: Arc<RwLock<a3net_dht::query::DhtQuery>>,
}
...
impl DhtQueryBackend {
    pub fn new(query: Arc<RwLock<a3net_dht::query::DhtQuery>>) -> Self {
        Self { inner: query }
    }
}
...
impl DhtBackend for LocalDhtBackend {
    async fn put_value(&self, key: &DhtKey, data: Vec<u8>, ttl: Duration) {
        let value = a3net_dht::record::DhtValue {
            data,
