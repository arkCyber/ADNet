...
//! (and not in `adnet-gossip` / `adnet-blobstore`) so the call
...
impl ReputationReporter {
    ...
    pub fn record(&self, event: ReputationEvent) -> crate::error::ReputationResult<()> {
        trace!(target: "adnet_reputation", event = %event.kind_tag(), peer = %event.peer().short(), "recording");
...
impl<'a> GossipSignal<'a> {
    ...
    pub fn valid(&self, peer: NodeId, topic: Option<adnet_types::Topic>, size_bytes: u32) {
    ...
    pub fn invalid(
        &self,
        peer: NodeId,
        topic: Option<adnet_types::Topic>,
    ...
    pub fn first_delivery(&self, peer: NodeId, topic: Option<adnet_types::Topic>) {
    ...
    pub fn mesh_delivery(&self, peer: NodeId, topic: Option<adnet_types::Topic>) {
    ...
    pub fn duplicate(&self, peer: NodeId, topic: Option<adnet_types::Topic>) {
