...
> Criterion-based benchmark suite for A3Net — measure network, storage and crypto hot paths.

## 概览 (Overview)

`a3net-bench` is the **performance regression suite** for the A3Net workspace. It uses
...
`a3net-bench` is a workspace-internal crate — it is **not** published. Add it
as a path dependency from another workspace crate (e.g. a future
`xtask`) if you want to call `BenchReport` programmatically:

```toml
[dev-dependencies]
a3net-bench = { workspace = true }
```

The Criterion binary entrypoint lives at `src/main.rs` (`a3net-bench`); run
...
a3net_bench::crypto::hashing::register(&mut c);
a3net_bench::network::dht::register(&mut c);
c.final_summary();
```
...
```rust
use a3net_bench::{BenchConfig, BenchReport, report::BenchmarkResult};
...
- **Crypto-bench triage** — when a signing-related PR lands, isolate it
  with `cargo bench -p a3net-bench -- ed25519_sign` and look at the
