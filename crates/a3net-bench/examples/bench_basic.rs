...
//!
//! Run with:
//!   cargo run -p a3net-bench --example bench_basic --release

use a3net_bench::crypto::hashing;
use a3net_bench::{BenchConfig, report::BenchmarkResult};
...
fn main() {
    ...
    println!("=== a3net-bench basic example ===");
    println!(
    ...
    let results = vec![BenchmarkResult {
        ...
        throughput: Some(a3net_bench::report::ThroughputMetric {
            value: 4_000_000.0 / 950.0,
            unit: "MiB/s".into(),
        }),
    }];

    let report = a3net_bench::BenchReport {
        config,
        results,
