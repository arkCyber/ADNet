...
impl Registry {
    ...
    /// ADNet's convention is to gate registration behind a
    ...
    /// is already taken. ADNet callers gate each metric behind
        ...
        if write.contains_key(name) {
            ...
            panic!(
                "metric name {name:?} is already registered — \
                 ...
                 use a distinct prefix per crate (e.g. \"adnet_transport_dial_total\")"
            );
        }
...
impl RegistrySnapshot {
    ...
    /// ADNet layer), so this is the right place to do it
...
mod tests {
    ...
    fn register_rejects_duplicate_name() {
        // ADNet requires every metric to have a unique
    ...
    fn register_name_collision_across_kinds_panics() {
        ...
        // must panic. ADNet requires every metric to have
