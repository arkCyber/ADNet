...
//! `adnet-transport/src/iroh/discovery/diagnostics.rs`) and
//! `EndpointDiagnosticsRecorder` (in
//! `adnet-transport/src/iroh/endpoint_diagnostics.rs`) are
//! *typed* diagnostic snapshots — they expose `snapshot()` methods
//! that return `Clone + Serialize` structs. They live behind
//! `#[cfg(feature = "iroh")]`, so the `adnet-observability`
//! crate (which has **no** iroh dependency) cannot directly
//! import them.
//!
//! PR2 is the **bridge**: we define a transport-agnostic
//! [`DiagnosticsSource`] trait in `adnet-observability` and
//! write an adapter in `adnet-transport/src/iroh/metrics_bridge.rs`
...
//!
//! Same reason as PR1: ADNet's three metric primitives are enough
//! for what we surface. The bridge is a single linear pass over
//! the snapshot fields — no streaming, no quantiles.
//!
//! ## Metric names
//!
//! All names are prefixed with `adnet_discovery_*` and
//! `adnet_endpoint_*` so they don't collide with the
//! `adnet_transport_*` counters from
//! `crates/adnet-transport/src/metrics.rs`. The `endpoint_*`
...
//! emit a single aggregated `adnet_discovery_resolutions_hit_pct`
...
/// Snapshot view that the bridge can read. Every ADNet
...
/// `adnet-observability` counters are monotonic
...
pub fn update_from_source_into(metrics: &DiscoveryMetrics, source: &dyn DiagnosticsSource) {
    ...
    let hit_rate = if denom == 0 {
        ...
        // precision for values > 2^53; ADNet's resolutions
