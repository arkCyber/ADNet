...
//! `adnet-transport/src/iroh/discovery/diagnostics.rs`.
//!
//! ## Label storage
//!
...
mod tests {
    ...
    fn counter_render_prometheus_includes_help_type_and_samples() {
        let c = Counter::new("adnet_test_total", "test counter help");
        c.inc_by(3);
        let l = LabelSet::new([("topic".into(), "lobby".into())]).unwrap();
        c.inc_labels(&l);
        c.inc_labels(&l);
        let out = c.render_prometheus();
        assert!(out.contains("# HELP adnet_test_total test counter help"));
        assert!(out.contains("# TYPE adnet_test_total counter"));
        assert!(out.contains("adnet_test_total 3"));
        // Labeled variant rendered with sorted label order and
        // Prometheus escaping.
        assert!(out.contains(r#"adnet_test_total{topic="lobby"} 2"#));
    }
