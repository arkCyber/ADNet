...
> 轻量级 metrics、histogram、health-check 与 Prometheus 导出 — 无 `prometheus` crate 依赖、进程内实现的 ADNet 运行时观测层。

## 概览 (Overview)

`adnet-observability` 是 ADNet 工作空间的"指标 + 健康检查"中心:
...
- **HTTP tracing bridge** — `bridge` 模块把 `adnet-transport` 的 HTTP 指标接入本 crate 的 `Registry`。
...
无 `Cargo.toml` 改动即可在 ADNet workspace 内任意 crate 引用。
...
```rust
use adnet_observability::health::{HealthCheck, run_checks};
...
adnet_observability::health::register(DbPing);
// 之后 run_checks() 会聚合所有注册的 check,全部通过返回 true。
```

## 应用案例 (Use Cases / Examples)

- **ADNet 节点入口** — `adnet-node` 在启动时 `Registry::default()`,注册 bitswap 流量、DHT 查询、gossip 速率等 Counter / Histogram,并通过 `http::serve` 在 `:9090` 暴露 `/metrics`;Kubernetes scrape 直接生效。
- **网关观测** — `adnet-gateway` 在每个 HTTP handler 前后观察一次延迟 Histogram,把状态码打成 Label;运维通过 `histogram_quantile` 计算 p95 / p99。
- **混沌测试** — `adnet-chaos` 在每次注入故障时增加 `chaos_injections_total{kind}` 计数器,实验报告里直接读取导出。
- **可观测性回路** — `adnet-error` 在 `metrics` feature 开启后,把 `AdnetErrorReport::emit()` 接入 `adnet_error_total{code,kind,severity}` 计数器;`adnet-observability` 的 HTTP 端点一起暴露给 Prometheus。
