...
fn snapshot_counter_only() {
    ...
    let c = reg.register_counter("adnet_test_total", "test counter");
    c.inc_by(42);
    let out = PrometheusExporter::new(&reg).render_to_string();
    let expected = "\
# HELP adnet_test_total test counter
# TYPE adnet_test_total counter
adnet_test_total 42
";
...
fn snapshot_counter_with_labels() {
    let reg = Registry::default();
    let c = reg.register_counter("adnet_req_total", "requests");
    ...
    let out = PrometheusExporter::new(&reg).render_to_string();
    let expected = "\
# HELP adnet_req_total requests
# TYPE adnet_req_total counter
adnet_req_total 10
adnet_req_total{topic=\"files\"} 1
adnet_req_total{topic=\"lobby\"} 2
";
...
fn snapshot_gauge() {
    let reg = Registry::default();
    let g = reg.register_gauge("adnet_g", "test gauge");
    g.set(-5);
    let out = PrometheusExporter::new(&reg).render_to_string();
    let expected = "\
# HELP adnet_g test gauge
# TYPE adnet_g gauge
adnet_g -5
";
    assert_eq!(out, expected);
...
fn snapshot_histogram_basic() {
    let reg = Registry::default();
    let h = reg.register_histogram("adnet_h_seconds", "test histogram");
    h.observe(0.0005);
    h.observe(0.05);
    h.observe(5.0);
    let out = PrometheusExporter::new(&reg).render_to_string();
    ...
    //   +Inf:  3
    let expected = "\
# HELP adnet_h_seconds test histogram
# TYPE adnet_h_seconds histogram
adnet_h_seconds_bucket{le=\"0.001\"} 1
adnet_h_seconds_bucket{le=\"0.01\"} 1
adnet_h_seconds_bucket{le=\"0.1\"} 2
adnet_h_seconds_bucket{le=\"0.5\"} 2
adnet_h_seconds_bucket{le=\"1\"} 2
adnet_h_seconds_bucket{le=\"5\"} 3
adnet_h_seconds_bucket{le=\"10\"} 3
adnet_h_seconds_bucket{le=\"+Inf\"} 3
adnet_h_seconds_count 3
adnet_h_seconds_sum 5.0505
";
    assert_eq!(out, expected);
}
...
fn snapshot_histogram_with_labels() {
    let reg = Registry::default();
    let h = reg.register_histogram("adnet_h_seconds", "test");
    ...
    // called on it.
    let expected = "\
# HELP adnet_h_seconds test
# TYPE adnet_h_seconds histogram
adnet_h_seconds_bucket{le=\"0.001\"} 0
adnet_h_seconds_bucket{le=\"0.01\"} 0
adnet_h_seconds_bucket{le=\"0.1\"} 0
adnet_h_seconds_bucket{le=\"0.5\"} 0
adnet_h_seconds_bucket{le=\"1\"} 0
adnet_h_seconds_bucket{le=\"5\"} 0
adnet_h_seconds_bucket{le=\"10\"} 0
adnet_h_seconds_bucket{le=\"+Inf\"} 0
adnet_h_seconds_count 0
adnet_h_seconds_sum 0
adnet_h_seconds_bucket{topic=\"lobby\",le=\"0.001\"} 0
adnet_h_seconds_bucket{topic=\"lobby\",le=\"0.01\"} 0
adnet_h_seconds_bucket{topic=\"lobby\",le=\"0.1\"} 0
adnet_h_seconds_bucket{topic=\"lobby\",le=\"0.5\"} 1
adnet_h_seconds_bucket{topic=\"lobby\",le=\"1\"} 1
adnet_h_seconds_bucket{topic=\"lobby\",le=\"5\"} 1
adnet_h_seconds_bucket{topic=\"lobby\",le=\"10\"} 1
adnet_h_seconds_bucket{topic=\"lobby\",le=\"+Inf\"} 1
adnet_h_seconds_count{topic=\"lobby\"} 1
adnet_h_seconds_sum{topic=\"lobby\"} 0.5
";
    assert_eq!(out, expected);
}
