...
//!    `pkarr` is configured with the ADNet instrumented
//!    publisher.
...
use adnet_transport::iroh::discovery::{
...
async fn discovery_builder_emits_publish_events() {
    ...
    let pkarr_cfg = PkarrPublisherConfig::default()
        .with_policy(adnet_transport::iroh::discovery::PublishPolicy::RelayOnly);
...
async fn endpoint_snapshot_captures_identity_path() {
    ...
    let snap = EndpointSnapshot::capture_with_identity_path(
        bound.endpoint(),
        Some("/var/lib/adnet/iroh-secret.key".into()),
    );
    assert_eq!(
        snap.identity_path.as_deref(),
        Some("/var/lib/adnet/iroh-secret.key")
    );
...
async fn wait_for_publish(
    ...
    timeout: Duration,
) -> adnet_transport::iroh::discovery::IrohDiscoverySnapshot {
...
async fn no_publisher_config_does_not_synthesize_publish_event() {
    ...
    // ADNet's `bind_internal` must NOT have stamped a "kept"
...
async fn discovery_builder_with_user_data_pre_stamps_diagnostics() {
    ...
    let cfg = DiscoveryConfig::default()
        .with_diagnostics(Arc::clone(&diag))
        .with_user_data(adnet_transport::iroh::discovery::UserData::new("audit-marker").unwrap());
