...
/// `conn`. Used by `adnet-node::download::try_transport`.
...
/// be the side that holds the blob in a [`adnet_blobstore::BlobStore`].
...
pub async fn serve_blob_request(
    conn: &mut Box<dyn OutgoingConnection>,
    store: &adnet_blobstore::BlobStore,
) -> TransportResult<()> {
...
mod tests {
    ...
    fn make_store(
        dir: &std::path::Path,
        payload: &[u8],
    ) -> (adnet_blobstore::BlobStore, ContentHash) {
        let store = adnet_blobstore::BlobStore::new(dir).unwrap();
    ...
    async fn serve_blob_request_errors_on_unknown_hash() {
        let dir = tempfile::tempdir().unwrap();
        let store = adnet_blobstore::BlobStore::new(dir.path()).unwrap();
        let (mut client, mut server) = make_pipe_pair();
