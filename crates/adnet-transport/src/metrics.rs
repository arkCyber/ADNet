...
//!
//! [`adnet_observability::registry::GLOBAL`]: adnet_observability::registry::GLOBAL
...
mod tests {
    ...
    use adnet_observability::registry::GLOBAL as GLOBAL_REGISTRY;
    ...
    fn register_creates_all_metrics_with_unique_names() {
        ...
        assert!(names.contains(&"adnet_transport_dial_attempts_total".to_string()));
        assert!(names.contains(&"adnet_transport_dial_failures_total".to_string()));
        assert!(names.contains(&"adnet_transport_dial_successes_total".to_string()));
        assert!(names.contains(&"adnet_transport_accepts_total".to_string()));
        assert!(names.contains(&"adnet_transport_frames_sent_total".to_string()));
        assert!(names.contains(&"adnet_transport_frames_received_total".to_string()));
        assert!(names.contains(&"adnet_transport_bytes_sent_total".to_string()));
        assert!(names.contains(&"adnet_transport_bytes_received_total".to_string()));
        assert!(names.contains(&"adnet_transport_active_connections".to_string()));
        assert!(names.contains(&"adnet_transport_identity_mismatch_total".to_string()));
        // Inc + read back.
        ...
        assert!(text.contains("adnet_transport_dial_attempts_total 3"));
        assert!(text.contains("adnet_transport_dial_successes_total 1"));
        assert!(text.contains("adnet_transport_dial_failures_total 1"));
        assert!(text.contains("adnet_transport_active_connections 7"));
        assert!(text.contains("adnet_transport_bytes_sent_total 42"));
    }
    ...
    fn for_tests_uses_isolated_registry() {
        ...
        let exporter = PrometheusExporter::new(&registry);
        let text = exporter.render().body;
        assert!(text.contains("adnet_transport_dial_attempts_total 5"));
    }
    ...
    async fn transport_round_trip_increments_metrics() {
        ...
        // metrics.
        let exporter = PrometheusExporter::new(&adnet_observability::registry::GLOBAL);
        let text = exporter.render().body;
        assert!(
            text.contains("adnet_transport_dial_attempts_total"),
