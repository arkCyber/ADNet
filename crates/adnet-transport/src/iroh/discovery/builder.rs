...
//! ADNet-specific address-lookup services and a publish-policy
//! filter.
//!
//! # Example
//!
...
//! use adnet_transport::iroh::discovery::{DiscoveryBuilder, DiscoveryConfig, MemoryLookup};
//!
//! # #[tokio::main]
//! # async fn main() {
...
/// ADNet node behaves identically to a stock iroh 1.0 endpoint
...
pub struct DiscoveryConfig {
    ...
    /// ADNet-instrumented publisher is registered *instead of* the
    ...
    /// `iroh_dns::endpoint_info::UserData` (v1.0.3) so ADNet
...
impl DiscoveryBuilder {
    ...
    async fn bind_internal(
        ...
        if let Some(mut pkarr_cfg) = config.pkarr.clone() {
            ...
            let adnet_publisher: AdnetPkarrPublisher =
                build_publisher(pkarr_cfg, Arc::clone(&diagnostics))?;
            builder = builder.address_lookup(adnet_publisher);
