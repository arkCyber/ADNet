...
//!   `Arc<RwLock<…>>` plus an ADNet-shaped surface (`NodeId`
...
impl MemoryLookup {
    ...
    pub const PROVENANCE: &'static str = "adnet-memory";
...
impl MainlineLookup {
    ...
    pub const PROVENANCE: &'static str = "adnet-mainline-dht";
...
mod tests {
    ...
    fn memory_lookup_provenance_is_stable() {
        assert_eq!(MemoryLookup::PROVENANCE, "adnet-memory");
        assert_eq!(MainlineLookup::PROVENANCE, "adnet-mainline-dht");
    }
    ...
    fn put_then_get_user_data_round_trips() {
        ...
        let ud = UserData::new("adnet/role=worker").unwrap();
        let prev = lk
