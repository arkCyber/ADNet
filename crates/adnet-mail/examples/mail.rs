//! `adnet-mail` — interactive mail client example
//!
//! Demonstrates the full `adnet_mail` API surface:
...
//!
//! # Send an email
//! cargo run -p adnet-mail --example mail -- smtp-send \
//!     --to bob@example.com \
//!     --subject "Hello" \
//!     --body "Hi Bob"
//!
//! # List recent messages
//! cargo run -p adnet-mail --example mail -- imap-list
//!
//! # Watch INBOX for new mail
//! cargo run -p adnet-mail --example mail -- imap-idle
//!
//! # Interactive mode
//! cargo run -p adnet-mail --example mail -- interactive
//! ```
...
use adnet_mail::mime::{Address, Attachment, Mail};
use adnet_mail::prelude::{ImapSession, SendOutcome};
use adnet_mail::provider::Provider;
use adnet_mail::retry::{RetryPolicy, send_with_retry};
use adnet_mail::smtp;
...
#[derive(Parser)]
#[command(name = "adnet-mail-cli", about = "adnet-mail interactive client")]
struct Cli {
    ...
    let path = match path {
        ...
        None => {
            let candidates = [
                ...
                dirs::config_dir()
                    .map(|p| p.join("adnet-mail").join("mail.conf.json"))
                    .unwrap_or_default(),
            ];
            candidates
                .into_iter()
...
async fn cmd_smtp_send(
    ...
    mail.extra_headers.insert(
        "X-Mailer".into(),
        format!("adnet-mail-cli/{}", env!("CARGO_PKG_VERSION")),
    );
...
async fn cmd_smtp_send_file(
    ...
    for path in files {
        ...
        let att = Attachment {
            ...
            content_type: mime,
            data,
            disposition: adnet_mail::mime::Disposition::Attachment,
        };
        println!(
            "  [file] {} ({} bytes, {})",
            att.filename,
...
async fn cmd_imap_list(cfg: &ResolvedConfig, limit: usize, unseen: bool) -> Result<()> {
    ...
    let mut handle = adnet_mail::imap::FetchHandle::new(&mut session, info.clone());
    let msgs = if unseen {
...
async fn cmd_imap_fetch(cfg: &ResolvedConfig, uid: u32) -> Result<()> {
    ...
    let mut handle = adnet_mail::imap::FetchHandle::new(&mut session, info);
...
async fn cmd_imap_idle(cfg: &ResolvedConfig, timeout_secs: u64) -> Result<()> {
    ...
    loop {
        match session.idle_once().await {
            Ok(adnet_mail::imap::IdleEvent::NewMail) => {
            ...
            Ok(adnet_mail::imap::IdleEvent::Timeout) => {
                info!("IDLE timeout -- restarting");
            }
            Ok(adnet_mail::imap::IdleEvent::Interrupted) => {
...
fn cmd_auto_configure(addr: &str) -> Result<()> {
    ...
    println!();
    println!("  # Then run:");
    println!("  cargo run -p adnet-mail --example mail -- smtp-send \\");
    println!("    --to recipient@example.com \\");
    println!("    --subject \"Hello\" \\");
    println!("    --body \"This is a test.\"");
    println!();
    println!("  # Or interactive mode:");
    println!("  cargo run -p adnet-mail --example mail -- interactive");
...
async fn interactive_mode(cli: &Cli) -> Result<()> {
    println!("===========================================");
    println!("     adnet-mail interactive client");
    println!("===========================================");
    println!();
    println!("Commands:");
