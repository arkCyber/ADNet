...
//!
//! Each ADNet IPNS publish is stored under two keys:
//!
//!   * `_adnet.<ipns-name>.<zone>` (TXT) — base64 of the pkarr packet
...
//!     by the IPNS record. This lets a `dig alice.adnet.example`
...
pub enum RecordKind {
    /// ADNet IPNS TXT record (pkarr-compatible wire payload).
...
impl ZoneStore {
    ...
    pub fn ipns_txt_key(&self, ipns_name: &str) -> String {
        ...
        format!("_adnet.{}.{}.", ipns_name.to_ascii_lowercase(), zone)
    }
...
mod tests {
    ...
    fn cfg(tmp: &std::path::Path) -> DnsServerConfig {
        DnsServerConfig::default()
            ...
            .with_zone("adnet.test")
    }

    #[test]
    fn keys_are_lowercased_and_zoned() {
        let store = ZoneStore::new(DnsServerConfig::default().with_zone("AdNet.Test"));
        assert_eq!(store.ipns_txt_key("Alice"), "_adnet.alice.adnet.test.");
        assert_eq!(store.relay_key("Alice"), "alice.adnet.test.");
    }
