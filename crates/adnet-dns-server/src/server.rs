...
//! (TXT for `_adnet.<name>.<zone>` and A/AAAA for `<name>.<zone>`).
//!
...
//! the **pkarr-compatible ADNet zone**, not the DNS plumbing.
    ...
    fn round_trip_txt_query() {
        ...
        store
            .put(crate::zone::ZoneRecord {
                key: "_adnet.alice.adnet.test.".into(),
        ...
        // Build a wire-format query for `_adnet.alice.adnet.test.` TXT.
        let mut q = Vec::new();
        ...
        q.extend_from_slice(&[0x00, 0x00]);
        // qname: 6 _adnet 5 alice 6 adnet 4 test 0
        for label in ["_adnet", "alice", "adnet", "test"] {
