...
> 自建权威 DNS 服务器，承载 pkarr 兼容的 IPNS zone。 / Self-hostable authoritative DNS server — pkarr-compatible zone for ADNet.

## 概览 (Overview)

`adnet-dns-server` 把"自建 pkarr zone"这件事做成了一个最小 DNS 服务器。它对外提供两类记录：

- `_adnet.<ipns-name>.<zone>` —— TXT 记录，承载 base64 编码的 pkarr 包（即该 IPNS 名
  在公共 pkarr relay 上原本应该发的内容）。
- `<relay-name>.<zone>` —— A / AAAA 记录，公布本机运行的 ADNet relay 入口。

这让"自建 zone"成为可能：

- 公有 pkarr relay 是 *联邦的*（一条记录会在所有 relay 上可见）。大多数运营商只想
  跑自己的 `*.adnet.example` 区。
...
```rust
use adnet_dns_server::{DnsServerConfig, serve, DnsServerHandle};
use adnet_dns_server::zone::{ZoneRecord, RecordKind};
```
...
use adnet_dns_server::{DnsServerConfig, serve};

let cfg = DnsServerConfig::default()
    .with_zone("adnet.example")
...
```rust,no_run
use adnet_dns_server::zone::{ZoneRecord, RecordKind};
let store = adnet_dns_server::zone::open(
    adnet_dns_server::DnsServerConfig::default(),
)?;
let rec = ZoneRecord {
...
```rust,no_run
use adnet_dns_server::http::{HttpApi, PublishBody};

let api = HttpApi::from_config(adnet_dns_server::DnsServerConfig::default())?;
...
```rust,no_run
use adnet_dns_server::http::HttpApi;
let api = HttpApi::from_config(adnet_dns_server::DnsServerConfig::default())?;
for rec in api.list() {
...
- **运营自建** —— 把 DNS 服务暴露到公网 (`0.0.0.0:53`)，让团队成员走
  `dig _adnet.<ipns>.adnet.example TXT` 而不是 `pkarr.pub`。
- **`adnet-cli`** 的 `adnet-dns-server` 二进制直接调用 `serve()` + `HttpApi`。
