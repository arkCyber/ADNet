...
fn main() -> anyhow::Result<()> {
    ...
    let invitation = a3net_pairing::SignedInvitation::create(
        &node_id,
        &wallet,
    ...
    let content = InvitationContent {
        ...
        subject: "A3Net Device Pairing Invitation".into(),
        body: "Hi Bob, please pair with my A3Net device using the attached QR code.".into(),
    };
    ...
    let url = a3net_pairing::wire::PairingInvitation::to_url(&invitation)?;
    println!("Pairing URL (for testing in QR scanner):");
    println!("{}", url);
    println!();

    // 7. Instructions for using SMTP
    println!("To send this email via SMTP, use the a3net-mail crate:");
    println!("  use a3net_mail::smtp::{{connect, send::send_raw}};");
    println!("  use a3net_mail::login_param::Account;");
    println!();
