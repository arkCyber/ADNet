//! Comprehensive tests for `a3net-invite`.
//!
//! Tests cover:
...
fn constants_are_correct() {
    ...
    assert_eq!(ADNET_PAIRING_MIME, "application/x-a3net-pairing");
    assert_eq!(ADNET_PAIRING_FILENAME, "a3net-pairing.inv");
...
fn error_from_pairing() {
    ...
    let pairing_err = a3net_pairing::error::PairingError::Malformed {
        what: "test".into(),
...
fn error_from_mail() {
    ...
    let mail_err = a3net_mail::error::MailError::Parse("bad input".into());
...
fn build_email_has_attachment() {
    ...
    assert_eq!(att.filename, "a3net-pairing.inv");
    assert_eq!(att.content_type, "application/x-a3net-pairing");
...
fn build_email_attachment_is_valid_json() {
    ...
    let parsed: a3net_pairing::invitation::SignedInvitation =
...
fn extract_ignores_wrong_filename() {
    ...
    mail.attachments.push(Attachment {
        ...
        content_type: "application/x-a3net-pairing".into(),
        data: json,
        disposition: Disposition::Attachment,
    });
...
fn extract_takes_first_matching_attachment() {
    ...
    mail.attachments.push(Attachment {
        filename: "a3net-pairing.inv".into(),
        content_type: "application/x-a3net-pairing".into(),
        data: json,
        disposition: Disposition::Attachment,
    });
...
fn default_body_contains_pairing_scheme() {
    ...
    assert!(
        mail.text.contains("a3net-pairing://"),
...
fn default_body_contains_instructions() {
    ...
    assert!(mail.text.contains("QR") || mail.text.contains("a3net"));
}
...
fn x_mailer_header_contains_version() {
    ...
    assert!(x_mailer.starts_with("A3Net/"));
}
...
fn text_code_case_insensitive_parsing() {
    ...
    let result2 = parse_text_code(&("a3net-"[..].to_owned() + &code_str[6..]));
