...
Fan ADNet events out to HTTP endpoints with HMAC-SHA256 signing,
...
| `load_endpoints` / `save_endpoints` / `from_config_file` | File-based config helpers used by `adnet webhook load/save/list` CLI subcommands. |
| `pump` module | Long-running task that bridges a `broadcast::Receiver<Announcement>` (the same shape `adnet_gossip::GossipBus::subscribe` returns) into the sink. |
...
```rust,no_run
use adnet_webhook::{EndpointConfig, WebhookSink, AdnetEvent, EventSink};
...
```rust,no_run
use std::sync::Arc;
use adnet_webhook::{DroppedHandler, EndpointConfig, WebhookSink};
# use std::time::Duration;
# let cfg = vec![EndpointConfig { url: "http://127.0.0.1:8080".into(), secret: vec![], room_filter: None, request_timeout: Duration::from_secs(5) }];
...
impl DroppedHandler for Metrics {
    async fn on_dropped(&self, _event: &adnet_webhook::AdnetEvent,
...
let sink = WebhookSink::with_spool_and_budget(cfg,
    "/var/lib/adnet/webhook-spool.jsonl".into(), 5)?;
...
`adnet-webhook` ships a `pump` module that bridges the gossip
event stream to the HTTP sink. Wire it up after
`adnet_gossip::GossipBus::subscribe`:

```rust,ignore
use std::sync::Arc;
use adnet_webhook::{pump, WebhookSink};
