...
//! (the same shape `adnet_gossip::GossipBus::subscribe` returns),
...
//!
//! Run with: `cargo run -p adnet-webhook --example webhook_pump_smoke`.

use std::sync::Arc;
use std::time::Duration;

use adnet_types::{Announcement, CdnContentKind, ContentHash, NodeId, RoomId};
use adnet_webhook::{pump, EndpointConfig, WebhookSink, sign_body};
