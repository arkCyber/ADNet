...
pub const ADNET_PAIRING_MIME: &str = "application/x-adnet-pairing";
...
pub const ADNET_PAIRING_FILENAME: &str = "adnet-pairing.inv";

/// Filename used on the QR code image attachment.
pub const ADNET_QR_FILENAME: &str = "adnet-pairing-qr.svg";
...
impl InvitationMailer {
    ...
    /// - A plain-text body with instructions and the `adnet-pairing://`
    ///   URL rendered inline.
    /// - An attachment named `adnet-pairing.inv` with
    ///   `application/x-adnet-pairing` MIME type, containing the
    ///   invitation as UTF-8 JSON.
    ///
    /// This form is the most robust for email transport: the inline
    /// `adnet-pairing://` URL works on mobile clients that can open
    /// deep links, while the attachment works on desktop clients that
    /// can't parse custom schemes.
    pub fn build_invitation_email(
        invitation: &adnet_pairing::invitation::SignedInvitation,
        ...
        let subject = if content.subject.is_empty() {
            "ADNet Device Pairing Invitation".to_string()
        } else {
        ...
        let body = if content.body.is_empty() {
            format!(
                "You have been invited to pair with an ADNet device.\n\n\
                 Scan the attached QR code or open the file \"adnet-pairing.inv\" \
                 with the ADNet app.\n\n\
                 Pairing link:\n  {url}\n\n\
        ...
        mail.extra_headers.insert(
            "X-Mailer".into(),
            format!("ADNet/{}", env!("CARGO_PKG_VERSION")),
        );
        mail.extra_headers
    ...
    /// The email has:
    /// - `From:` / `To:` / `Subject:` as specified.
    /// - A plain-text body with instructions and the `adnet-pairing://` URL.
    /// - An HTML body with an embedded inline QR code image.
    /// - An inline SVG attachment named `adnet-pairing-qr.svg`.
    /// - An attachment named `adnet-pairing.inv` with
    ///   `application/x-adnet-pairing` MIME type.
    pub fn build_invitation_email_with_qr(
        ...
        let svg_content = adnet_qr::generator::create_qr_svg(&url)
        ...
        // Subject default.
        let subject = if content.subject.is_empty() {
            "ADNet Device Pairing - Scan QR Code".to_string()
        } else {
            content.subject.clone()
        };
        ...
        let html_body = format!(
            r#"<!DOCTYPE html>
<html>
<head><meta charset="utf-8"><title>ADNet Pairing</title></head>
<body style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto; padding: 20px;">
<h2 style="color: #333;">ADNet Device Pairing Invitation</h2>
<p>You have been invited to pair with an ADNet device.</p>
...
<p style="color: #888; font-size: 12px;">
  ...
  <em>If the QR code doesn't work, use the pairing URL above or open the attached file.</em>
        ...
        let text_body = if content.body.is_empty() {
            format!(
                "ADNet Device Pairing Invitation\n\
                 ...
                 2. OPEN THE URL: Copy and paste this link into your ADNet app:\n\
                 {url}\n\n\
                 3. USE THE ATTACHMENT: Save and open \"adnet-pairing.inv\" \
                 with your ADNet app.\n\n\
    ...
    /// `application/x-adnet-pairing` and filename `adnet-pairing.inv`.
    ///
    ...
    pub fn extract_from_mail(mail: &Mail) -> InviteResult<PairingInvitation> {
        ...
        let inv: adnet_pairing::invitation::SignedInvitation =
    ...
    pub fn extract_from_wire(wire: &[u8]) -> InviteResult<PairingInvitation> {
        ...
        let mail = Mail::from_wire_bytes(wire)
            .map_err(|e| InviteError::Mail(adnet_mail::error::MailError::Parse(e.to_string())))?;
...
pub fn create_text_code(invitation: &adnet_pairing::invitation::SignedInvitation) -> InviteResult<TextCode> {
    // Serialize to JSON
...
pub fn parse_text_code(code: &str) -> InviteResult<Option<adnet_pairing::invitation::SignedInvitation>> {
    ...
    let invitation: adnet_pairing::invitation::SignedInvitation =
...
mod tests {
    ...
    fn build_invitation_email_with_qr() {
        ...
        assert!(mail.subject.contains("ADNet"));
