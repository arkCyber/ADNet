...
fn main() -> anyhow::Result<()> {
    println!("╔══════════════════════════════════════════════════════════════════════════════╗");
    println!("║           ADNet Pairing via Email with QR Code Exchange Demo                 ║");
    println!("╚══════════════════════════════════════════════════════════════════════════════╝");
    println!();
    ...
    let alice_invitation = adnet_pairing::SignedInvitation::create(
        &alice_node_id,
        &alice_wallet,
    ...
    let alice_content = InvitationContent {
        from: adnet_mail::mime::Address::new("alice@example.com").with_name("Alice"),
        to: vec![adnet_mail::mime::Address::new("bob@example.com")],
    ...
    let bob_invitation = adnet_pairing::SignedInvitation::create(
        &bob_node_id,
        &bob_wallet,
        CapabilitySet::from_names(["chat"]),
        15 * 60,
    ...
    let bob_content = InvitationContent {
        from: adnet_mail::mime::Address::new("bob@example.com").with_name("Bob"),
        to: vec![adnet_mail::mime::Address::new("alice@example.com")],
        subject: "ADNet Pairing - My Response".into(),
