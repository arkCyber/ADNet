//! End-to-end "Alice invites Bob by email" example for adnet-invite.
//!
//! Simulates the realistic flow when two ADNet users pair without a
...
//!
//! ```bash
//! cargo run -p adnet-invite --example invite_app
//! ```

use adnet_identity::wallet::Wallet;
use adnet_invite::{InvitationContent, InvitationMailer, create_text_code, parse_text_code};
use adnet_mail::mime::Address;
use adnet_pairing::{CapabilitySet, PairingInvitation, SignedInvitation};
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let mail = InvitationMailer::build_invitation_email_with_qr(
        ...
        &InvitationContent {
            ...
            subject: "Let's pair our ADNet devices".into(),
            body: String::new(),
        },
    )?;
