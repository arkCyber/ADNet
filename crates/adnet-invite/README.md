...
> 邮件邀请投递层:把 `adnet-pairing` 的签名邀请打包成 MIME 邮件 / 短文本码,完成双向配对流程 / Email-invitation delivery layer for ADNet — wraps signed pairing invitations as MIME messages and short text codes.

## 概览(Overview)

`adnet-invite` 是 ADNet 邮件配对通道的"最后一公里"。它不做 SMTP 投递本身,也不做密码学 — 它只负责**把现有邀请变成可投递的载体**:

- 把 `SignedInvitation` 序列化为 `application/x-adnet-pairing` 的 MIME 附件,再附带一封可读的邮件(纯文本 + 可选 HTML + 内嵌 QR SVG),通过 `adnet-mail` 投递;
...
- 接收端 `extract_from_mail` / `extract_from_wire` 把邮件还原回 `PairingInvitation`,供 `adnet-pairing` 验签。
...
| 名称 | 描述 |
|------|------|
| `build_invitation_email` | 生成带 `.inv` 附件 + `adnet-pairing://` URL 内联的纯文本邮件 |
| `build_invitation_email_with_qr` | 在邮件正文内嵌 QR SVG,附件含 `.inv` + `.svg` |
| `extract_from_mail` | 从 `Mail` 中筛 `application/x-adnet-pairing` 附件并解析 |
...
`adnet-invite` 已经是 ADNet workspace 的 path 依赖。其调用方通常是 `adnet-cli` / `adnet-ssh` 的"发送邀请"路径,以及 `adnet-mail` 的 IMAP 拉取回调。直接 `use` 即可:

```rust
use adnet_invite::{InvitationMailer, InvitationContent, create_text_code, parse_text_code};
use adnet_pairing::SignedInvitation;
```
...
use adnet_pairing::{CapabilitySet, SignedInvitation};
...
let content = InvitationContent {
    ...
    body: "scan the QR or open adnet-pairing://…".into(),
};
...
1. **跨团队异地不再交换 QR。** 客户支持人员的 MacBook 在广东,用户的 desktop 在北京,两人隔着防火墙看不到对方的二维码。客服在自己的 ADNet 桌面端发"邀请此邮箱",对方在邮件里点击 `adnet-pairing://…` 链接,导入 IMAP 收到的 `adnet-pairing.inv` 附件,即可完成配对。`extract_from_wire` 把整个流程闭合到一行代码。
...
3. **企业 SSO 邮件集成。** 公司希望走自己 SMTP 网关,模板打到 `mail.text` + `mail.attachments` 之后,客服直接接管。`build_invitation_email` 已经填好 `X-Adnet-Invite` / `X-Mailer` 自定义头,以及 `adnet-pairing.inv` 标准附件名,企业 MTA 的 filter 规则可以直接匹配。
