//! Integration tests for `adnet-ipc-adapter`.
//!
//! Tests cover:
...
use adnet_ipc::client::{json_rpc_call, json_rpc_stream, StreamItem};
use adnet_ipc::RpcHandler;
use adnet_ipc_adapter::{start_daemon, NodeRpc, ANNOUNCEMENT_METHOD, METHODS};
use adnet_node::{Node, NodeConfig};
use adnet_types::{ContentHash, NodeId, RoomId};
...
async fn announce_file_content_verification() {
    ...
    let content = b"ADNet IPC Adapter Test File Content";
...
async fn make_ticket_returns_valid_ticket() {
    ...
    let parsed = adnet_types::BlobTicket::parse(ticket_str);
...
async fn make_ticket_ticket_can_be_parsed() {
    ...
    // Parse the ticket
    let ticket = adnet_types::BlobTicket::parse(ticket_str);
    assert!(ticket.is_ok());
...
async fn peers_for_returns_valid_ticket_strings() {
    ...
    for ticket_str in result.as_array().unwrap() {
        if let Some(s) = ticket_str.as_str() {
            ...
            let parsed = adnet_types::BlobTicket::parse(s);
