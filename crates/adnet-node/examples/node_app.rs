//! `adnet-node` 应用示例：构造一个完整的 `Node`，加入一个房间，导入一个临时文件，
//! 把它广播出去，再通过 `room_feed` 看自己的公告。模拟一次端到端的 "share to room"
//! 场景。
//!
//! 运行：`cargo run -p adnet-node --example node_app`
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let file = dir.path().join("hello.txt");
    {
        ...
        writeln!(f, "hello from adnet-node app demo @ {}", chrono::Utc::now())?;
    }
