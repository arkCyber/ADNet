...
//! `MockGraphSyncTransport` already shipped in `adnet-blobstore`.
//!
//! The QUIC bridge itself is exercised by the integration tests in
//! `adnet-transport`; we don't pull a real `SharedTransport` here.
...
async fn client_server_round_trip_through_mock() {
    ...
    use adnet_blobstore::graphsync::GraphSyncServer;
