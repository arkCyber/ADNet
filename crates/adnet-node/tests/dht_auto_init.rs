...
use adnet_node::{Node, NodeBuilder, NodeConfig};
use adnet_transport::{SharedTransport, Transport, quic::QuicTransportBuilder};
...
async fn builder_with_auto_init_dht_wires_handle() {
    ...
    let node = NodeBuilder::new(cfg)
        ...
        .with_auto_init_dht(adnet_node::DhtConfig::default())
        .build_with_bus(adnet_gossip::GossipBus::default())
        .await
...
async fn builder_with_auto_init_ipns_sets_up_resolver() {
    ...
    let node = NodeBuilder::new(cfg)
        .with_transport(transport)
        .with_auto_init_dht(adnet_node::DhtConfig::default())
        .with_auto_init_ipns(adnet_node::IpnConfig::default())
        .build_with_bus(adnet_gossip::GossipBus::default())
        .await
...
async fn node_init_dht_then_init_ipns_round_trip() {
    ...
    node.init_dht(adnet_node::DhtConfig::default())
        .await
        .expect("init_dht");
    assert!(node.dht_handle().await.is_some());

    // Then init IPNS read-only (no keypair).
    node.init_ipns(adnet_node::IpnConfig::default(), None)
        .await
        .expect("init_ipns");
...
async fn init_ipns_without_init_dht_errors() {
    ...
    let err = node
        .init_ipns(adnet_node::IpnConfig::default(), None)
        .await
...
fn sample_hash() -> adnet_types::ContentHash {
    adnet_types::ContentHash::from_bytes(b"p0-d-auto-init")
}
