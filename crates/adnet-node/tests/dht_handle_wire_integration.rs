...
use adnet_dht::{DhtCodec, DynResponseSink};
use adnet_node::dht::{DhtConfig, DhtHandle};
use adnet_transport::{Frame, QuicTransport, QuicTransportBuilder, SharedTransport};
...
impl TransportBridge for MockBridge {
    ...
    async fn dial_and_send(
        ...
        peer: &NodeId,
        msg: Vec<u8>,
    ) -> Result<(), adnet_dht::query::QueryError> {
...
async fn dht_handle_find_providers_routes_over_wire() {
    ...
    bridge.set_responder(Arc::new(move |request_bytes| {
        ...
        let request_id = match msg {
            adnet_dht::protocol::DhtWireMessage::GetProviders(p) => p.request_id,
            _ => return None,
        };
        let resp = adnet_dht::protocol::DhtWireMessage::Providers(
            adnet_dht::protocol::ProvidersPayload {
                request_id,
                providers: vec![adnet_dht::protocol::ProviderRecordWire {
...
async fn dht_handle_full_quic_roundtrip() {
    ...
    use adnet_node::dht_bridge::DynTransportBridge;
    ...
    for _ in 0..40 {
        let key = adnet_dht::DhtKey::from_bytes(hash.as_bytes().to_vec());
...
fn spawn_dht_inbound(
    ...
    let handle = tokio::spawn(async move {
        ...
        let handler = Arc::new(tokio::sync::Mutex::new(
            adnet_dht::DhtProtocolHandler::new(local_id, node.routing_table(), node.store())
                .0,
        ));
