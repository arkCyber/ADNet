...
use adnet_dht::store::{new_in_memory_store, SharedDhtStore};
use adnet_namespace::transport::dht::DhtIpnTransport;
use adnet_namespace::transport::IpnTransport;
use adnet_namespace::{IpnRecord, IpnResolver};
...
async fn ipns_dht_local_round_trip() {
    ...
    let decoded = adnet_namespace::transport::dht::decode_ipns_record(name, &payload.data)
...
async fn dht_node_query_returns_none_without_sender() {
    let node = adnet_dht::DhtNode::new(adnet_dht::DhtConfig::default());
    assert!(
        node.query().is_none(),
