...
> ADNet 节点编排：把 BlobStore、GossipBus、MeshServer、Transport 拼成一个统一运行时。 / ADNet node orchestration — wire BlobStore, GossipBus, MeshServer and Transport into a single runtime.

## 概览 (Overview)

`adnet-node` 是 ADNet 对外的"门面 crate"。一个 `Node` 持有：
...
- `dht` — 启 DHT + IPNS。
- `billing` — 启 adnet-token + adnet-identity。
- `news` — 启用 adnet-news。
...
```rust
use adnet_node::{Node, NodeConfig, NodeBuilder};
...
- **`adnet-cli`** 的 `adnet` 二进制背后就是一个 `Node`。
