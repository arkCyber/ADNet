...
//! receipts) but the API is shaped for the rest of the ADNet stack:
//!
//! - every record is the typed record from [`adnet_types::group_chat`]
...
pub use adnet_types::group_chat::MessageReceipt;

use crate::error::{ChatStoreError, Result};
use crate::schema;

/// Re-export the cyclic sequence ceiling from `adnet_types`, so the
/// hub layer can talk about its own ceiling without an extra import
/// in every call site.
pub use adnet_types::group_chat::MAX_SEQUENCE;
...
/// with a current one.
const INTEGRITY_HASH_TAG: &[u8] = b"adnet-chatstore/v1";
...
// `MessageReceipt` is re-exported from `adnet_types::group_chat` above
...
impl ImManager {
    ...
    pub async fn touch_user(&self, user_id: &str) -> Result<()> {
        adnet_types::invariants::validate_id("user_id", user_id)?;
        let conn = self.conn.lock().await;
    ...
    pub async fn create_conversation(
        ...
        adnet_types::invariants::validate_name("title", title)?;
    ...
    pub async fn get_conversation(&self, conversation_id: &str) -> Result<Option<Conversation>> {
        adnet_types::invariants::validate_id("conversation_id", conversation_id)?;
        let conn = self.conn.lock().await;
        let mut stmt = conn.prepare(
    ...
    pub async fn add_group_member(
        ...
        adnet_types::invariants::validate_id("conversation_id", conversation_id)?;
        adnet_types::invariants::validate_id("user_id", user_id)?;
        adnet_types::invariants::validate_id("role", role)?;
        let conn = self.conn.lock().await;
    ...
    /// [`adnet_types::invariants::MAX_CONTENT_LEN`]. For group
    ...
    pub async fn send_message(
        ...
        adnet_types::invariants::validate_id("conversation_id", conversation_id)?;
        adnet_types::invariants::validate_id("sender_id", sender_id)?;
        if let Some(rid) = receiver_id {
            adnet_types::invariants::validate_id("receiver_id", rid)?;
        }
        adnet_types::invariants::validate_content("content", content)?;
        if let Some(r) = reply_to {
            adnet_types::invariants::validate_id("reply_to", r)?;
        }
    ...
    pub async fn get_message_by_id(&self, message_id: &str) -> Result<Option<Message>> {
        adnet_types::invariants::validate_id("message_id", message_id)?;
        let conn = self.conn.lock().await;
        let mut stmt = conn.prepare(
            "SELECT id, conversation_id, sender_id, receiver_id, content, timestamp, sequence,
                    reply_to, integrity_hash, is_edited, edited_at
    ...
    pub async fn edit_message(
        ...
        adnet_types::invariants::validate_id("message_id", message_id)?;
        adnet_types::invariants::validate_content("new_content", new_content)?;
    ...
    pub async fn add_pending_message(
        ...
        adnet_types::invariants::validate_id("message_id", message_id)?;
        adnet_types::invariants::validate_id("receiver_id", receiver_id)?;
        adnet_types::invariants::validate_id("conversation_id", conversation_id)?;
        let id = Uuid::new_v4().to_string();
        let created_at = Utc::now();
        let conn = self.conn.lock().await;
...
fn validate_user_fields(username: &str, display_name: &str) -> Result<()> {
    use adnet_types::invariants::{validate_id, validate_name};
