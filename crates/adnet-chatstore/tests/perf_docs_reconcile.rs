...
async fn bridge_with_shared_api(
    dir: &TempDir,
) -> (IrohDocsChat, Arc<DocsApi>, adnet_blobstore::IrohBlobStore) {
    let blob_store = adnet_blobstore::IrohBlobStore::open(dir.path())
        .await
        .expect("blobs");
...
async fn replica_converges_within_budget() {
    ...
    let blob_store_b = adnet_blobstore::IrohBlobStore::open(dir.path())
        .await
        .expect("blobs B");
