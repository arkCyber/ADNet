//! Integration tests for the `adnet-reputation` × `adnet-chatstore`
...
use adnet_chatstore::storage::{ChatStorage, ChatStorageConfig};
use adnet_chatstore::trust::ChatTrustStore;
...
/// global PeerScore up.
#[test]
fn positive_chat_trust_lifts_peer_score() {
    let s = store();
...
/// score below zero.
#[test]
fn blocked_chat_trust_pushes_score_below_zero() {
    let s = store();
    let table = PeerScoreTable::new(ReputationParams::default());
    let reporter = ReputationReporter::in_memory(table);
    s.with_reputation(reporter.clone());
...
#[test]
fn user_id_mapping_is_deterministic() {
    let s = store();
    let table = PeerScoreTable::new(ReputationParams::default());
    let reporter = ReputationReporter::in_memory(table);
    s.with_reputation(reporter.clone());
    ...
    s.set("alice2", "bob", 2, None).unwrap(); // different owner, same target
...
#[test]
fn store_without_reputation_still_works() {
    let s = store();
    s.set("alice", "bob", 1, None).expect("set without reporter");
    let rec = s.get("alice", "bob").unwrap().expect("row persisted");
