//! Per-function coverage audit for `adnet-chatstore`.
//!
//! Every public function in `adnet_chatstore::{storage, im, error,
...
use adnet_chatstore::error::ErrorClass;
use adnet_chatstore::im::{
...
use adnet_chatstore::{ChatStoreError, SCHEMA_VERSION};
...
#[test]
fn error_from_adnet_types_uses_validation_variant() {
    // `adnet_types::error::AdnetError::Validation` → `ChatStoreError::Validation`.
    let adnet_err: adnet_types::error::AdnetError =
        adnet_types::error::AdnetError::Validation("x".into());
    let chat_err: ChatStoreError = adnet_err.into();
...
fn schema_version_constant_matches_documented() {
    ...
    // The migration ladder is `migrate_to(1)` then `migrate_to(2)`.
...
fn save_group_message_rejects_too_many_mentions() {
    ...
    // validation error.
    use adnet_types::invariants::MAX_MENTIONS;
    let storage = ChatStorage::new(ChatStorageConfig {
        storage_dir: tempfile::tempdir().unwrap().path().to_path_buf(),
    })
    .unwrap();
...
fn save_direct_message_rejects_too_many_attachments() {
    use adnet_types::invariants::MAX_ATTACHMENTS;
    let storage = ChatStorage::new(ChatStorageConfig {
        storage_dir: tempfile::tempdir().unwrap().path().to_path_buf(),
    })
    .unwrap();
