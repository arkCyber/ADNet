...
> A3Net 数据库抽象层 —— 基于 sqlx 的连接池 + 多数据库支持(SQLite / PostgreSQL / Redis)+ 版本化迁移 + Repository 模式。
>
> A3Net database abstraction layer — sqlx-based connection pool with multi-database support (SQLite / PostgreSQL / Redis), versioned migrations, and the Repository pattern.

## 概览(Overview)

`a3net-database` 给 A3Net 体系内的其他 crate 提供一个"统一、能装不同后端、能跑迁移"的数据库层。它把连接、事务、迁移、查询这四件事压成一致的 API,让上层不必每次都重新选 sqlx vs deadpool vs diesel。
...
```rust
use a3net_database::{DatabaseConfig, DatabaseConnection};
...
```rust
use a3net_database::ConnectionPool;
let pool = ConnectionPool::sqlite(&a3net_database::config::SqliteConfig::default()).await?;
...
```rust
use a3net_database::{Migration, MigrationManager};
...
```rust
use a3net_database::InMemoryRepository;
...
```rust
use a3net_database::QueryBuilder;
