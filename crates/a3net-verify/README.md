...
> A3Net 协议的形式化验证套件 — TLA+ 规范(`verification/tla/`)与 Kani 模型检查器集成,覆盖 DHT/Kademlia、Gossip、Bitswap 三大协议。

## 概览 (Overview)

`a3net-verify` 把 A3Net 的核心 P2P 协议放进形式化验证流水线:
...
use a3net_verify::{RoutingTable, xor_distance};
...
```rust
use a3net_verify::bitswap::{WantEntry, WantList, LedgerBook, BitswapInvariants};
...
```rust
use a3net_verify::gossip::{Message, MessageDeduper, GossipInvariants};
...
- **`a3net-dht`** — 复用 `RoutingTable` 与 `xor_distance` 的实现,且 `closest_peers` 行为与 Kani 证明中的 `proof_closest_peers_sorted` 一致。
- **`a3net-blobstore`** — bitswap 借 / 贷账户由 `LedgerBook` 维护,`is_balanced` 是 gossip / ledger 闸门;`BitswapInvariants::debt_ratio_valid` 在 release build 里被断言。
- **`a3net-gossip`** — `MessageDeduper` 保证同一消息不会被重复广播;`GossipInvariants` 检查 mesh 不变量。
- **CI 流水线** — Kani 证明在 `cargo kani --package a3net-verify` 步骤中跑,任意"已证"属性失败都会阻止 merge。
- **协议演进** — 改 DHT 桶大小 / 改 ledger 借 / 贷比例时,TLA+ 规范允许先证明再实现。
- **学术 / 文档** — TLA+ 文件可作为 A3Net 协议规范的官方参考,与实现一一对应。
