    ...
    if dash.alerts.is_empty() {
        ...
        for alert in &dash.alerts {
            let level_str = match alert.level {
                adnet_observability::dashboard::AlertLevel::Critical => "CRITICAL",
                adnet_observability::dashboard::AlertLevel::Warn => "WARNING",
                adnet_observability::dashboard::AlertLevel::Info => "INFO",
            };
            let icon = match alert.level {
                adnet_observability::dashboard::AlertLevel::Critical => "⛔",
                adnet_observability::dashboard::AlertLevel::Warn => "⚠",
                adnet_observability::dashboard::AlertLevel::Info => "ℹ",
            };
            let color = match alert.level {
                adnet_observability::dashboard::AlertLevel::Critical => Color::Red,
                adnet_observability::dashboard::AlertLevel::Warn => Color::Yellow,
                adnet_observability::dashboard::AlertLevel::Info => Color::Cyan,
            };
            println!(
                "  {} [{}] {}",
