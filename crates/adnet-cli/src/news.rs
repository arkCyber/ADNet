//! Dispatcher for `adnet news …` subcommands.
//!
//! The `NewsService` lives on the `adnet-news` crate and is wired
//! into a `Node` through `adnet-node`'s `news` feature. The CLI
//! is a thin façade: it constructs a stand-alone `NewsService`
//! backed by an `InProcessGossip` transport so `adnet news …`
...
const NEWS_TOPIC_PREFIX: &str = "adnet-news-";
...
async fn open_service(data_dir: &Path) -> Result<NewsService> {
    ...
    let cfg = adnet_news::NewsServiceConfig {
        store_dir,
...
fn parse_severity(s: &str) -> Result<BulletinSeverity> {
    use adnet_types::BulletinSeverity as B;
    Ok(match s.to_ascii_lowercase().as_str() {
        "info" => B::Info,
...
fn parse_category(s: &str) -> Result<BulletinCategory> {
    use adnet_types::BulletinCategory as C;
    Ok(match s.to_ascii_lowercase().as_str() {
        "general" => C::General,
        "security" => C::Security,
        "ops" | "outage" => C::Outage,
...
fn get(svc: &NewsService, room: &str, id: &str, json: bool) -> Result<()> {
    let id = adnet_types::BulletinId::from_hex(id)
