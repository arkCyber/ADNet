...
//!
//! The `adnet diagnostics` CLI subcommand surfaces this to
...
//!   env var points at a real ADNet root before starting the
//!   service.
//!
...
pub struct DiagnosticsSnapshot {
    ...
    /// describes.
    pub data_dir: String,
    /// Full 32-byte NodeId (hex-encoded, no `adnet-` prefix).
    pub node_id: String,
    /// Short fingerprint (first 8 bytes hex-encoded). Matches
    /// the format `adnet-<short>` that other ADNet UIs use.
    pub node_id_short: String,
    /// Hex-encoded Ed25519 public key derived from the same
    /// secret. Mirrors `adnet_types::NodeId::public_key_hex()`
...
///
/// The on-disk format matches what `adnet-identity` writes:
...
mod tests {
    ...
    fn diagnostics_snapshot_missing_dir_errors() {
        let dir = std::env::temp_dir().join("adnet-nonexistent-zzz");
