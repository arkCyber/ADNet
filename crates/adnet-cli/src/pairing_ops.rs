//! `adnet pair`, `adnet invite`, `adnet qr` and `adnet mesh` handlers.
//!
//! Thin wrappers that surface the `adnet-pairing` / `adnet-invite` /
//! `adnet-qr` / `adnet-mesh-coordinator` crates on the existing
//! `adnet` CLI. The handlers are deliberately small — they keep all
//! the cryptographic work in the dedicated crates and just
//! orchestrate the file I/O the CLI needs to expose them.
//!
//! Top-level commands:
//!
//! - `adnet pair create`  — issue a fresh `SignedInvitation` and
//!   persist the issuer record under `<data_dir>/pairing/issuer.json`.
//! - `adnet pair list`    — list trusted-device records persisted
//!   on disk (raw JSON dump, the store itself is append-only JSONL).
//! - `adnet pair revoke`  — drop a trusted-device record by its
//!   hex-encoded `credential_id` (16-byte array, 32 hex chars).
//! - `adnet invite render`— emit a draft `.eml` body that wraps the
//!   most recent issuer record so it can be dropped into an SMTP
//!   pipeline.
//! - `adnet invite text`  — print the human-readable summary of the
//!   most recent issuer record.
//! - `adnet qr render`    — copy the most recent issuer record into
//!   a JSON file that `adnet-qr` can ingest (CLI does not render
//!   SVG directly; use the dedicated example binary for that).
//! - `adnet qr parse`     — read a text payload from disk and decode
//!   it back into a typed `QrPayload`.
//! - `adnet mesh admit`   — queue a `mesh admit` request for a node
//!   id in the closed-mesh flow. The coordinator consumes the
//!   resulting JSON file at `<data_dir>/mesh/pending.json`.

use std::path::{Path, PathBuf};

use adnet_pairing::capability::Capability;
...
pub async fn run_invite(sub: &InviteCmd, data_dir: &Path) -> Result<()> {
    match sub {
        InviteCmd::Render {
            ...
            if !path.exists() {
                return Err(anyhow!(
                    "no issuer record at {} — run `adnet pair create` first",
                    path.display()
                ));
            }
            let raw = std::fs::read(&path)?;
            ...
            let body = format!(
                "From: adnet-pairing@localhost\n\
                 To: {recipient}\n\
                 Subject: {subject}\n\
                 X-Adnet-Pair-Issuer: present\n\
                 \n\
                 Hi,\n\n\
                 You have been invited to pair with this ADNet node. Scan the\n\
                 attached QR code with the ADNet mobile app, or paste the text\n\
...
pub async fn run_qr(sub: &QrCmd, data_dir: &Path) -> Result<()> {
    match sub {
        QrCmd::Parse { input } => {
            ...
            // `adnet_qr::scan::check_qr` is the public surface for
            // decoding an arbitrary QR text payload into the typed
            // `QrPayload` enum.
            match adnet_qr::check_qr(&text) {
                Ok(payload) => {
                    println!("{}", serde_json::to_string_pretty(&payload)?);
                    Ok(())
                }
        ...
        QrCmd::Render {
            ...
            // / TXT rendering lives in the dedicated `adnet-qr`
            // example binary.
            let path = issuer_path(data_dir);
            if !path.exists() {
                return Err(anyhow!(
...
fn parse_capabilities(items: &[String]) -> Result<adnet_pairing::capability::CapabilitySet> {
    let mut out = adnet_pairing::capability::CapabilitySet::default();
    for item in items {
...
mod tests {
    ...
    fn invite_text_errors_without_issuer() {
        ...
        assert!(err.to_string().contains("run `adnet pair create` first"));
    }
