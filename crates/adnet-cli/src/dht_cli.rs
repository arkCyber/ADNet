//! `adnet dht` — DHT command implementations.
//!
//! These handlers use the real `adnet_node::Node` DHT API
//! surface (`dht_provide`, `dht_find_providers`, `dht_find_node`,
//! `dht_num_peers`, `dht_stats`). When `adnet-node` is built
...
//!
//! Commands:
//! - `adnet dht provide <cid>` — announce that we provide a CID
//! - `adnet dht findprovs <cid>` — find providers for a CID
//! - `adnet dht findpeer <peer-id>` — closest peers (Kademlia
//!   iterative FIND_NODE)
//! - `adnet dht peers` — list routing table peers
//! - `adnet dht stats` — show DHT statistics
...
fn parse_cid(s: &str) -> Result<ContentHash> {
    ...
    adnet_types::ContentHash::from_hex(s)
...
/// Run `adnet dht provide <cid>` when the `dht` feature is off.
#[cfg(not(feature = "dht"))]
pub async fn run_dht_provide(
    cid: &str,
    ...
    let _ = parse_cid(cid)?;
    anyhow::bail!(
        "DHT not enabled. Rebuild adnet-cli with `--features dht` to use `adnet dht provide`."
    )
}

/// Run `adnet dht findprovs <cid>`.
#[cfg(feature = "dht")]
...
/// Run `adnet dht findprovs <cid>` when the `dht` feature is off.
#[cfg(not(feature = "dht"))]
pub async fn run_dht_findprovs(
    cid: &str,
    _num: Option<u32>,
    ...
    let _ = parse_cid(cid)?;
    anyhow::bail!(
        "DHT not enabled. Rebuild adnet-cli with `--features dht` to use `adnet dht findprovs`."
    )
}

/// Run `adnet dht findpeer <peer-id>` — iterative FIND_NODE
...
/// Run `adnet dht findpeer <peer-id>` when the `dht` feature is off.
#[cfg(not(feature = "dht"))]
pub async fn run_dht_findpeer(
    peer_id: &str,
    _json: bool,
    ...
    anyhow::bail!(
        "DHT not enabled. Rebuild adnet-cli with `--features dht` to use `adnet dht findpeer`."
    )
}
...
pub async fn run_dht_peers(
    ...
    if json {
        ...
        println!("(use `adnet dht findpeer <id>` to enumerate closest peers)");
    }
    Ok(())
}

/// Run `adnet dht peers` when the `dht` feature is off.
#[cfg(not(feature = "dht"))]
pub async fn run_dht_peers(
    _json: bool,
    _node: &adnet_node::Node,
    ...
    anyhow::bail!(
        "DHT not enabled. Rebuild adnet-cli with `--features dht` to use `adnet dht peers`."
    )
}
...
/// Run `adnet dht stats` when the `dht` feature is off.
#[cfg(not(feature = "dht"))]
pub async fn run_dht_stats(
    _json: bool,
    _node: &adnet_node::Node,
    ...
    anyhow::bail!(
        "DHT not enabled. Rebuild adnet-cli with `--features dht` to use `adnet dht stats`."
    )
}
