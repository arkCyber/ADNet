...
/// Top-level dispatcher for `adnet name <sub>`. Offline — does not require
/// a running node.
pub fn run_name(sub: &CliNameCmd, data_dir: &std::path::Path) -> anyhow::Result<()> {
    ...
    match sub {
        CliNameCmd::Publish { path, lifetime, json } => {
            ...
            // node-side `init_ipns` path via `adnet node` or
            ...
            let msg = "name publish requires a running node. \
                       Run `adnet node init_ipns` on the daemon, then \
                       re-issue this command via the IPC client. \
                       The offline `adnet key gen` command can mint a \
        ...
        CliNameCmd::Resolve { name, recursive, json } => {
            ...
            let msg = "name resolve requires a running node. \
                       ...
                       `adnet ipns lookup`; the local-only path is \
                       via `adnet ipns local`.";
            if *json {
                println!("{{\"error\": \"{msg}\"}}");
            } else {
...
/// Top-level dispatcher for `adnet key <sub>`. Offline — does not require
/// a running node.
