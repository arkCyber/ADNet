//! `adnet share …` — P2P file & directory sharing CLI surface.
//!
//! Subcommands:
//! - `adnet share send <path>` — walk a local file or directory,
//!   ingest it into the local blob store, and print a printable
//!   `ShareTicket` that a peer can hand to `adnet share receive`.
//! - `adnet share receive <ticket>` — materialise a shared
...
//! - `adnet share resume ls|clean|info|continue` — operate on
...
//! `adnet config` / `adnet roster` / `adnet user` — the
//! `run_share_subcmd` dispatcher in `main.rs` fires **before**
//! the `Node` is constructed, so `adnet share send` works in
...
//! `resume.json` + `manifest.bin` (the `adnet_share::resume`
//! module). Both sidecars survive across `adnet` invocations and
...
#[cfg(feature = "iroh")]
use {adnet_share::receive_p2p, adnet_blobstore::IrohBlobStore};
...
async fn run_send(
    ...
    // flow through `adnet_blob_*` immediately.
    ...
    let put_bytes: PutBytesFn = std::sync::Arc::new(move |bytes: &[u8]| {
        let (hash, _size) = store_for_put
            ...
            .map_err(|e| adnet_share::ShareError::Backend(format!("blobstore: {e}")))?;
        Ok(hash)
    });
    ...
    // store. Without this step `adnet share receive` (on the
        ...
        Err(e) => {
            warn!(
                ...
                "share send: could not serialise manifest for local-cache; \
                 adnet share receive will not find the manifest on this machine"
            );
            Vec::new()
        }
    };
...
/// the rest of ADNet uses so a `share send` ticket carries the
/// operator's real NodeId — the same one `adnet diagnostics`
/// reports and the gossip / mesh layers sign with.
fn load_or_create_local_node_id(data_dir: &Path) -> Result<adnet_types::NodeId> {
    ...
    if path.exists() {
        ...
        if let Ok(s) = std::str::from_utf8(&bytes) {
            ...
            if trimmed.len() == 64
                ...
                && let Ok(id) = adnet_types::NodeId::from_bytes(&raw)
            {
                return Ok(id);
            }
        }
...
async fn build_ticket_endpoint(node_id: &adnet_types::NodeId) -> adnet_types::NodeAddr {
    #[cfg(feature = "iroh")]
    {
        match adnet_share::discover_endpoints(node_id).await {
                ...
                if addr.direct.is_none() && addr.relay.is_none() {
                    warn!(
                        "share send: ticket has no direct/relay addresses; \
                         ...
                         Run `adnet serve` first for full P2P support."
                    );
                }
            ...
            Err(e) => {
                ...
                adnet_types::NodeAddr::new(node_id.clone())
            }
        }
    }
...
async fn run_receive_local_only(
    ...
    let incoming_root = adnet_share::resume_dir(data_dir, &manifest_hash);
    std::fs::create_dir_all(&incoming_root)?;
    let mut resume_state = adnet_share::ResumeState::new(ticket, manifest_hash.clone());
    resume_state.total_bytes = manifest_total_bytes(&manifest);
    adnet_share::save(&incoming_root, &resume_state)
    ...
    let stats = adnet_share::receive(ticket, &manifest, store.as_ref(), opts)
        .await
    ...
    for (name, hash) in manifest.iter() {
        ...
        resume_state.files.push(adnet_share::ResumeFileProgress {
            name: name.to_string(),
            hash: hash.clone(),
            total_bytes: size,
            bytes_done: size,
    ...
    resume_state.status = adnet_share::ResumeStatus::Completed;
    adnet_share::save(&incoming_root, &resume_state)
        .map_err(|e| anyhow!("write resume.json: {e}"))?;
...
async fn run_receive_iroh(
    ...
    let stats = adnet_share::receive(ticket, &outcome.manifest, iroh_store.as_ref(), opts)
        .await
...
async fn run_local_receive(
    data_dir: &Path,
    ticket: &ShareTicket,
    manifest: &adnet_share::Collection,
    out_dir: &Path,
    overwrite: bool,
) -> Result<()> {
    let blobs_dir = data_dir.join("blobs");
    ...
    let stats = adnet_share::receive(ticket, manifest, legacy_store.as_ref(), opts)
        .await
        .map_err(|e| anyhow!("receive: {e}"))?;
...
async fn update_resume_complete_from_remote(
    resume_state: &adnet_share::ResumeState,
    data_dir: &Path,
    store: &IrohBlobStore,
    stats: &adnet_share::ReceiveStats,
) -> Result<()> {
    let mut updated = resume_state.clone();
    updated.status = adnet_share::ResumeStatus::Completed;
    ...
    let incoming_root = adnet_share::resume_dir(data_dir, &updated.manifest_hash);
    adnet_share::save(&incoming_root, &updated)
        .map_err(|e| anyhow!("write resume.json: {e}"))?;
    Ok(())
}
...
async fn try_local_manifest(
    store: &BlobStore,
    target: &ContentHash,
) -> Option<adnet_share::Collection> {
    if !store.has(target).await {
        return None;
    }
    let bytes = BlobReader::read_all(store, target).await.ok()?;
    let manifest: adnet_share::Collection = postcard::from_bytes(&bytes).ok()?;
    Some(manifest)
}
...
async fn find_local_manifest(
    store: &BlobStore,
    target: &ContentHash,
) -> Result<adnet_share::Collection> {
    try_local_manifest(store, target).await
        .ok_or_else(|| {
            anyhow!(
                "manifest {} is not in the local blob store; \
                 run `adnet share send <path>` on this machine first \
...
fn manifest_total_bytes(_m: &adnet_share::Collection) -> u64 {
...
fn resume_ls(data_dir: &Path, json: bool) -> Result<()> {
    let mut states = adnet_share::list(data_dir)?;
    ...
    for s in &states {
        let short = &s.manifest_hash.as_hex()[..adnet_share::HASH_SHORT_LEN];
...
fn resume_info(data_dir: &Path, hash_short: &str, json: bool) -> Result<()> {
    ...
    if json {
        ...
        println!("hash_short     : {}", &state.manifest_hash.as_hex()[..adnet_share::HASH_SHORT_LEN]);
...
fn resume_clean(data_dir: &Path, hash_short: &str, yes: bool) -> Result<()> {
    ...
    if !yes {
        let dir = adnet_share::resume_dir(data_dir, &hash);
        eprintln!("adnet: about to delete {}", dir.display());
    ...
    let removed = adnet_share::clean(data_dir, &hash)?;
    if removed {
...
fn load_state_by_short(data_dir: &Path, short: &str) -> Result<Option<adnet_share::ResumeState>> {
    let states = adnet_share::list(data_dir)?;
    for s in states {
...
fn status_label(s: adnet_share::ResumeStatus) -> &'static str {
    match s {
        adnet_share::ResumeStatus::InProgress => "in_progress",
        adnet_share::ResumeStatus::Completed => "completed",
        adnet_share::ResumeStatus::Interrupted => "interrupted",
        adnet_share::ResumeStatus::Failed => "failed",
    }
}
...
mod tests {
    ...
    fn parse_hash_short_accepts_exact_len() {
        ...
        assert_eq!(s.len(), adnet_share::HASH_SHORT_LEN);
        let hash = parse_hash_short(s).unwrap();
        assert_eq!(&hash.as_hex()[..adnet_share::HASH_SHORT_LEN], s);
    }

    #[test]
    fn status_label_matches_variant() {
        assert_eq!(status_label(adnet_share::ResumeStatus::InProgress), "in_progress");
        assert_eq!(status_label(adnet_share::ResumeStatus::Completed), "completed");
        assert_eq!(status_label(adnet_share::ResumeStatus::Interrupted), "interrupted");
        assert_eq!(status_label(adnet_share::ResumeStatus::Failed), "failed");
    }
