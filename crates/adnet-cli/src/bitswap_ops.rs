...
//! ```bash
//! # Request a block
//! adnet-cli bitswap want QmXoypizjW3WknFiJnKLwHCnL72vedxjQkDDP1mXWo6uco
//!
//! # List current wants
//! adnet-cli bitswap ls
//!
//! # Show statistics
//! adnet-cli bitswap stat
//!
//! # Show peer ledgers
//! adnet-cli bitswap ledger
//!
//! # Cancel a want
//! adnet-cli bitswap cancel QmXoypizjW3WknFiJnKLwHCnL72vedxjQkDDP1mXWo6uco
//!
//! # Announce local content
//! adnet-cli bitswap announce QmXoypizjW3WknFiJnKLwHCnL72vedxjQkDDP1mXWo6uco
//! ```

use std::sync::Arc;
use std::time::Duration;

use adnet_node::{BitswapHandle, BitswapStats};
...
pub struct BitswapCliContext {
    ...
    adapter: Arc<RwLock<Option<Arc<adnet_node::BitswapNetworkAdapter>>>>,
}
...
impl BitswapCliContext {
    ...
    pub async fn set_adapter(&self, adapter: Arc<adnet_node::BitswapNetworkAdapter>) {
    ...
    pub async fn get_adapter(&self) -> Result<Arc<adnet_node::BitswapNetworkAdapter>> {
    ...
    if let Some(adapter) = adapter {
        ...
        for provider in providers.iter().take(3) {
            ...
            match handle.want_block_from_peer(
                ...
                adnet_node::BitswapBlockResult::Received { hash, from } => {
                    println!("Successfully received block {} from {}", hash.short(), from);
                    return Ok(());
                }
                adnet_node::BitswapBlockResult::Local(ref local_hash) => {
                    println!("Content {} is available locally!", local_hash.short());
                    return Ok(());
                }
                adnet_node::BitswapBlockResult::NotFound => {
                    println!("Provider {} does not have the content", peer_id.short());
                    continue;
                }
                adnet_node::BitswapBlockResult::Error(e) => {
...
async fn execute_stat(ctx: &BitswapCliContext, _args: ListArgs) -> Result<()> {
    ...
    println!("  adnet-cli bitswap ledger --verbose");
...
/// Top-level dispatcher for `adnet bitswap <sub>`. Requires a running node
