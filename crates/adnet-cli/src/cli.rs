...
//! - `qr render|parse`        — ADNet QR payload handling
...
pub struct Cli {
    ...
    /// Log filter (e.g., info,adnet=debug).
...
pub enum Cmd {
    ...
    /// Render / parse ADNet QR payloads (pairing, peer tickets,
    ...
    /// Manage webhook endpoints (`adnet-webhook`).
    Webhook {
        #[command(subcommand)]
        sub: WebhookCmd,
    },
...
pub enum InviteCmd {
    ...
    Render {
        ...
        recipient: String,
        #[arg(long, default_value = "Pair with me on ADNet")]
        subject: String,
...
pub enum QrCmd {
    ...
    Render {
        ...
        /// dedicated `adnet-qr` example binary.
...
/// `adnet share resume <sub>` subcommands.
#[derive(Debug, Subcommand)]
