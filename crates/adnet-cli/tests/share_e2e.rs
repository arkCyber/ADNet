//! End-to-end integration tests for `adnet share …`.
//!
//! Exercises the full send → receive round-trip via the public
//! `adnet_cli::share::run` entry point, plus the
//! `adnet share resume …` operators on the sidecar that
...
use adnet_cli::cli::{ShareCmd, ShareResumeCmd};
use adnet_cli::share;
use adnet_share::{ResumeStatus, ShareTicket};
...
/// `adnet_share::walk_import`. Used by the receive-side tests
...
async fn ticket_for_dir(source: &Path) -> Result<(ShareTicket, ContentHash)> {
    ...
    let (manifest, manifest_hash, _stats) =
        adnet_share::walk_import(source, put, adnet_share::WalkOptions::default())
            .await?;
...
fn share_send_then_receive_round_trip() -> Result<()> {
    ...
    let short_hex = &manifest_hash.as_hex()[..adnet_share::HASH_SHORT_LEN];
    let incoming = data_dir.path().join("incoming").join(short_hex);
    assert!(incoming.exists(), "incoming/<short>/ was created");
    let resume_state = adnet_share::load(&incoming)?.expect("resume.json exists");
...
fn share_resume_ls_lists_completed_receives() -> Result<()> {
    ...
    // `adnet share resume ls` (human-readable, no JSON) — we
    ...
    // `adnet_share::list` exercises. We re-call it directly so
    // we can inspect the returned state without stdout tricks.
    let states = adnet_share::list(data_dir.path())?;
    assert_eq!(states.len(), 1);
    assert_eq!(
        &states[0].manifest_hash.as_hex()[..adnet_share::HASH_SHORT_LEN],
        short_hex
    );
    ...
    let incoming = data_dir.path().join("incoming").join(short_hex);
    let state = adnet_share::load(&incoming)?.unwrap();
...
fn share_resume_clean_refuses_in_progress() -> Result<()> {
    ...
    let empty_manifest = adnet_share::Collection::new();
    let ticket =
        ShareTicket::new(&node_id, &endpoint, &manifest_hash, &empty_manifest, 0)?;

    let incoming = adnet_share::resume_dir(data_dir.path(), &manifest_hash);
    fs::create_dir_all(&incoming)?;
    let mut state = adnet_share::ResumeState::new(&ticket, manifest_hash.clone());
    state.status = ResumeStatus::InProgress;
    adnet_share::save(&incoming, &state)?;
...
fn share_receive_bumps_share_metrics_counters() -> Result<()> {
    ...
    // `adnet_share_receive_*` Prometheus counters. This test
    ...
    let before_total_bytes = adnet_share::share_metrics().receive_bytes_total.get();
    let before_total_files = adnet_share::share_metrics().receive_files_total.get();
    let before_done_bytes = adnet_share::share_metrics().receive_bytes_done.get();
    let before_done_files = adnet_share::share_metrics().receive_files_done.get();
    let before_count = adnet_share::share_metrics().receive_seconds.count();
    ...
    let m = adnet_share::share_metrics();
