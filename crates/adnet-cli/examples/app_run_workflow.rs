...
use adnet_cli::{feed_view::feed_for_humans, Cli, Cmd};
use adnet_node::RoomFeed;
use adnet_types::{BlobTicket, ContentHash, NodeAddr, NodeId, RoomAsset, RoomId};
use anyhow::Result;
use clap::Parser;
...
fn main() -> Result<()> {
    println!("=== ADNet CLI: end-to-end ingest + announce + feed ===\n");
    ...
    let sequence = [
        ("adnet status", "snapshot of node + mesh"),
        ("adnet feed --room home-photos", "re-read the feed"),
        ("adnet get <cid> -o ./holiday.jpg", "download the asset"),
        ("adnet pin add <cid>", "pin it locally"),
        ("adnet diagnostics --json", "gather diagnostics"),
    ];
