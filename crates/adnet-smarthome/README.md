...
智能家居设备控制中心 - 集成在 ADNet NAS 服务器中，支持多种智能家居协议和设备。
...
```rust
use adnet_smarthome::api::{self, ApiConfig};
...
```rust
use adnet_smarthome::mqtt::MqttConfig;
...
ADNet 通过 Matter 协议与 Apple Home app 无缝集成。Matter 是 HomeKit 的现代替代方案，支持原生 Apple HomeKit。

### 架构

```
┌─────────────────┐     ┌──────────────────┐     ┌─────────────────┐
│  ADNet Hub      │────▶│  Matter 协议     │────▶│  Apple Home App │
...
### ADNet → Matter → HomeKit 映射表

| ADNet 能力 | Matter Cluster | HomeKit 等效 |
|------------|:--------------:|--------------|
...
1. 通过 ADNet CLI 或 REST API 配对 Matter 设备
...
```rust
use adnet_smarthome::{Result, SmartHomeError};
