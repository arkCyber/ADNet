...
pub struct MqttConfig {
    ...
    /// MQTT broker port
    pub port: u16,
    /// Client ID (defaults to `adnet-smarthome-{hostname}`)
    pub client_id: Option<String>,
    /// Username (optional)
    pub username: Option<String>,
    /// Password (optional)
    pub password: Option<String>,
    /// Topic base namespace (default: "adnet")
...
impl MqttClient {
    ...
    pub fn new(config: MqttConfig) -> Result<Self> {
        let client_id = config
            .client_id
            .clone()
            .unwrap_or_else(|| format!("adnet-smarthome-{}", hostname()));
...
mod tests {
    ...
    #[test]
    fn mqtt_topic_state() {
        let topic = MqttTopic::new("adnet");
        assert_eq!(topic.state("lamp-1", "2.1"), "adnet/lamp-1/2.1");
    }

    #[test]
    fn mqtt_topic_set() {
        let topic = MqttTopic::new("adnet");
        assert_eq!(topic.set("lamp-1"), "adnet/lamp-1/set");
    }

    #[test]
    fn mqtt_topic_availability() {
        let topic = MqttTopic::new("adnet");
        assert_eq!(topic.availability("lamp-1"), "adnet/lamp-1/availability");
    }

    #[test]
    fn mqtt_topic_discovery() {
        let topic = MqttTopic::new("adnet");
        assert_eq!(topic.discovery(), "adnet/discovery");
    }
    ...
    async fn mqtt_client_new_succeeds_against_unreachable_broker() {
        let cfg = MqttConfig {
            ...
            // construction.
            port: 1,
            client_id: Some("adnet-smarthome-test".into()),
            username: None,
            password: None,
            topic_base: "adnet".into(),
            keep_alive_secs: 5,
