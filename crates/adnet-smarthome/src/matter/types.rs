//! Matter cluster → ADNet device capability mapping.
//!
//! Maps Matter cluster IDs (CSA spec, rev 1.4) to the protocol-agnostic
//! ADNet [`DeviceCapability`] set so the hub can treat Matter devices
...
impl MatterCluster {
    /// ADNet capabilities this cluster exposes.
...
impl MatterClusterSpec {
    /// Convert cluster IDs → ADNet capabilities.
