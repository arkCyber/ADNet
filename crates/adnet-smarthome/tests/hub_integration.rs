...
use adnet_smarthome::miot::{Action, MiotAuth, Property, PropertyValue};
use adnet_smarthome::{
    api::{self, ApiConfig},
...
fn temp_data_dir(label: &str) -> std::path::PathBuf {
    ...
    std::env::temp_dir().join(format!("adnet-smarthome-it-{}-{}", label, suffix))
}
...
async fn without_miot_property_calls_return_not_supported() {
    ...
    let err = hub
        ...
        .await
        .unwrap_err();
    assert!(matches!(err, adnet_smarthome::SmartHomeError::NotSupported(_)));
