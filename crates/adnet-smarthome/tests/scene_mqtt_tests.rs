...
use adnet_smarthome::scene::{Scene, SceneAction, SceneManager};
use adnet_smarthome::mqtt::{MqttConfig, MqttTopic};
use adnet_smarthome::homekit::{HomeKitConfig, HomeKitBridge, HomeKitCategory, HomeKitAccessory};
use adnet_smarthome::device::{Device, DeviceType, DeviceCapability};
...
fn mqtt_config_default() {
    ...
    assert_eq!(config.topic_base, "adnet");
...
fn temp_dir() -> String {
    ...
    std::env::temp_dir()
        .join(format!("adnet-smarthome-it-{}-{}", "scene-mqtt-hk", suffix))
        .to_string_lossy()
        .into_owned()
}
