...
//! the TUN device and resolves `.ray` / `.adnet` names
...
use adnet_types::VirtualIp;
use adnet_tun::packet::{IpProtocol, IpVersion, ParsedPacket};
use tracing::debug;
...
/// Magic DNS forwarder that intercepts `.ray` / `.adnet` DNS
...
mod tests {
    use super::*;
    use adnet_tun::packet::parse_packet;
    ...
    fn is_mesh_query() {
        ...
        assert!(fwd.is_mesh_query("bob.gaming.adnet")); // adnet is built-in
