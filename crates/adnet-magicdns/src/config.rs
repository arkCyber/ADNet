...
pub struct ResolverConfig {
    ...
    /// forwarding entirely (only `.ray` / `.adnet` names
    /// will resolve).
    ...
    /// `["adnet", "mesh"]`) so that `alice.adnet` is
...
impl ResolverConfig {
    ...
    /// recognised mesh TLD (`.ray` or `.adnet`, or an extra TLD).
    pub fn is_mesh_tld(&self, tld: &str) -> bool {
        let tld_lower = tld.to_ascii_lowercase();
        if tld_lower == TLD_LOWER || tld_lower == "adnet" {
            return true;
        }
        self.extra_tlds
            .iter()
...
mod tests {
    ...
    #[test]
    fn is_mesh_tld_recognises_adnet() {
        let c = cfg();
