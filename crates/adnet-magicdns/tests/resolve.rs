//! Integration tests for `adnet-magicdns`.

use adnet_magicdns::{MagicError, MagicName, Resolver, ResolverConfig};
use adnet_types::{MeshMember, MeshMembership, MeshNetworkId, NodeId, VirtualIp};
...
fn snapshot_roundtrips_via_serde() {
    ...
    let back: adnet_magicdns::ResolverSnapshot = serde_json::from_str(&json).unwrap();
