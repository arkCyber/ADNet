//! `adnet-magicdns` 应用示例：构造一个 resolver，应用两个网络 roster，
//! 然后批量解析一组名字并展示 `VirtualIp` 的确定性。
//!
//! 运行：`cargo run -p adnet-magicdns --example magicdns_app`

use adnet_magicdns::{Resolver, ResolverConfig};
use adnet_types::{MeshMember, MeshMembership, MeshNetworkId, VirtualIp};

fn main() -> Result<(), Box<dyn std::error::Error>> {
    println!("--- adnet-magicdns app demo ---");
