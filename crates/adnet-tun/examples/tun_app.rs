//! `adnet-tun` 应用示例：模拟一次 mesh 内两个节点之间的 UDP 包往返。
//! 把 `UserspaceTun` 当成一个 packet loop 沙盒，演示 send / inject / drain 的组合用法。
//!
//! 运行：`cargo run -p adnet-tun --example tun_app`

use adnet_tun::{TunDevice, UserspaceTun, UserspaceTunConfig};
use adnet_types::{VirtualIp, VirtualIpv4};

fn main() -> Result<(), Box<dyn std::error::Error>> {
    println!("--- adnet-tun app demo ---");

    let tun = UserspaceTun::new(UserspaceTunConfig {
        name: "adnet-tun-app".into(),
        mtu: 1420,
