...
//! (e.g. type mismatches between `adnet-types` versions,
//!   behaviour drift between `adnet-mesh-coordinator`'s
//!   `roster()` shape and what `adnet-magicdns` expects).

use std::net::{IpAddr, Ipv4Addr};
use std::sync::Arc;

use adnet_exit_node::{Client, Gateway, RouteAction, Router};
...
use adnet_tun::{TunDevice, UserspaceTun, UserspaceTunConfig, parse_packet};
use adnet_types::{MeshNetworkId, MeshPolicy, NodeId, VirtualIp};
...
fn end_to_end_three_members_with_firewall_and_router() {
    ...
    tun.bring_up();
    assert_eq!(tun.state(), adnet_tun::DeviceState::Up);
    ...
    assert_eq!(parsed.protocol, adnet_tun::IpProtocol::Tcp);
...
fn end_to_end_pending_request_does_not_resolve() {
    ...
    assert!(matches!(
        err,
        adnet_magicdns::MagicError::UnknownHost(_, _)
    ));
