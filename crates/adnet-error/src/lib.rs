//! `adnet-error` — unified error model for ADNet.
//!
...
/// ADNet's transports actually distinguish.
///
...
impl AdnetErrorReport {
    ...
    /// `adnet-observability` crate's metrics counter is
    /// updated when the `metrics` feature is enabled
    /// (the dependency is optional so `adnet-error` can
    ...
    pub fn emit(&self) {
        ...
        // the dependency graph (i.e. `adnet-observability`
        // can in turn depend on this crate without forming a
        // cycle).
        #[cfg(feature = "metrics")]
        adnet_metrics_emit(self);
    }
}

#[cfg(feature = "metrics")]
fn adnet_metrics_emit(report: &AdnetErrorReport) {
    // Direct call into `adnet-observability` — kept behind
