//! `adnet-backup` — snapshot / restore for an ADNet data directory.
//!
//! ADNet stores a mix of binary blobs, JSONL spools, SQLite
...
//! ## Header
//!
//! The first 16 bytes of every `.adnet-snap` file are the magic
...
pub enum BackupError {
    ...
    #[error("archive is missing magic header — is this an adnet-snap file?")]
    BadMagic,
...
/// The output is a self-contained `.adnet-snap` file with the
...
mod tests {
    ...
    fn snapshot_then_restore_round_trip() {
        ...
        let out = snap.path().join("snap.adnet-snap");
    ...
    fn empty_source_dir_produces_empty_manifest() {
        ...
        let out = tmp.path().join("snap.adnet-snap");
        let manifest = snapshot(src.path(), &out).unwrap();
    ...
    fn restore_rejects_non_snapshot_file() {
        ...
        std::fs::write(&bogus, b"this is definitely not an adnet snapshot").unwrap();
        let dst = tempfile::tempdir().unwrap();
