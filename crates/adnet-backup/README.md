...
Snapshot / restore for an ADNet data directory. Packages blobstore,
gossip spools, DHT store, pair records and identity material into a
single self-describing `.adnet-snap` file.
...
```rust,no_run
use adnet_backup::{snapshot, restore, describe};

// Capture
let manifest = snapshot("/var/lib/adnet", "/tmp/snap.adnet-snap")?;
println!("{}", describe(&manifest));

// Verify (e.g. on the receiving end before doing anything)
let _ = adnet_backup::verify("/tmp/snap.adnet-snap")?;

// Restore
let restored = restore("/tmp/snap.adnet-snap", "/var/lib/adnet-new")?;
```
