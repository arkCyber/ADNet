# adnet-integration-tests

> 跨 crate 的端到端集成测试套件,验证整个 ADNet 协议栈在真实多节点网络条件下的一致性与弹性(End-to-end integration tests for the full ADNet stack)。

## 概览 (Overview)

`adnet-integration-tests` 是 ADNet 协议栈的"系统测试"层。它把 `adnet-node`、`adnet-dht`、`adnet-gossip`、`adnet-blobstore`、`adnet-relay`、`adnet-identity`、`adnet-transport`、`adnet-simulator` 等多个 crate 串起来,在受控的临时目录、临时节点 ID、临时网络条件之下,跑出真实协议交互的端到端 happy-path 和故障恢复路径。
...
- `chaos_tests` — `src/chaos.rs`,使用 `adnet-simulator` 的 `NetworkEmulator` 制造故障。
...
- **可注入网络条件**:通过 `adnet-simulator::NetworkEmulator`,可以在测试运行时人为拉低延迟、加丢包或者制造分区。
...
# 单独跑网络层
cargo test -p adnet-integration-tests --no-default-features --features network_tests

# 把全部模块打开
cargo test -p adnet-integration-tests --features legacy_tests
```
...
```toml
[dev-dependencies]
adnet-integration-tests = { path = "../adnet-integration-tests" }
```
...
use adnet_integration_tests::network;
...
```rust,ignore
let ok = adnet_integration_tests::wait_for(|| async { false }, 1).await;
...
```rust,ignore
let dir = adnet_integration_tests::temp_dir();
...
2. **网络层回归** —— 持续跑 `cargo test -p adnet-integration-tests --features network_tests`,每次 DHT / Gossip 的改动都能在此被捕获。
3. **故障演练** —— `chaos_tests` feature 打开后,通过 `adnet-simulator::NetworkEmulator` 注入丢包 / 分区 / 限速,验证 gossip / bitswap 的重试与降级路径。
