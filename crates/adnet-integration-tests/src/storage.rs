...
mod tests {
    ...
    use adnet_blobstore::{BlobStore, BlobImporter, BlobReader, CHUNK_SIZE};
    ...
    async fn test_blobstore_complete_check() {
        ...
        let fake_hash = adnet_types::ContentHash::from_bytes(b"fake-content-not-existing");
    ...
    async fn test_blobstore_nonexistent_read() {
        ...
        let fake_hash = adnet_types::ContentHash::from_bytes(b"nonexistent-content");
    ...
    async fn test_blobstore_with_gossip() {
        ...
        // Create gossip bus
        let transport = Arc::new(adnet_gossip::InProcessGossip::new());
        let node_id = adnet_types::NodeId::random();
        let bus = adnet_gossip::GossipBus::new(node_id, transport);

        let room: adnet_types::RoomId = "content-room".into();
        ...
        let ann = adnet_types::Announcement {
            room_id: room.clone(),
            content_hash: hash,
            node_id: adnet_types::NodeId::random(),
