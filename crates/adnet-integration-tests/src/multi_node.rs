...
mod tests {
    ...
    use adnet_dht::DhtNode;
    use adnet_gossip::{GossipBus, InProcessGossip};
    use adnet_simulator::{NetworkTopology, presets, NetworkEmulator, ConnectionId};
    ...
    async fn test_two_nodes_connect_and_exchange() {
        ...
        let bus1 = GossipBus::new(adnet_types::NodeId::random(), Arc::clone(&transport) as _);
        let bus2 = GossipBus::new(adnet_types::NodeId::random(), Arc::clone(&transport) as _);

        let room: adnet_types::RoomId = "two-node-room".into();
        ...
        let ann = adnet_types::Announcement {
            room_id: room.clone(),
            content_hash: adnet_types::ContentHash::from_bytes(b"two-node-content"),
    ...
    async fn test_two_nodes_bidirectional() {
        ...
        let room: adnet_types::RoomId = "bidirectional".into();
        ...
        let ann1 = adnet_types::Announcement {
            room_id: room.clone(),
            content_hash: adnet_types::ContentHash::from_bytes(b"from-bus1"),
            node_id: bus1.local_node().clone(),
        ...
        let ann2 = adnet_types::Announcement {
            room_id: room.clone(),
            content_hash: adnet_types::ContentHash::from_bytes(b"from-bus2"),
    ...
    async fn test_five_nodes_all_receive() {
        ...
        let room: adnet_types::RoomId = "five-node-room".into();
        ...
        let ann = adnet_types::Announcement {
            room_id: room.clone(),
            content_hash: adnet_types::ContentHash::from_bytes(b"five-node-content"),
    ...
    async fn test_ten_nodes_partial_join() {
        ...
        let room: adnet_types::RoomId = "partial-join-room".into();
        ...
        let ann = adnet_types::Announcement {
            room_id: room.clone(),
            content_hash: adnet_types::ContentHash::from_bytes(b"partial-content"),
            node_id: buses[0].local_node().clone(),
    ...
    async fn test_cluster_ring_topology() {
        ...
        let room: adnet_types::RoomId = "ring-room".into();
        ...
        let ann = adnet_types::Announcement {
            room_id: room.clone(),
            content_hash: adnet_types::ContentHash::from_bytes(b"ring-content"),
            node_id: buses[0].local_node().clone(),
    ...
    async fn test_many_messages_many_nodes() {
        ...
        let room: adnet_types::RoomId = "load-test-room".into();
        ...
        for (i, bus) in buses.iter().enumerate() {
            ...
            let handle = tokio::spawn(async move {
                for j in 0..messages_per_node {
                    let ann = adnet_types::Announcement {
                        room_id: room.clone(),
                        content_hash: adnet_types::ContentHash::from_bytes(
    ...
    async fn test_partition_isolation() {
        ...
        let bus1 = GossipBus::new(adnet_types::NodeId::random(), transport1);
        let bus2 = GossipBus::new(adnet_types::NodeId::random(), transport2);

        let room: adnet_types::RoomId = "partitioned-room".into();
        ...
        let ann = adnet_types::Announcement {
            room_id: room.clone(),
            content_hash: adnet_types::ContentHash::from_bytes(b"partitioned-content"),
            node_id: bus1.local_node().clone(),
    ...
    async fn test_rapid_subscribe_unsubscribe() {
        ...
        let transport = Arc::new(InProcessGossip::new());
        let bus = GossipBus::new(adnet_types::NodeId::random(), transport);

        let room: adnet_types::RoomId = "rapid-sub-room".into();
        bus.join_room(&room).await.expect("join failed");
        ...
        let ann = adnet_types::Announcement {
            room_id: room,
            content_hash: adnet_types::ContentHash::from_bytes(b"stress-content"),
            node_id: bus.local_node().clone(),
