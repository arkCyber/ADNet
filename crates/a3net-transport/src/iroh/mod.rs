...
//!   `a3net/frame/1` ALPN handler on the same router as
...
//! A3Net's [`NodeId`]; arbitrary A3Net ids and native-QUIC certificate
...
impl Default for IrohTransport {
    ...
    /// a hard `cargo build -p a3net-node --example
...
impl IrohTransport {
    ...
    /// Wrap a sole-owner `iroh::Endpoint` as an A3Net transport.
...
/// the local A3Net `NodeId`. The `a3net/frame/1` ALPN is advertised
/// so the standard A3Net framed transport still works.
#[cfg(feature = "iroh")]
...
/// [`IrohRuntime`](crate::IrohRuntime) `Router`) as an A3Net
...
/// Callers should `use a3net_transport::iroh_prelude::*` to get
...
/// `a3net-observability` global registry. Feature-gated
/// on `iroh` because the source types live behind this
/// cfg.
#[cfg(feature = "iroh")]
pub mod metrics_bridge;

/// Frame-level `ProtocolHandler` for the custom `a3net/frame/1`
/// ALPN. See [`frame_handler::IrohFrameHandler`].
#[cfg(feature = "iroh")]
pub mod frame_handler;

/// Persistent Ed25519 identity, used to root an iroh endpoint so the
/// resulting `EndpointId` is exactly equal to the local A3Net
...
/// ALPN identifier used by the A3Net framed transport. The
/// matching `Endpoint::connect(.., b"a3net/frame/1")` call on the
/// dial side is wired to accept through the iroh `Router` via
/// [`IrohFrameHandler`].
#[cfg(feature = "iroh")]
pub const ADNET_FRAME_ALPN: &[u8] = b"a3net/frame/1";

// ─────────────────────────── NodeId ⇄ PublicKey bridge ──────────────────

/// Reinterpret an A3Net [`NodeId`] as an iroh `PublicKey`. The 32 raw
...
/// Convert an iroh `PublicKey` to an A3Net [`NodeId`].
#[cfg(feature = "iroh")]
...
/// Reinterpret an A3Net [`NodeAddr`] (with `direct: Endpoint` and
...
mod tests {
    ...
    use crate::frame::Frame;
    #[cfg(not(feature = "iroh"))]
    use a3net_types::{Endpoint, RelayUrl};
    #[cfg(not(feature = "iroh"))]
    ...
    /// (`a3net_node::node::NodeBuilder::with_iroh_runtime`) rely
    ...
    async fn iroh_endpoint_frame_round_trip_via_router() {
        ...
        let mut eaddr = crate::iroh::node_addr_to_endpoint_addr(&a3net_types::NodeAddr {
            node_id: server_node,
            direct: None,
            relay: None,
        })
