...
//! `Router` invokes for incoming `a3net/frame/1` connections.
//!
//! When two A3Net nodes talk over an iroh `Endpoint`, the wire-level
//! handshake is identical to the iroh blobs / gossip ALPNs, but the
//! payload is the A3Net `Frame` codec (see [`crate::frame`]). The
...
//! confirms a single `Frame` round-trips on `a3net/frame/1`.
...
/// A single incoming `a3net/frame/1` connection, ready to be wrapped
...
/// result here. A3Net's `Transport` talks in frames — one stream per
...
/// [`ProtocolHandler`] for `a3net/frame/1`.
///
/// The handler is `Clone` (the `mpsc::Sender` is) so the router can
/// keep one strong reference and A3Net can keep another for
...
impl IrohFrameHandler {
    /// Build a handler that forwards every accepted `a3net/frame/1`
...
mod tests {
    ...
    /// Real two-endpoint round-trip on `a3net/frame/1` via the
