...
//! `a3net-observability` global registry.
//!
//! ## What this file does
//!
//! The V8 audit called for a "transport-diagnostics →
//! `a3net-observability`" bridge. The bridge trait and
//! alignment logic live in `a3net-observability::bridge`
//! (transport-agnostic). This file is the
//! `a3net-transport`-side adapter: it implements
//! [`a3net_observability::bridge::DiagnosticsSource`] for
...
use a3net_observability::bridge::{
...
/// [`a3net_observability::bridge::update_from_source`].
pub fn publish_discovery_metrics(diag: &DiscoveryDiagnostics) {
    let adapter = DiscoverySourceAdapter::capture(diag);
    update_from_source_into(&a3net_observability::bridge::DISCOVERY, &adapter);
}
...
pub async fn publish_endpoint_metrics(rec: &EndpointDiagnosticsRecorder) -> bool {
    match EndpointSourceAdapter::from_recorder(rec).await {
        Some(adapter) => {
            update_endpoint_from_source_into(&a3net_observability::bridge::ENDPOINT, &adapter);
            true
        }
        None => false,
    }
...
mod tests {
    ...
    fn for_tests() -> (DiscoveryMetrics, EndpointMetrics) {
        let reg = a3net_observability::registry::Registry::default();
        (
