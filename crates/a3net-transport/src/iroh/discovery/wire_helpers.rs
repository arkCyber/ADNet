...
//! reach for when wiring A3Net nodes into a custom relay /
...
///
/// ## Why this is the A3Net helper, not an iroh-dns helper
///
/// iroh-dns intentionally does not expose this inverse because
/// its serialiser is an internal implementation detail. A3Net
/// surfaces it because operators debugging wire-format
/// mismatches between A3Net nodes and stock iroh nodes need a
...
pub fn parse_transport_addr(s: &str) -> Result<TransportAddr, TxtAddrParseError> {
    ...
    // wire. A3Net operators that want to gate which custom
...
mod tests {
    ...
    fn endpoint_info_from_borrow_preserves_ip_addresses() {
        ...
        // A3Net's borrow-form helper is a thin wrapper over
