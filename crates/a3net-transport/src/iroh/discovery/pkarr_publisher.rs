...
//!
//! This is the A3Net-facing hook for **custom pkarr relays** (for
...
/// A3Net's discovery stack stays wire-compatible with stock iroh
...
impl From<UserData> for IrohUserData {
    /// Convert A3Net's `UserData` into iroh's wire-format
    ...
    fn from(value: UserData) -> Self {
        ...
        // when the input exceeds `MAX_LENGTH` (245). A3Net's
        // `UserData::new` already gates on the same bound, so a
        // freshly-constructed A3Net `UserData` is always a valid
...
impl From<IrohUserData> for UserData {
    /// Convert iroh's wire-format `UserData` into A3Net's
...
/// but at the A3Net layer: callers describe what they want in
...
impl<L: AddressLookup> AddressLookup for InstrumentedPublisher<L> {
    fn publish(&self, data: &EndpointData) {
        ...
        // safe. The A3Net→iroh conversion is infallible (both
...
mod tests {
    ...
    fn user_data_to_iroh_roundtrip() {
        ...
        let a3net_ud = UserData::new(s).unwrap();
        let iroh_ud: IrohUserData = a3net_ud.clone().into();
        let back: UserData = iroh_ud.into();
        assert_eq!(a3net_ud, back);
    }
    ...
    fn user_data_to_iroh_at_max_length() {
        ...
        let a3net_ud = UserData::new(s.clone()).unwrap();
        let iroh_ud: IrohUserData = a3net_ud.into();
