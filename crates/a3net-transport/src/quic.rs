...
/// Default `ALPN` used by A3Net's QUIC transport.
pub const DEFAULT_ALPN: &[u8] = b"a3net/0";
...
impl TransportIdentity {
    pub fn generate() -> Result<Self, TransportError> {
        let params = rcgen::CertificateParams::new(vec!["a3net".to_string()])
...
impl Transport for QuicTransport {
    async fn dial(&self, node: NodeId) -> TransportResult<Box<dyn OutgoingConnection>> {
        ...
        let result = self
            .dial_addr(NodeAddr::new(node).with_direct(a3net_types::Endpoint::new(
                addr.ip().to_string(),
                addr.port(),
            )))
            .await;
...
mod tests {
    ...
    fn default_alpn_is_correct() {
        assert_eq!(DEFAULT_ALPN, b"a3net/0");
    }
    ...
    async fn dial_addr_bad_port() {
        ...
        addr.direct = Some(a3net_types::Endpoint::new("127.0.0.1", 0));
        let err = t.dial_addr(addr).await.unwrap_err();
