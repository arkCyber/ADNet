...
> 抽象传输层：把上层（`a3net-node`）与底层 QUIC / iroh 解耦，提供统一的连接与成帧 API。 / Abstract transport layer — uniform connection / framing API decoupling `a3net-node` from QUIC / iroh.

## 概览 (Overview)

`a3net-transport` 是 A3Net P2P 节点之间的"线"。它对外暴露一组最小但完整的 trait：
...
`a3net-mesh` 提供的 HTTP fallback 永远可用，传输层离线时上层会无缝退化。
...
use a3net_transport::{QuicTransportBuilder, Transport};
...
let target = a3net_types::NodeAddr::new(server.local_node_id().clone())
...
```rust,no_run
use a3net_transport::derive_node_id_from_cert;
...
- **`a3net-node`** 在 `NodeBuilder::with_transport` 处接收任何实现 `Transport` 的实例，
  生产环境是 `QuicTransport`，开发环境可以换成 in-memory loopback。
- **`a3net-blobstore`** 通过 `fetch_blob_over_transport` / `serve_blob_request` 利用
  `Frame` 把 blob 切成 chunk 流过同一条传输链路。
- **`a3net-dht`** 在 `DhtTransportAdapter` / `TransportBridge` 处复用 `Transport` 的连接
