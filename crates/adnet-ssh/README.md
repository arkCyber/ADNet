...
>    speaks the `adnet/ssh-tunnel/1` ALPN, and proxies each
...
>    format and adds an ADNet-specific `from-adnet` option.

## Why

Upstream `iroh-ssh` proves the idea works: turn SSH into a
peer-to-peer service by tunneling the daemon's TCP stream through
an iroh QUIC connection, so any ADNet node can be addressed by its
endpoint id (`adnet-<short>`) without port forwarding, dynamic
DNS, or a public IP. This crate ports the same idea onto the
ADNet stack:

- reuses the durable Ed25519 identity ADNet already mints
  (`<data-dir>/iroh_secret_key`), so the SSH endpoint id equals
  the ADNet node id (no drift across restarts, no second identity
  file);
- advertises the namespaced `adnet/ssh-tunnel/1` ALPN, so a
  server here will never accidentally accept a connection from a
  generic `adnet/frame/1` peer (or vice-versa);
- ships an auth layer that understands both OpenSSH's standard
  `authorized_keys` file *and* an ADNet-specific
  `from-adnet=<endpoint-id>` option for endpoint-scoped allow-lists;
- sits behind the same `adnet-cli` REPL as the rest of the
...
| `metrics`    | Prometheus counters (`adnet_ssh_*` series)                    |
...
The server side loads the persistent ADNet identity from
...
identity file is the same one the rest of ADNet uses, so this
id also matches the `adnet` node id shown by `adnet info`.
...
`adnet-ssh` doesn't bundle an SSH client on purpose; it leans on
...
```bash
$ ssh alice@<endpoint-id>  # or ssh alice@<endpoint-id>.adnet.local
```

Under the hood `adnet-ssh proxy <token>` resolves the
`user@<endpoint-id>` token, opens a QUIC stream to the ALPN
`adnet/ssh-tunnel/1`, spawns the system `ssh(1)` in proxy mode
...
The same key powers the `adnet/frame/1` ALPN, the gossip layer,
and the blob store — so the SSH endpoint id is *always* equal
to the ADNet node id printed by `adnet info`. There is no second
...
`adnet-ssh` ships two auth mechanisms out of the box. They live
under `keys::authorized_keys` (`AuthorizedKeys`,
`AuthorizedKeyEntry`) and `keys::authorized_keys::TrustedPeers`.

### `authorized_keys`

A standard OpenSSH `~/.ssh/authorized_keys` file with one ADNet
extension: the `from-adnet=<endpoint-id>` option.

```text
# Standard OpenSSH form (key blob authenticated by the SSH protocol)
ssh-ed25519 AAAAC3NzaC1lZDI1NTE5AAAAILiH… alice@laptop

# ADNet form: restrict to one ADNet peer regardless of IP.
...
# ADNet peer restriction.
from-adnet=38b7dc10…, from="10.0.0.0/8" \
    ssh-ed25519 AAAAC3NzaC1lZDI1NTE5AAAAILiH… carol@home
```

`AuthorizedKeys::check(key_blob, peer_endpoint_id)` honours
`from-adnet` first (skip entries whose pinned id does not match
...
comment)` appends a fresh `from-adnet=…` line; the `ssh/`
...
```rust,ignore
use adnet_ssh::keys::authorized_keys::TrustedPeers;

let tp = TrustedPeers::new("./.adnet-data");
tp.add(peer_id)?;
assert!(tp.check(peer_id));
tp.remove(peer_id).ok();
```

This is the layer `adnet` would call from "add friend" / "remove
...
keeping the documentation build (`cargo doc -p adnet-ssh`) green.

To use the runtime:

```bash
cargo build -p adnet-ssh --features iroh
cargo test  -p adnet-ssh --features iroh
```
...
```bash
cargo test -p adnet-ssh                  # default: types + no-iroh stubs
cargo test -p adnet-ssh --features iroh  # full suite (incl. SSH-tunnel loopback)
```
...
- **`parse_one_authorized_key`** — quoted and unquoted options,
  `from-adnet`, multiple options, comments, blanks.
...
### Why a separate ALPN (`adnet/ssh-tunnel/1`)?

The rest of the runtime speaks `adnet/frame/1`, the framed
transport. SSH tunnels want a raw byte stream, not the framed
multiplexer, and naming the SSH ALPN under `adnet/` rather than
overloading `frame/1` ensures:

- a generic ADNet peer can never accidentally `accept_bi()` an
