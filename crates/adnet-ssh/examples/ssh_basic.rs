...
//!
//! ```bash
//! cargo run -p adnet-ssh --example ssh_basic --features iroh
//! ```
//!
...
fn main() {
    eprintln!(
        "adnet-ssh example `ssh_basic` requires the `iroh` feature. \
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let data_dir: PathBuf = std::env::var("ADNET_DATA_DIR")
        .map(PathBuf::from)
        .unwrap_or_else(|_| {
            std::env::temp_dir().join(format!("adnet-ssh-basic-{}", std::process::id()))
        });
