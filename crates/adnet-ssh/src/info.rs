...
//!
//! This is the ADNet equivalent of iroh-ssh's `info` subcommand.
//! It reuses the persistent identity so the value printed here
//! matches the value the rest of the ADNet runtime publishes
...
/// - the `adnet-ssh user@<id>` invitation line the operator copies
///   into chat
///
/// # Example
///
...
/// use adnet_ssh::info::render_invite;
...
pub fn render_invite(data_dir: &Path) -> SshResult<String> {
    ...
    writeln!(out, "adnet-ssh {}", env!("CARGO_PKG_VERSION"))?;
    writeln!(
        out,
        "https://github.com/rustonbsd/iroh-ssh (vendored as adnet-ssh)"
    )?;
    writeln!(out)?;
    ...
    #[cfg(feature = "iroh")]
    {
        ...
        writeln!(out, "Your adnet-ssh endpoint id: {endpoint_id}")?;
        writeln!(out, "  (short: adnet-{short})")?;
        writeln!(out)?;
        writeln!(out, "Your server adnet-ssh invite:")?;
        writeln!(out, "  adnet ssh connect {user}@{endpoint_id}")?;
        writeln!(out)?;
