...
//! endpoint derived from the ADNet persistent identity.
//!
//! This is the ADNet equivalent of iroh-ssh's `IrohSsh` builder.
//! The differences from upstream are deliberate:
//!
//! - We use `adnet_transport::iroh::IrohIdentity` as the single
...
//! - The ALPN we register is `adnet/ssh-tunnel/1`, not
//!   `iroh-ssh`. Keeping the ALPN namespaced under `adnet/`
//!   ensures an SSH-tunnel server doesn't accept connections from
//!   a generic `adnet/frame/1` peer and vice-versa.
...
/// ALPN advertised by `adnet-ssh` over QUIC. Distinct from
/// `adnet/frame/1` (the framed transport ALPN used by the rest
/// of ADNet) and from `iroh-ssh` (upstream) so the two stacks
/// can coexist on the same endpoint if needed.
pub const SSH_TUNNEL_ALPN: &[u8] = b"adnet/ssh-tunnel/1";
