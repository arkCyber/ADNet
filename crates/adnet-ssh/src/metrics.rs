...
//! global [`adnet_observability::registry::GLOBAL`] registry
//! the first time it is touched. The pattern matches the rest
//! of the ADNet codebase (see `adnet-transport/src/iroh/metrics_bridge.rs`):
//!
...
//! because the `adnet-observability` crate is a no-op stub
...
//! through the always-available `adnet_observability::metrics`
//! facade.

use std::sync::Arc;

use adnet_observability::metrics::{Counter, Gauge};
use adnet_observability::registry::{GLOBAL, Registry};
...
pub static TUNNEL_CONNECTIONS_ACCEPTED: Lazy<Arc<Counter>> = Lazy::new(|| {
    Registry::register_counter(
        ...
        "Inbound SSH-tunnel connections accepted by the adnet-ssh server.",
    )
});
...
///
/// ```text
/// rate(adnet_ssh_client_bridges_started_total[5m])
///   - rate(adnet_ssh_client_bridges_completed_total[5m])
/// ```
///
...
pub static CLIENT_BRIDGES_IN_FLIGHT: Lazy<Arc<Gauge>> = Lazy::new(|| {
    Registry::register_gauge(
        &GLOBAL,
        "adnet_ssh_client_bridges_in_flight",
...
/// the `adnet_ssh_*` series with value 0 even before the first
/// connection.
pub fn init() {
