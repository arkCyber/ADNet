...
//!
//! This is the ADNet equivalent of iroh-ssh's `server_mode`. It:
//!
//! 1. Constructs an `iroh::protocol::Router` that listens on the
//!    `adnet/ssh-tunnel/1` ALPN.
...
#[cfg(feature = "iroh")]
use adnet_transport::iroh::public_key_to_node_id;
#[cfg(feature = "iroh")]
...
impl Server {
    ...
    /// ADNet `NodeId` form of [`Self::endpoint_id`], suitable for
    /// printing next to other ADNet identities.
    pub fn node_id(&self) -> adnet_types::NodeId {
...
impl ProtocolHandler for SshTunnelHandler {
    async fn accept(&self, connection: Connection) -> Result<(), AcceptError> {
        ...
        match proxy_one_connection(connection, ssh_port, shutdown_rx).await {
            Ok(()) => {}
            Err(e) => {
                tracing::debug!("adnet-ssh: tunnel connection ended: {e}");
