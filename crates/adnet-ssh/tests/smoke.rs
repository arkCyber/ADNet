//! Smoke tests for the public-API surface of `adnet-ssh`.
//!
...
use adnet_ssh::error::{SshError, SshResult};
...
fn feature_missing_error_when_iroh_disabled() {
    ...
    let out = adnet_ssh::info::render_invite(dir.path()).unwrap();
    assert!(
...
fn resolve_data_dir_uses_cli_value_when_set() {
    let resolved = adnet_ssh::keys::resolve_data_dir(Some("/tmp/ssh-test"));
    assert_eq!(resolved.to_str().unwrap(), "/tmp/ssh-test");
}

#[test]
fn resolve_data_dir_falls_back_to_default_when_none() {
    let resolved = adnet_ssh::keys::resolve_data_dir(None);
    assert_eq!(
        resolved.to_str().unwrap(),
        "./.adnet-data",
        "default data dir must match adnet-cli's default"
    );
}

#[test]
fn resolve_data_dir_falls_back_to_default_when_empty() {
    let resolved = adnet_ssh::keys::resolve_data_dir(Some(""));
    assert_eq!(resolved.to_str().unwrap(), "./.adnet-data");
}
...
fn ssh_tunnel_alpn_is_namespace_scoped() {
    ...
    assert_eq!(
        adnet_ssh::builder::SSH_TUNNEL_ALPN,
        b"adnet/ssh-tunnel/1",
        "ALPN must stay namespaced under adnet/ and version-pinned"
    );
}
...
async fn stub_builder_errors_when_iroh_disabled() {
    ...
    let dir = tempfile::tempdir().unwrap();
    let res = adnet_ssh::IrohSshBuilder::new(dir.path())
...
async fn probe_local_ssh_fails_on_unbound_port() {
    ...
    let res = adnet_ssh::server::probe_local_ssh(port).await;
    match res {
...
fn metrics_init_does_not_panic() {
    ...
    adnet_ssh::metrics::init();
    adnet_ssh::metrics::TUNNEL_CONNECTIONS_ACCEPTED.inc();
    adnet_ssh::metrics::TUNNEL_CONNECTIONS_FAILED.inc();
    adnet_ssh::metrics::CLIENT_BRIDGES_STARTED.inc();
    adnet_ssh::metrics::CLIENT_BRIDGES_COMPLETED.inc();
}
...
fn metrics_are_shared_across_calls() {
    ...
    let before = adnet_ssh::metrics::CLIENT_BRIDGES_STARTED.render_prometheus();
    adnet_ssh::metrics::CLIENT_BRIDGES_STARTED.inc();
    let after = adnet_ssh::metrics::CLIENT_BRIDGES_STARTED.render_prometheus();
    assert_ne!(
        before, after,
...
fn in_flight_gauge_is_balanced() {
    ...
    adnet_ssh::metrics::CLIENT_BRIDGES_IN_FLIGHT.inc();
    adnet_ssh::metrics::CLIENT_BRIDGES_IN_FLIGHT.inc();
    adnet_ssh::metrics::CLIENT_BRIDGES_IN_FLIGHT.dec();
    let mid = adnet_ssh::metrics::CLIENT_BRIDGES_IN_FLIGHT.get();
    adnet_ssh::metrics::CLIENT_BRIDGES_IN_FLIGHT.dec();
    let after = adnet_ssh::metrics::CLIENT_BRIDGES_IN_FLIGHT.get();
    assert_eq!(
        mid - after,
        1,
