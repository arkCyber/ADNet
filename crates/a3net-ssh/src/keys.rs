//! Persistent-key resolution helpers for `a3net-ssh`.
//!
//! iroh-ssh stores its server key at `~/.ssh/irohssh_ed25519` and
//! its service-mode key at `~/.ssh/irohssh_service_ed25519`. A3Net
//! already mints a single persistent Ed25519 identity at
//! `<data-dir>/iroh_secret_key` via
//! [`a3net_transport::iroh::IrohIdentity::load_or_create`], and we
...
//! - The same key is shared with the `a3net/frame/1` ALPN, the
//!   blob store, and the gossip layer (because the iroh endpoint
//!   is the same one the rest of the A3Net runtime is using).
//!
...
//! are permitted to open tunnels to this node. In A3Net's trust model
...
#[cfg(feature = "iroh")]
use a3net_transport::iroh::IrohIdentity;
...
/// `a3net_transport::iroh::IROH_SECRET_KEY_FILE` so callers don't
...
///
/// Returns the same identity that the rest of the A3Net runtime
/// uses, so the endpoint id printed by `a3net ssh info` matches
...
/// - Otherwise default to `./.a3net-data` (matching
///   `a3net-cli::Cli::data_dir`).
pub fn resolve_data_dir(cli_data_dir: Option<&str>) -> PathBuf {
    match cli_data_dir {
        Some(d) if !d.is_empty() => PathBuf::from(d),
        _ => PathBuf::from("./.a3net-data"),
    }
}
...
/// Path to the `authorized_keys` file relative to the A3Net data
/// directory. Mirrors OpenSSH's `~/.ssh/authorized_keys` layout.
const AUTHORIZED_KEYS_FILE: &str = "ssh/authorized_keys";

/// Path to the trusted-peers file relative to the A3Net data
...
/// (which restricts the connecting IP address) and the A3Net-specific
/// `from-a3net="<endpoint-id>` option (which restricts which A3Net
...
pub struct AuthorizedKeyEntry {
    ...
    /// Restrict to a specific A3Net endpoint id (the `from-a3net` option).
    pub from_a3net_id: Option<iroh::EndpointId>,
}
...
impl AuthorizedKeys {
    ...
    pub fn load(&self) -> Vec<AuthorizedKeyEntry> {
        ...
        let file = match fs::File::open(&path) {
            ...
            Err(e) => {
                tracing::debug!("a3net-ssh: failed to open authorized_keys at {path:?}: {e}");
                return Vec::new();
            }
        };
        ...
        for (line_no, line) in io::BufReader::new(file).lines().enumerate() {
            match line {
                ...
                Err(e) => {
                    tracing::debug!(
                        "a3net-ssh: skipping line {line_no} in authorized_keys: {e}"
                    );
                }
            }
        }
        tracing::debug!(
            "a3net-ssh: loaded {} authorized_keys entries from {:?}",
            entries.len(),
            path
        );
        entries
    ...
    /// no matching entry.
    ///
    /// The `from-a3net` option on an entry takes priority: if an entry
    /// specifies `from-a3net="<peer-id>"` and the connecting peer's
    ...
    pub fn check(&self, key_blob: &[u8], peer_endpoint_id: iroh::EndpointId) -> bool {
        ...
        for entry in entries {
            // `from-a3net` is an A3Net-specific restriction: skip if the
            // peer endpoint id does not match.
            if let Some(ref required) = entry.from_a3net_id
    ...
    pub fn ensure(&self) -> io::Result<()> {
        ...
        if !file_path.exists() {
                ...
                .open(&file_path)
            {
                let _ = writeln!(
                    f,
                    "# A3Net SSH authorized keys\n\
                    # Format: one OpenSSH authorized_keys line per peer.\n\
                    # Supported A3Net-specific options:\n\
                    #   from-a3net=<endpoint-id>  — restrict to a specific A3Net peer\n\
    ...
    /// ```ignore
    /// from-a3net=<endpoint-id> ssh-ed25519 <base64-blob> <comment>
    /// ```
    ///
    ...
    pub fn add_peer(&self, peer_endpoint_id: iroh::EndpointId, key_blob: &[u8], comment: &str) -> io::Result<()> {
        ...
        writeln!(
            file,
            "from-a3net={} ssh-ed25519 {} {}",
            peer_endpoint_id,
            blob_b64,
            comment
        )?;
        tracing::info!(
            "a3net-ssh: added authorized_keys entry for peer {}",
            peer_endpoint_id
        );
        Ok(())
    }
...
pub(crate) fn parse_one_authorized_key(line: &str) -> Option<AuthorizedKeyEntry> {
        ...
        regex::Regex::new(r#"([a-zA-Z0-9._-]+)="([^"]*)""#)
    {
        for cap in quoted_re.captures_iter(options_str) {
            ...
            } else if name == "from-a3net"
                && let Ok(id) = value.parse::<iroh::EndpointId>()
            {
                from_a3net_id = Some(id);
            }
        }
    }
...
impl TrustedPeers {
    ...
    pub fn add(&self, endpoint_id: iroh::EndpointId) -> io::Result<()> {
        ...
        tracing::info!(
            "a3net-ssh: added {} to trusted_peers",
            endpoint_id
        );
        Ok(())
    }
    ...
    pub fn remove(&self, endpoint_id: iroh::EndpointId) -> io::Result<bool> {
        ...
        tracing::info!(
            "a3net-ssh: removed {} from trusted_peers",
            endpoint_id
        );
        Ok(true)
    }
    ...
    fn do_load(&self) {
        let file = match fs::File::open(&self.path) {
            ...
            Err(e) => {
                tracing::debug!(
                    "a3net-ssh: failed to open trusted_peers at {:?}: {e}",
                    self.path
                );
                return;
            }
        ...
        tracing::debug!(
            "a3net-ssh: loaded {} trusted peers from {:?}",
            set.len(),
            self.path
        );
...
mod tests {
    ...
    #[test]
    fn parse_authorized_key_with_a3net_option() {
        let line =
        ...
        let expected_id: iroh::EndpointId = "38b7dc10df96005255c3beaeaeef6cfebd88344aa8c85e1dbfc1ad5e50f372ac"
            .parse()
            .unwrap();
        assert_eq!(entry.from_a3net_id, Some(expected_id));
    ...
    fn parse_authorized_key_with_from_option() {
        ...
        assert!(entry.from_a3net_id.is_none());
    }
    ...
    fn authorized_keys_add_peer() {
        ...
        assert!(content.contains("from-a3net="), "should contain from-a3net option");
        assert!(content.contains("ssh-ed25519"), "should contain key type");
    }

    #[test]
    fn parse_authorized_key_with_multiple_options() {
        // from-a3net and from= together.
        let line = r#"from-a3net=38b7dc10df96005255c3beaeaeef6cfebd88344aa8c85e1dbfc1ad5e50f372ac, from="192.168.1.0/24" ssh-ed25519 AAAAC3NzaC1lZDI1NTE5AAAAC4 test"#;
        let entry = parse_one_authorized_key(line).unwrap();
        assert_eq!(entry.key_type, "ssh-ed25519");
        assert!(entry.from_pattern.is_some());
        assert!(entry.from_a3net_id.is_some());
    }
