//! `a3net-ssh` — SSH tunneled over an iroh QUIC endpoint.
//!
...
//! A3Net stack — iroh 1.0.3, the persistent Ed25519 identity in
//! `<data-dir>/iroh_secret_key`, and the `a3net/frame/1` ALPN we
...
//!   [`a3net_transport::iroh::IrohIdentity`] so that the SSH
//!   endpoint id is the same value as the A3Net node id and doesn't
...
//!   (`install` / `uninstall`) is intentionally out of scope — A3Net
//!   already ships `a3net-relay` and `a3net-node` as long-running
//!   services; the `a3net-ssh server` REPL command is the supported
//!   way to run a tunnel.
//!
//! # Crate layout
//!
...
//! // with friends as `a3net ssh connect <user>@<id>`.
...
//!   wired up automatically by `a3net-cli`'s `/ssh connect`
//!   slash command.
