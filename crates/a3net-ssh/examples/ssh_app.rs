//! Realistic a3net-ssh app example.
//!
//! Walks through how an A3Net node would:
//!
...
//!
//! ```bash
//! cargo run -p a3net-ssh --example ssh_app --features iroh
//! ```
//!
...
fn main() {
    eprintln!(
        "a3net-ssh example `ssh_app` requires the `iroh` feature. \
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let banner = a3net_ssh::info::render_invite(std::env::temp_dir().as_path())?;
    println!("{banner}");
    ...
    println!("Tunnel ALPN: {}", a3net_ssh::builder::SSH_TUNNEL_ALPN.len());
    println!("ALPN bytes: {:?}", a3net_ssh::builder::SSH_TUNNEL_ALPN);
