...
impl MigrationRunner {
    ...
    pub async fn run(&self, conn: &DatabaseConnection) -> DatabaseResult<MigrationResult> {
        let create_table = r#"
            CREATE TABLE IF NOT EXISTS _adnet_migrations (
        ...
        let applied: Vec<AppliedMigration> = conn
            .fetch_all("SELECT version, name, applied_at FROM _adnet_migrations ORDER BY version")
            .await
            .unwrap_or_default();
                ...
                match result {
                    Ok(_) => {
                        ...
                        let record_sql = format!(
                            "INSERT INTO _adnet_migrations (version, name, applied_at) VALUES ({}, '{}', '{}')",
                            migration.version,
