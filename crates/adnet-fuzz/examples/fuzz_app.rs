...
//! `adnet-fuzz` targets attack, but with a hand-curated corpus
//! instead of mutated random inputs. Useful as a quick smoke test
//! after touching any of:
//!
//!   * `adnet-types` — Announcement, NodeId, Cid, ContentHash
//!   * `adnet-dht`   — DhtMessage wire variants
//!   * `adnet-blobstore` — BitswapCodec, WantlistManager
//!
//! Run with:
//!   cargo run -p adnet-fuzz --example fuzz_app --release
...
fn main() {
    println!("=== adnet-fuzz corpus replay ===\n");
