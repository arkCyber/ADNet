...
//!
//! This is the A3Net port of the original
...
//! that:
//!
//! - hangs off the same `a3net-ipc` JSON-RPC machinery as the
//!   other A3Net services (typed `params` / `result`, fail-closed
//!   `Validate` gates);
...
//! [`a3net_types::social_feed`], serialised via `serde_json`.
...
use a3net_types::invariants::ReactionType;
...
pub use a3net_ipc::validation::ValidationPolicy as SocialFeedValidationPolicy;
...
impl Default for SocialFeedIpcConfig {
    fn default() -> Self {
        Self {
            socket_path: std::env::temp_dir().join("a3net_social_feed.sock"),
...
impl SocialFeedIpcService {
    ...
    fn check<T: a3net_ipc::validation::Validate>(&self, value: &T, what: &str) -> ValidationOutcome {
    ...
    fn gate<T: a3net_ipc::validation::Validate>(
        &self,
        value: &T,
        what: &str,
...
mod tests {
    ...
    async fn end_to_end_json_rpc_round_trip() {
        use a3net_ipc::client::json_rpc_call;
        let dir = tempfile::tempdir().unwrap();
