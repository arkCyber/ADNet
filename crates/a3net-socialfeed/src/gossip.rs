...
//! `Mutex<HashMap<…>>` inside the Unix socket service. The A3Net
//! port supplies an explicit gossip envelope so that fans-out of
//! "moments" over [`a3net_gossip`] stay type-safe end to end.
//!
//! # Topic naming
//!
//! All social-feed messages are routed through a topic of the form
//! `a3net-socialfeed-{room_id}` where `room_id` is the social-feed
...
//! ([`a3net_types::invariants::Visibility`]) for posts; comments
...
impl Envelope {
    ...
    /// Lift into an A3Net gossip [`AnnouncementPayload`]. The
    /// envelope lives in the `payload` JSON field; the typed
    /// social-feed payload doesn't share shape with the
    /// file-announcement record in `a3net-types::announce` (the
