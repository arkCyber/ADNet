...
//! ADNet-friendly allowlist / denylist tri-state. Each
...
/// ADNet-flavoured accessor — names the three supported modes
...
impl AccessControl for AllowlistAccess {
    async fn on_connect(&self, request: &ClientRequest) -> Access {
        ...
        if self.allow.contains(&id) {
            trace!(endpoint = %id.fmt_short(), "adnet-relay: allowlist admit");
            Access::Allow
        } else {
            trace!(endpoint = %id.fmt_short(), "adnet-relay: allowlist deny");
...
impl AccessControl for DenylistAccess {
    async fn on_connect(&self, request: &ClientRequest) -> Access {
        ...
        if self.deny.contains(&id) {
            trace!(endpoint = %id.fmt_short(), "adnet-relay: denylist reject");
            Access::Deny { reason: None }
        } else {
            trace!(endpoint = %id.fmt_short(), "adnet-relay: denylist admit");
            Access::Allow
        }
    }
}
