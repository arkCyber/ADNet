...
//! ## Metric names
//!
//! All names are prefixed with `adnet_relay_*` so the
...
use adnet_observability::registry::{Registry, GLOBAL as OBSERVABILITY_GLOBAL};
...
mod tests {
    ...
    fn relay_metrics_register_all() {
        ...
        assert!(names.contains(&"adnet_relay_requests_total".to_string()));
        assert!(names.contains(&"adnet_relay_forwards_total".to_string()));
        assert!(names.contains(&"adnet_relay_policy_filtered_total".to_string()));
        assert!(names.contains(&"adnet_relay_bytes_sent_total".to_string()));
        assert!(names.contains(&"adnet_relay_bytes_received_total".to_string()));
        assert!(names.contains(&"adnet_relay_upstream_errors_total".to_string()));
        assert!(names.contains(&"adnet_relay_active_sessions".to_string()));
    ...
    fn billing_metrics_register_all() {
        ...
        let names: Vec<String> =
            snap.metrics.iter().map(|x| x.name().to_string()).collect();
        assert!(names.contains(&"adnet_relay_pledges_total".to_string()));
        assert!(names.contains(&"adnet_relay_pledge_errors_total".to_string()));
        assert!(names.contains(&"adnet_relay_receipts_issued_total".to_string()));
        assert!(names.contains(&"adnet_relay_receipts_redeemed_total".to_string()));
        assert!(names.contains(&"adnet_relay_receipt_redeem_errors_total".to_string()));
        assert!(names.contains(&"adnet_relay_receipt_redeem_success_total".to_string()));
        m.pledges.inc_by(5);
