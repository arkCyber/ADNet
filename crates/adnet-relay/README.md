...
> ADNet mesh 流量的 WAN 中继 HTTP 代理：让 NAT 之后的节点也能相互访问。 / WAN relay HTTP proxy that lets ADNet mesh nodes behind NAT reach each other.

## 概览 (Overview)

`adnet-relay` 既是 *客户端*（构造代理 URL），也是 *服务端*（一个 axum 实现的轻量 HTTP 转发器）。
...
- **服务端** [`RelayServer`] 监听端口，校验 path 必须以 `/blobs/...` 开头（防滥用），
  转发到内网 mesh HTTP 端点。重试 / backoff 由 `adnet-resilience` 提供。
...
```rust
use adnet_relay::{RelayClient, RelayConfig, RelayServer, BillingMode};
```
...
```rust,no_run
use adnet_relay::{BillingMode, RelayServer};
...
```rust,no_run
use adnet_relay::{RelayClient, RelayConfig};
...
```rust,no_run
use adnet_relay::{HostPolicy, RelayConfig};
...
- **`adnet-node`** 在 `RelayEndpointInfo` 处把 relay URL 注入节点的传输层；fetch 走不通时
  自动通过 relay 兜底。
- **`adnet-cli`** 暴露 `ray relay start / stop / status` 命令，直接复用 `RelayServer` 与
