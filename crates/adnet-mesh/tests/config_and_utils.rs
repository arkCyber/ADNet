//! Integration tests for adnet-mesh that focus on:
...
use adnet_mesh::client::test_export::extract_multipart_bodies_for_tests;
use adnet_mesh::server::MeshConfig;
use adnet_mesh::server::MeshServer;
use adnet_mesh::server::test_export::strip_prefix_for_tests;
...
fn mesh_config_default_is_serialised_as_camel_case() {
    ...
    // so it can be embedded in adnet-cli's config JSON without an
