//! `adnet-mesh` 应用示例：导入多个 blob 启动 mesh server，然后从另一个
//! peer base 列表里并发 fetch。这是 `adnet-node` 在传输层不可用时的兜底逻辑。
//!
//! 运行：`cargo run -p adnet-mesh --example mesh_app`

use std::io::Write;
use std::sync::Arc;

use adnet_blobstore::{BlobStore, CHUNK_SIZE};
