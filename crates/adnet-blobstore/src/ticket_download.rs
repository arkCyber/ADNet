...
use adnet_types::ticket::BlobTicket;
use adnet_types::{ContentHash, NodeId, RangeSpec};
...
impl<T: ChunkFetcher + 'static> TicketDownloadService<T> {
    ...
    fn resolve_peers(&self, tickets: &[BlobTicket]) -> TicketDownloadResult<Vec<ResolvedPeer>> {
        ...
        for ticket in tickets {
            ...
            // Convert adnet_types::NodeAddr to replicator::NodeAddr
...
mod tests {
    ...
    fn make_test_ticket() -> BlobTicket {
        ...
        BlobTicket::whole(&node_id, &adnet_types::NodeAddr::default(), &hash)
    }
