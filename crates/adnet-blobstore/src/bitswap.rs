...
//!
//! This module implements the core Bitswap protocol for ADNet, enabling:
//!
//! ## Key Features
//!
...
pub struct BitswapEngine {
    ...
    /// global PeerScore so the rest of ADNet (gossipsub, chat-trust
    ...
    reputation: Option<adnet_reputation::ReputationReporter>,
}
...
impl BitswapEngine {
    ...
    /// ADNet (gossipsub, chat-trust fusion) can react to the same
    ...
    #[cfg(feature = "reputation")]
    fn peer_id_to_node_id(peer_id: &str) -> adnet_types::NodeId {
    ...
    pub fn start_request_for_peer(&mut self, peer_id: &str, block: &adnet_types::ContentHash) -> bool {
                    ...
                    #[cfg(feature = "reputation")]
                    {
                        ...
                        if wanted {
                            if let Some(rep) = self.reputation.as_ref() {
                                ...
                                adnet_reputation::reporter::BitswapSignal(rep)
                                    .invalid(node, adnet_reputation::event::InvalidReason::Other);
                            }
                        }
                    }
                }
