...
//! ## Metric names
//!
//! All names are prefixed with `adnet_blob_*` so the
...
pub struct BlobMetrics {
    /// `adnet_blob_imports_total` — total successful
    /// `import_file_sync` calls.
    pub imports: Arc<Counter>,
    /// `adnet_blob_import_errors_total` — total blob-layer
    ...
    /// `adnet_blob_put_bytes_total` — total successful
    /// `put_bytes_sync` calls.
    pub put_bytes: Arc<Counter>,
    /// `adnet_blob_exports_total` — total
    /// `export_to_file_sync` calls.
    pub exports: Arc<Counter>,
    /// `adnet_blob_removes_total` — total `remove` calls
    /// that returned `Ok(true)`.
    pub removes: Arc<Counter>,
    /// `adnet_blob_store_size_bytes` — total bytes used
    ...
    /// `adnet_blob_blobs_total` — count of complete blobs
    /// in the store.
    pub blobs_total: Arc<Gauge>,
    /// `adnet_blob_read_hash_mismatch_total` — total read
    ...
    /// `adnet_blob_import_path_rejected_total` — total
...
mod tests {
    ...
    fn register_creates_all_metrics() {
        ...
        assert!(names.contains(&"adnet_blob_imports_total".to_string()));
        assert!(names.contains(&"adnet_blob_import_errors_total".to_string()));
        assert!(names.contains(&"adnet_blob_put_bytes_total".to_string()));
        assert!(names.contains(&"adnet_blob_exports_total".to_string()));
        assert!(names.contains(&"adnet_blob_removes_total".to_string()));
        assert!(names.contains(&"adnet_blob_store_size_bytes".to_string()));
        assert!(names.contains(&"adnet_blob_blobs_total".to_string()));
        // DO-178C DAL-B metrics — SR-1..SR-5.
        assert!(names.contains(&"adnet_blob_read_hash_mismatch_total".to_string()));
        assert!(names.contains(&"adnet_blob_remove_rejected_total".to_string()));
        assert!(names.contains(&"adnet_blob_quarantined_total".to_string()));
        assert!(names.contains(&"adnet_blob_bytes_deleted_total".to_string()));
        assert!(names.contains(&"adnet_blob_import_path_rejected_total".to_string()));
        m.imports.inc_by(2);
        m.import_errors.inc();
        m.put_bytes.inc();
        m.exports.inc_by(3);
