...
> Disk-backed content-addressed blob store and NAS-style logical namespace for ADNet. BLAKE3-chunked storage with iroh-blobs layout parity, an optional iroh-backed Bao-verified transport, and a DO-178C DAL-A WebDAV surface.

## 概览 (Overview)

`adnet-blobstore` is the storage layer of the ADNet workspace. It provides:
...
The crate sits between the chat/feed layers and the WebDAV/iroh transport layers; every byte ADNet stores under a content hash ultimately flows through here.
...
The crate is a workspace path-dependency; reference it from any ADNet member with:

```toml
[dependencies]
adnet-blobstore = { workspace = true }
```

Then in source:

```rust
use adnet_blobstore::{BlobStore, BlobReader};
use adnet_blobstore::namespace::{Nas, NamespaceRead, NamespaceWrite};
use adnet_blobstore::filename::safe_filename;
```
...
2. **P2P file sharing via Bao-verified chunks.** `adnet-share` calls `walk_import` to build a `Collection` manifest, sends the `ShareTicket`, and the receiver reconstructs content via `BaoTree`/`BaoProof`. The `examples/round_trip.rs` example demonstrates the byte-for-byte invariants.
