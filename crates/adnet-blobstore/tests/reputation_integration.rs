//! Integration tests for the `adnet-reputation` × `adnet-blobstore`
//! bitswap bridge. Requires the `reputation` feature on
//! `adnet-blobstore` so that `BitswapEngine::with_reputation` is in
//! scope and `process_message` feeds `BitswapSignal` events into
//! the global PeerScore.

#![cfg(feature = "reputation")]

use adnet_blobstore::bitswap::{
    BitswapEngine, BitswapMessage,
};
use adnet_reputation::{PeerScoreTable, ReputationParams, ReputationReporter};
...
fn blake3_node(peer_id: &str) -> adnet_types::NodeId {
    let h = blake3::hash(peer_id.as_bytes());
    let mut bytes = [0u8; 32];
    bytes.copy_from_slice(&h.as_bytes()[..32]);
    adnet_types::NodeId::from_bytes(&bytes).expect("32 bytes")
}