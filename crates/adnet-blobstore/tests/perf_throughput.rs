...
//
// Performance baseline tests for `adnet-blobstore`.
//
...
//
//     cargo test -p adnet-blobstore --test perf_throughput -- --ignored --nocapture
//
...
use adnet_blobstore::{BlobImporter, BlobReader, BlobStore, chunked::CHUNK_SIZE};
...
async fn put_blob(data: &[u8]) -> (Arc<BlobStore>, adnet_types::ContentHash, Arc<TempDir>) {
...
//   `crates/adnet-blobstore` (32 MiB writer, 4 readers, 4 KiB
...
async fn concurrent_writer_does_not_starve_readers() {
    ...
    for _ in 0..N_READERS {
        ...
        readers.push(tokio::spawn(async move {
            let _keep = dir;
            let range = adnet_types::ByteRange::new(0, CHUNK as u64).unwrap();
