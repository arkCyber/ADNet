...
//! `crates/adnet-blobstore/SAFETY_CASE.md` and is independently
...
use adnet_blobstore::chunked::ChunkError;
use adnet_blobstore::*;
...
fn sr_1_verified_read_rejects_chunk_corruption() {
    ...
    let m = adnet_blobstore::metrics::BlobMetrics::register(&registry);
...
fn sr_4_quarantine_moves_corrupt_blob() {
    ...
    let whole = store.read_range_sync_verified_spec(&h, &adnet_types::RangeSpec::All);
...
fn sr_5_import_path_rejected_counter_increments() {
    let dir = tempdir().unwrap();
    let m = adnet_blobstore::metrics::BlobMetrics::register(&std::sync::Arc::new(
...
fn robustness_32mib_import_completes_quickly() {
    ...
    // within i64 range.
    let g = adnet_blobstore::metrics::blob_metrics()
        .store_size_bytes
        .get();
